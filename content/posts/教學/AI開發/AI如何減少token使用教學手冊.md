...
- [第八章 codebase-memory-mcp 思維——Hybrid LSP Memory](#第八章-codebase-memory-mcp-思維hybrid-lsp-memory)
  ...
  - [8.3 14 個 MCP 工具](#83-14-個-mcp-工具)
...
- [第十一章 SSDLC 如何減少 Token](#第十一章-ssdlc-如何減少-token)
  - [11.1 Requirement 階段](#111-requirement-階段)
  - [11.2 Design 階段](#112-design-階段)
  - [11.3 Development 階段](#113-development-階段)
  - [11.4 Testing 階段](#114-testing-階段)
  - [11.5 Deployment 階段](#115-deployment-階段)
  - [11.6 Maintenance 階段](#116-maintenance-階段)
- [第十二章 Agent Team 如何節省 Token](#第十二章-agent-team-如何節省-token)
  - [12.1 Agent Team 架構](#121-agent-team-架構)
  - [12.2 各 Agent 職責與 Token 策略](#122-各-agent-職責與-token-策略)
  - [12.3 單 Agent VS 多 Agent 比較](#123-單-agent-vs-多-agent-比較)
- [第十三章 大型 Web Application 開發策略](#第十三章-大型-web-application-開發策略)
  - [13.1 建立 System Knowledge Base](#131-建立-system-knowledge-base)
  - [13.2 五大 Memory 層](#132-五大-memory-層)
  - [13.3 Token 節省實務](#133-token-節省實務)
- [第十四章 Framework Upgrade 策略](#第十四章-framework-upgrade-策略)
  - [14.1 升級場景分析](#141-升級場景分析)
  - [14.2 Migration Knowledge Graph](#142-migration-knowledge-graph)
  - [14.3 避免重複分析的策略](#143-避免重複分析的策略)
- [第十五章 Reverse Engineering 策略](#第十五章-reverse-engineering-策略)
  - [15.1 Legacy System 盤點](#151-legacy-system-盤點)
  - [15.2 四層 Graph 架構](#152-四層-graph-架構)
  - [15.3 降低 Token 方法](#153-降低-token-方法)
- [第十六章 Prompt Engineering 節省 Token 技巧](#第十六章-prompt-engineering-節省-token-技巧)
  - [16.1 架構分析 Prompt](#161-架構分析-prompt)
  - [16.2 程式碼分析 Prompt](#162-程式碼分析-prompt)
  - [16.3 Bug 修復 Prompt](#163-bug-修復-prompt)
  - [16.4 SSDLC Prompt](#164-ssdlc-prompt)
  - [16.5 Security Review Prompt](#165-security-review-prompt)
  - [16.6 Unit Test Prompt](#166-unit-test-prompt)
  - [16.7 Refactoring Prompt](#167-refactoring-prompt)
  - [16.8 Framework Upgrade Prompt](#168-framework-upgrade-prompt)
  - [16.9 Reverse Engineering Prompt](#169-reverse-engineering-prompt)
- [第十七章 Claude Code 節省 Token 最佳實務](#第十七章-claude-code-節省-token-最佳實務)
  - [17.1 CLAUDE.md 配置](#171-claudemd-配置)
  - [17.2 Memory 機制](#172-memory-機制)
  - [17.3 Context Engineering](#173-context-engineering)
  - [17.4 Sub Agent 策略](#174-sub-agent-策略)
  - [17.5 MCP 整合](#175-mcp-整合)
- [第十八章 GitHub Copilot 節省 Token 最佳實務](#第十八章-github-copilot-節省-token-最佳實務)
  - [18.1 Copilot Instructions](#181-copilot-instructions)
  - [18.2 Prompt Files](#182-prompt-files)
  - [18.3 Agent Mode](#183-agent-mode)
  - [18.4 MCP 整合](#184-mcp-整合)
  - [18.5 Workspace Context 最佳化](#185-workspace-context-最佳化)
- [第十九章 企業級 AI 成本治理](#第十九章-企業級-ai-成本治理)
  - [19.1 AI Governance 框架](#191-ai-governance-框架)
  - [19.2 AI Cost Management](#192-ai-cost-management)
  - [19.3 AI Token Monitoring](#193-ai-token-monitoring)
  - [19.4 AI Usage / Agent / Security Policy](#194-ai-usage--agent--security-policy)
- [第二十章 建立企業級 Token 最佳化框架](#第二十章-建立企業級-token-最佳化框架)
  - [20.1 Enterprise Token Optimization Architecture](#201-enterprise-token-optimization-architecture)
  - [20.2 七層架構設計](#202-七層架構設計)
  - [20.3 導入流程](#203-導入流程)
- [第二十一章 實戰案例](#第二十一章-實戰案例)
  - [21.1 大型銀行核心系統升級](#211-大型銀行核心系統升級)
  - [21.2 百萬行程式碼逆向工程](#212-百萬行程式碼逆向工程)
  - [21.3 Spring Boot 升級專案](#213-spring-boot-升級專案)
  - [21.4 Vue3 重構專案](#214-vue3-重構專案)
- [第二十二章 最佳實務總結](#第二十二章-最佳實務總結)
  - [22.1 Top 100 Token 節省技巧](#221-top-100-token-節省技巧)
  - [22.2 企業導入檢查表](#222-企業導入檢查表)
  - [22.3 企業成熟度模型](#223-企業成熟度模型)
  - [22.4 結論與建議](#224-結論與建議)
- [附錄](#附錄)
...
GitNexus（v1.6.8，43.3K ★）的核心理念是將 Git Repository 索引為可查詢的知識庫，採用**預計算關聯智慧（Precomputed Relational Intelligence）**——在索引時預先計算叢集、追蹤、評分，使工具在單次呼叫中即可回傳完整上下文，而非讓 LLM 自行探索。提供 16 個 MCP（Model Context Protocol）工具（11 個單一倉庫 + 5 個跨倉庫群組）、另外 2 個 Prompts 與 4 個 Skills，供 AI Agent 精準存取 Repository 資訊。

**底層資料庫引擎**：LadybugDB（內嵌式圖譜資料庫，支援 Cypher 查詢）。
...
Graphify（v0.9.2，74.4K ★）是一個以 Python 開發的程式碼知識圖譜建構工具，支援 **36 種 tree-sitter grammars** 的程式碼以及文件、PDF、圖片、影片等多媒體內容。使用 Tree-sitter AST 解析器在本地端完成程式碼分析，不需要將程式碼傳送給 AI API，本身不消耗 AI Token。
...
Graphify v0.9.2 新增了 MCP Server 模式，讓 AI Agent 可以透過標準化的 MCP 協定查詢圖譜：
...
codebase-memory-mcp 採用 **Hybrid LSP** 架構——對 10 種主流語言（TypeScript、Python、Java、Go、Rust、C/C++、C#、Ruby、PHP、Swift）提供語義型別解析，其餘 148 種語言使用 Tree-sitter 結構解析。

```mermaid
graph LR
    A[Source Code] --> B{Language?}
    B -->|10 種主流語言| C[Hybrid LSP<br/>語義型別解析]
    B -->|148 種其他語言| D[Tree-sitter<br/>結構解析]
    C --> E[SQLite Index]
    D --> E
    E --> F[14 MCP Tools]
...
CodeGraph（v1.1.6，56.1K ★）是一個以 TypeScript 開發的程式碼知識圖譜工具，其核心差異化特點是 **自動同步（Auto-Sync）**：每次檔案變更時自動更新圖譜，無需手動重建。
...
```費用計算範例（Claude Sonnet 4）：
無 Ponytail：500 行輸出 × 5 tokens/行 = 2,500 tokens × $15/M = $0.0375
有 Ponytail：230 行輸出 × 5 tokens/行 = 1,150 tokens × $15/M = $0.0173
單次節省：$0.0202（54%）

20 人團隊每日 100 次 Code Generation：
月度節省：$0.0202 × 100 × 22 × 20 = $890
```
...
## 第十三章 大型 Web Application 開發策略

### 13.1 建立 System Knowledge Base
...
## 第十五章 Reverse Engineering 策略

### 15.1 Legacy System 盤點
...
## 第十六章 Prompt Engineering 節省 Token 技巧
...
### 16.9 Reverse Engineering Prompt
...
## 第十七章 Claude Code 節省 Token 最佳實務
...
## 第十八章 GitHub Copilot 節省 Token 最佳實務

### 18.1 Copilot Instructions
...
### 19.4 AI Usage / Agent / Security Policy
...
## 第二十章 建立企業級 Token 最佳化框架

### 20.1 Enterprise Token Optimization Architecture
...
│   └── → RTK（CLI Proxy，80% 節省）
│
├── 壓縮整個 Context Window（對話歷史、檔案內容）
│   └── → Headroom（60-95% 節省，與 RTK 互補）
│
├── 建立程式碼知識圖譜
│   ├── 需要可視化 + 團隊共享
│   │   └── → Graphify（MCP Server + HTTP 共享）
│   ├── 超大型專案（100 萬行+）
│   │   └── → codebase-memory-mcp（158 語言，極速索引）
│   ├── 多倉庫 + 語義搜尋
│   │   └── → GitNexus（Multi-Repo + LadybugDB）
│   └── 需要自動同步 + 框架感知
│       └── → CodeGraph（Auto-Sync + Framework-Aware）
│
├── 建立全專案知識圖譜 + 互動儀表板
│   └── → Understand-Anything（7 Agent Pipeline）
│
└── 減少 AI 生成的不必要程式碼
    └── → Ponytail（YAGNI Ladder，-54% 程式碼）
```
