...
- [第 2 章：Skills 平台深度解析（GitHub Copilot + Claude Code）](#第-2-章skills-平台深度解析github-copilot--claude-code)
  ...
  - [2.7 gh skill CLI（GitHub CLI 整合）](#27-gh-skill-cligithub-cli-整合)
  - [2.8 Skill 內容生命週期與 Token 預算](#28-skill-內容生命週期與-token-預算)
...
- [第 4 章：Skills 設計最佳實務](#第-4-章skills-設計最佳實務)
  ...
  - [4.6 社群設計模式參考（Community Patterns）](#46-社群設計模式參考community-patterns)
    - [4.6.1 CONTEXT.md 共用語言模式](#461-contextmd-共用語言模式mattpocock-skills)
    - [4.6.2 反合理化表格模式](#462-反合理化表格模式addyosmani-agent-skills)
    - [4.6.3 Agent Personas 模式](#463-agent-personas-模式addyosmani-agent-skills)
    - [4.6.4 生命週期導向組織模式](#464-生命週期導向組織模式addyosmani-agent-skills)
    - [4.6.5 四大失敗模式框架](#465-四大失敗模式框架mattpocock-skills)
    - [4.6.6 User-invoked vs Model-invoked 設計模式](#466-user-invoked-vs-model-invoked-設計模式mattpocock-skills)
    - [4.6.7 自主執行模式](#467-自主執行模式addyosmani-agent-skills-build-auto)
    - [4.6.8 Skills 安裝器生態](#468-skills-安裝器生態)
...
> - **[github/awesome-copilot](https://github.com/github/awesome-copilot)**（250+ Skills）— GitHub 官方社群 Skills 收藏庫，涵蓋開發、測試、安全、雲端（AWS / Azure）、資料庫、DevOps、MCP、IoT、GTM 等類別，持續由社群貢獻擴充
...
# Skill 執行期間從可用工具池中移除的工具
# 接受空格分隔、逗號分隔或 YAML list
# 限制在使用者傳送下一則訊息時清除
# 適用於背景迴圈等不應向使用者提問的場景
---
```
...
> 叫用 `/fix-and-branch 42 fix/login-bug` 時，`$issue` = `42`、`$branch` = `fix/login-bug`。
> 具名參數依 `arguments` 欄位中的順序對應叫用時傳入的位置參數。
...
Claude Code 將原本分開管理的 `.claude/commands/` 合併進 Skills 機制。舊的 Commands 目錄仍**向後相容**——已有的 `.claude/commands/*.md` 會自動被視為 Skills 載入，無需遷移。新建的命令建議直接建在 `.claude/skills/` 中。
...
│   │   └── deploy/
│   │       └── SKILL.md
│   └── commands/                  # ⬅️ 向後相容，不需遷移
│       └── legacy-format.md       # 自動等同 Skill 載入
```

**Monorepo 巢狀 Skills（Qualified Names）**：

在 Monorepo 中，Skills 可定義在各子專案的 `.claude/skills/` 目錄中。叫用時使用**合格名稱（Qualified Name）**以 `<package>:<skill>` 格式區分同名 Skill：

```bash
# Monorepo 結構
📦 monorepo/
├── apps/
│   ├── web/.claude/skills/deploy/SKILL.md      # web 的部署 Skill
│   └── api/.claude/skills/deploy/SKILL.md      # api 的部署 Skill
└── packages/
    └── shared/.claude/skills/lint/SKILL.md

# 叫用方式（Qualified Names）
/apps/web:deploy     # 叫用 web 子專案的 deploy Skill
/apps/api:deploy     # 叫用 api 子專案的 deploy Skill
...
> **Public Preview**：`gh skill` 自 GitHub CLI **v2.90.0** 起可用，仍在公開預覽階段。
...
當使用者或 Claude 叫用一個 Skill 時，渲染後的 `SKILL.md` 內容以單一訊息進入對話上下文，並在整個 Session 中持續存在。Claude Code **不會**在後續回合重新讀取 Skill 檔案，因此應將需要在整個任務過程中生效的指令撰寫為「持續性指令」而非「一次性步驟」。
...
Claude Code 會監控 Skills 目錄的檔案變更。在 `~/.claude/skills/`、專案 `.claude/skills/`、或 `--add-dir` 目錄中新增、編輯或刪除 Skill，**無需重啟**即可在當前 Session 中生效。唯一例外是：若在 Session 啟動後才建立頂層 `skills/` 目錄，則需重啟 Claude Code 以註冊新的監控路徑。
...
在企業管理環境中，可透過 Managed Settings 設定 `"disableSkillShellExecution": true`，全面禁止 User、Project、Plugin 及 Additional Directory 來源的 Skills 執行 `!`<command>`` Shell 命令。每個命令會被替換為 `[shell command execution disabled by policy]`。Bundled Skills 和 Managed Skills 不受此設定影響。

**`skillOverrides` 設定（進階控制）**：

透過 `settings.json`（`.claude/settings.json` 或 Managed Settings）中的 `skillOverrides` 物件，可在不修改 Skill 原始碼的情況下調整單一 Skill 的行為：
...
控制 Skill 描述（description + when_to_use）在系統提示詞中佔用的比例：
...
當安裝了大量 Skills 時，可降低此值以節省上下文空間，但可能降低 Claude 自動匹配 Skill 的準確度。

**`/doctor` 診斷命令**：

Claude Code 內建的 `/doctor` 命令可診斷 Skills 載入狀態、設定衝突與常見問題：

```bash
/doctor
# 輸出範例：
# ✅ 3 project skills loaded
# ✅ 2 user skills loaded
# ⚠️ Skill "old-format" uses deprecated `commands/` path — consider migrating to `skills/`
# ❌ Skill "broken" has invalid frontmatter — missing required `description` field
```

**Skill 評估與品質驗證（skill-creator Plugin）**：

`skill-creator` plugin 提供 Skill 撰寫輔助與自動化評估功能：

```bash
# 安裝 skill-creator plugin
/plugin marketplace add anthropics/skill-creator

# 使用 plugin 建立新 Skill（引導式流程）
/create-skill

# 評估現有 Skill 的品質（檢查描述清晰度、Token 效率、觸發條件完整性）
/evaluate-skill api-design
```

**視覺化輸出模式（Visual Output Pattern）**：

Skills 不僅限於文字輸出，也可指示 Agent 生成 Mermaid 圖表、HTML 視覺化或其他結構化產出。例如 `codebase-visualizer` Skill 會掃描整個 Codebase 並產出架構圖：

```markdown
---
name: codebase-visualizer
description: Generates interactive Mermaid diagrams of project architecture
disable-model-invocation: true
---
...
Output as fenced Mermaid blocks that render in markdown previews.
```

**Skills as Plugins（Plugin 封裝模式）**：

Skills 可封裝為 Claude Code Plugin 發布，讓使用者透過 Plugin Marketplace 一鍵安裝整套 Skills：

```
📦 my-skills-plugin/
├── .claude-plugin/
│   └── plugin.json               # Plugin 描述與 metadata
├── skills/
│   ├── api-design/
│   │   └── SKILL.md
│   ├── code-review/
...
#### 4.6.6 User-invoked vs Model-invoked 設計模式（mattpocock/skills）
...
#### 4.6.7 自主執行模式（addyosmani/agent-skills `/build auto`）

**概念**：addyosmani 的 `/build` Command 支援 `auto` 模式——Agent 在收到需求後**完全自主**完成 PLAN→BUILD→VERIFY→REVIEW 循環，中途不向使用者提問，直到產出完整的可工作程式碼後才回報結果。

```bash
# 自主執行模式
/build auto implement user authentication with JWT

# Agent 行為：
# 1. PLAN — 自動分析需求，拆分任務
# 2. BUILD — 逐一實作，每步驟跑測試
# 3. VERIFY — 確認所有測試通過、無 lint 錯誤
# 4. REVIEW — 自我審查（Staff Engineer 標準）
# 5. 回報完整結果給使用者
```

> **⚠️ 企業考量**：`auto` 模式適合 well-defined 的功能實作，但在涉及安全決策、架構選擇或合規需求時，應避免使用自主模式，改用 interactive 模式讓人類參與決策。
...
> **💡 Copilot CLI Skills 管理**：在 CLI Session 中可使用 `/skills` 系列命令管理 Skills。也可使用 `gh skill` CLI 子命令（詳見 [2.7 節](#27-gh-skill-cligithub-cli-整合)）從 GitHub Repository 搜尋並安裝 Skills。
...
將 Skills 封裝為 Claude Code Plugin 可透過 Marketplace 分發給團隊（詳見 §2.8 「Skills as Plugins」）。配合以下進階設定可在不修改 Skill 原始碼的情況下控制行為：

- **`skillOverrides`**（詳見 §2.8）：調整單一 Skill 的 `visibility`（`name-only` / `off`）或覆蓋 `model`
- **`skillListingBudgetFraction`**（詳見 §2.8）：微調 Skill 描述在系統提示詞中的 Token 佔比
- **`/doctor`**：快速診斷 Skills 載入狀態、frontmatter 錯誤與設定衝突
...
VS Code Copilot Chat 的 **Agent Mode**（`@workspace` 或 Ctrl+I 開啟）可自動探索專案中的 Skills。Skills 檔案放置在 `.github/skills/` 或 `.agents/skills/` 目錄中，Agent Mode 會根據任務描述自動匹配並載入相關 Skill。與 Claude Code 的區別在於：VS Code Agent Mode 目前不支援 `context: fork` Subagent 執行模式，所有 Skills 均在主對話中執行。
...
- **mattpocock/skills**：[github.com/mattpocock/skills](https://github.com/mattpocock/skills) — Skills for Real Engineers（⭐ 151k），20+ 個工程實務 Skills
- **addyosmani/agent-skills**：[github.com/addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) — 24 個生產級工程 Skills（⭐ 68.1k），融合 Google 工程文化
...
> 來源：[github.com/addyosmani/agent-skills](https://github.com/addyosmani/agent-skills)。支援 Claude Code、Cursor、Gemini CLI、Windsurf、OpenCode、GitHub Copilot、Kiro、Codex 等。最新版本 v0.6.2。
...
> 來源：[github.com/anthropics/skills](https://github.com/anthropics/skills)（⭐ 157k+）。可透過 Claude Code Plugin 一鍵安裝：`/plugin marketplace add anthropics/skills`。
