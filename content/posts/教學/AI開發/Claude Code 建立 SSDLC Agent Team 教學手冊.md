...
> **版本**：1.1.1 ｜ **最後更新**：2026-06-26 ｜ **作者**：企業級 AI Agent 架構顧問團隊  
...
- [Ch 8：建立 Skills](#ch-8建立-skills)
  ...
  - [8.5.6 內建 Skills（v2.1.145+）](#856-內建-skillsv21145)
...
**自動建立團隊（v2.1.178+）**：早期版本需先請 Claude 以 `TeamCreate` 工具明確建立並命名團隊；v2.1.178 起，Lead 第一次產生 Teammate 時會**自動建立團隊**，無需再手動呼叫 `TeamCreate`/`TeamDelete`。團隊設定與任務狀態分別儲存於：

```
~/.claude/teams/{team-name}/config.json   # 團隊成員（session ID、Agent 類型）
...
**Effort Level 繼承（v2.1.186+）**：Teammate 預設繼承 Lead 當前的 effort level 設定，不需individually 指定。
...
| **Remote Session** | 可在面板的 Remote 分頁接續在 claude.ai 啟動的雲端 Session，與本機 Session 並列管理 |
...
| `claudeCode.initialPermissionMode` | `default` | 開啟新對話時的初始權限模式 |
...
- 例外：以 **Agent Team Teammate** 身分執行時，`skills` 與 `mcpServers` frontmatter 一律不生效（見 6.11），Teammate 僅能使用 Lead 啟動時就已連線的 MCP Server。
- 設計原則：明確宣告而非隱性繼承，可避免 Subagent 意外取得超出其任務範圍的外部系統存取權。

**背景 Subagent 完成後的結果整合**：

- 以背景模式（`fork: true` 或 `--agent` 搭配背景旗標）啟動的 Subagent 完成後，其最終輸出會以一則系統訊息插入主對話歷史，主 Claude 在下一輪回應前會讀到該結果。
...
> ⚠️ **版本需求**：`if` 條件過濾欄位需 Claude Code **v2.1.85 以上**才支援，較舊版本只能依賴 `matcher` 做工具名稱層級的過濾。
...
Anthropic 官方 Marketplace（`/plugin marketplace add anthropics/claude-code` 即可瀏覽）目前涵蓋以下類別：

| 類別 | 內容範例 |
|------|---------|
| **程式碼智能（LSP）** | `clangd-lsp`、`csharp-lsp`、`gopls-lsp`、`jdtls-lsp`、`kotlin-lsp`、`lua-lsp`、`php-lsp`、`pyright-lsp`、`rust-analyzer-lsp`、`swift-lsp` 等，提供自動診斷與程式碼導覽，需本機已安裝對應語言的 LSP 執行檔 |
...
```bash
# 加入社群 Marketplace（plugin 經審核後 pin 至特定 commit SHA，由 CI 自動跟進更新）
/plugin marketplace add anthropics/claude-plugins-community

# 安裝社群 Marketplace 中的 Plugin
/plugin install code-review@claude-community

# 加入官方示範 Marketplace（用於學習 Plugin 開發範例）
...
- **提交至社群/企業 Marketplace**：通過驗證後，可至 `platform.claude.com/plugins/submit` 提出審核；通過審核的 Plugin 會被 pin 至特定 commit SHA 收錄進 `anthropics/claude-plugins-community`，後續上游更新由 CI 自動偵測並提升 pin 版本，使用者端可選擇是否同步。
...
> 📌 **Prompt Caching 提醒**：切換 Output Style 會改變系統提示內容，使既有的 prompt cache 失效。在同一場長對話中頻繁切換風格會增加 token 成本，建議在對話開始前就決定好風格。
...
也可以使用內建 `/schedule` Skill 明確指定 cron 表達式或固定間隔：

```bash
# 使用 cron 語法（底層呼叫 CronCreate）
...
```bash
# 列出所有排程任務（底層為 CronList 工具）
/schedule list

# 刪除特定排程任務，task-id 為 8 字元識別碼（底層為 CronDelete 工具）
/schedule delete <task-id>

# 建立任務（底層為 CronCreate 工具）
/schedule create --cron "*/30 * * * *" "安全掃描"
```

`CronCreate`、`CronList`、`CronDelete` 是 `/schedule` Skill 底層呼叫的實際工具：`CronCreate` 接受 5 欄 cron 表達式、prompt 內容、是否週期重複；`CronList` 列出目前 session 所有任務的 ID、排程與 prompt；`CronDelete` 依 8 字元 ID 取消指定任務。單一 session 上限 50 個任務（含週期性與一次性）。
...
> 📌 **Scope 命名澄清**：官方文件中的「Local scope」指的是寫入 `~/.claude.json`中、僅綁定單一專案路徑的個人項目，與本表為教學目的而劃分的「Local（`.claude/mcp.json`，已加入 `.gitignore`）」是同一概念的不同呈現方式——兩者皆為「不提交 Git、僅個人專案有效」的最低優先層級，企業導入時可任選其一作為團隊慣例，重點是保持全團隊一致。
...
claude -p "<prompt>"              # 非互動模式（Programmatic CLI）
  ...
  --plugin-dir <path>             # 從本機路徑載入 Plugin
...
| `AI_FLOW_GITLAB_TOKEN` | ✅ | GitLab Personal Access Token，供 Claude 操作 GitLab API（建立 MR、留言等） |
| `AI_FLOW_PROJECT_ID` | ✅ | GitLab Project ID，標示目前操作的專案 |
| `AI_FLOW_MR_IID` | ✅ | Merge Request IID，標示目前處理的 MR（由 Web/API 觸發時帶入的事件上下文） |
| `AI_FLOW_PIPELINE_ID` | ✅ | Pipeline ID，供稽核與追蹤對應到哪一次 Pipeline 執行 |
| `AI_FLOW_INPUT` | ❌ | 透過 Web/API 觸發時帶入的 prompt 內容 |
| `AI_FLOW_CONTEXT` | ❌ | 觸發事件關聯的 Issue/MR/討論串參照 |
| `AI_FLOW_EVENT` | ❌ | 觸發事件類型（如 `issue`、`merge_request`、`review`、`comment`） |
...
claude-mr-review:
  ...
  before_script:
    ...
    # 啟動 GitLab MCP Server（背景執行；提供開 MR、留言、查詢 Issue 等 GitLab 專用工具；若映像檔未內建則略過，不中斷 Pipeline）
...
| Job 無法留言或建立 MR | 確認 `AI_FLOW_GITLAB_TOKEN` 對應的 Personal Access Token / `CI_JOB_TOKEN` 是否具備 `api` 與 `write_repository` 權限 |
