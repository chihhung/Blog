...
> **官方指南**：  
> — [Shorthand Guide](https://x.com/affaan/status/2012378465664745795)（入門首選）  
> — [Longform Guide](https://x.com/affaan/status/2014040193557471352)（進階深入）  
> — [Security Guide](https://x.com/affaan/status/2033263813387223421)（安全防護）

---

## 📑 目錄

- [第一章：Everything Claude Code 架構總覽](#第一章ECC-架構總覽)
...
- [第三章：安裝與環境建置](#第三章安裝與環境建置)
  ...
  - [3.5 跨 Harness 整合（Cursor / Codex / Copilot / Zed / OpenCode）](#35-跨-harness-整合cursor--codex--copilot--zed--opencode)
...
- [第四章：企業級 Web 系統架構設計（搭配 ECC）](#第四章企業級-web-系統架構設計搭配-ecc)
  ...
  - [4.3 Orchestrator 家族（v2.0.0）](#43-orchestrator-家族v200)
  - [4.4 系統架構圖](#44-系統架構圖)
...
Everything Claude Code（ECC）是一個開源的 **Agent Harness Operating System**（代理控制操作系統），由 Anthropic 黑客松冠軍 Affaan Mustafa 建立。
...
- ✅ 提供持續學習與記憶持久化機制
- ✅ 跨平台支援：Claude Code、Codex、Cursor、OpenCode、Gemini、Zed、GitHub Copilot、CodeBuddy、Kiro、Trae、Qwen
- ✅ 224K+ Stars、34.3K+ Forks、289+ Contributors
- ✅ 在 Anthropic × Forum Ventures Hackathon 中獲得冠軍
- ✅ v2.0.0 正式畢業為 Agent Harness Operating System
...
graph TB
    subgraph "ECC Core System"
        ...
        Agents["🤖 Agents (67)<br/>Specialized Subagents"]
        Skills["⚡ Skills (277)<br/>Workflow Definitions"]
        Commands["📋 Commands (92)<br/>Legacy Slash Shims"]
...
# 注意：ECC_DISABLED_MCPS 是 install/sync filter，非 live Claude Code toggle
export ECC_DISABLED_MCPS="supabase,railway,vercel"

# 使用 /mcp 命令在 Claude Code 中即時停用
# Claude Code 會將選擇持久化到 ~/.claude.json
```

> ⚠️ **MCP Connector Policy（v2.0.0）**：ECC 現在僅預設啟用 **1 個連接器**（`chrome-devtools`），其餘全部為 opt-in catalog entry。詳見 [MCP-CONNECTOR-POLICY.md](https://github.com/affaan-m/ECC/blob/main/docs/MCP-CONNECTOR-POLICY.md)。每個 MCP 的工具描述都會消耗 Token，啟用過多 MCP 會將 200K Token 窗口壓縮到約 70K。建議每專案啟用 **< 10 MCPs**、**< 80 tools**。
...
> ⚠️ **只選一種安裝路徑**：大多數使用者應使用 **Plugin 安裝**，然後只手動複製需要的 Rules 資料夾。**不要** 疊加多種安裝方式（例如先 `/plugin install` 再跑 `install.sh --profile full`），這會造成重複元件和衝突行為。
...
不確定該用哪些 Skills / Agents？使用 `npx ecc consult` 取得建議：

```bash
# 詢問特定主題的建議安裝內容
npx ecc consult "Spring Boot microservice with Kafka" --target claude

# 快速比較不同 Harness 的能力
npx ecc consult "compare cursor vs codex for TDD workflow"
```
...
> ⚠️ 解除安裝只移除 ECC 管理的檔案（rules, hooks, commands, agents, skills）。你的 `~/.claude/settings.json` 中的個人設定不會被影響。
...
cp -r ECC/rules/typescript ~/.claude/rules/  # 依你的技術棧選擇
...
### 3.5 跨 Harness 整合（Cursor / Codex / Copilot / Zed / OpenCode）
...
### Q7：是否支援 Cursor / OpenCode / Codex / GitHub Copilot / Zed？
...
ECC v2.0.0 正式支援 11 種 Harness 的統一架構。所有 Harness 共用同一套 Plugin manifest（`plugin.json`）和 Rules 來源，但透過 **Adapter Layer** 轉譯為各 Harness 的原生格式：

```mermaid
graph TB
    subgraph "ECC Core（Harness-Agnostic）"
        Plugin["plugin.json<br/>67 agents · 277 skills · 34 rules"]
        Scripts["scripts/<br/>Hook scripts · Install scripts"]
        MCP["mcp-configs/<br/>14 MCP server 定義"]
    end
...
- **贊助**：[GitHub Sponsors](https://github.com/sponsors/affaan-m) ∣ [Sponsor Tiers](https://github.com/affaan-m/ECC/blob/main/SPONSORS.md)
...
> **文件維護**：本手冊基於 ECC v2.0.0（2026 年 6 月）撰寫。ECC 更新頻繁，建議定期查閱 [官方 CHANGELOG](https://github.com/affaan-m/ECC/blob/main/CHANGELOG.md) 和 [Releases](https://github.com/affaan-m/ECC/releases)。
