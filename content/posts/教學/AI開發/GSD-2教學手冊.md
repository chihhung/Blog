...
- [3. 系統架構設計（Architecture）](#3-系統架構設計architecture)
  - [3.1 GSD-2 內部架構](#31-gsd-2-內部架構)
  - [3.2 GSD-2 + 微服務架構整合](#32-gsd-2--微服務架構整合)
...
- [4. 安裝與環境建置（Installation）](#4-安裝與環境建置installation)
  - [4.1 系統需求](#41-系統需求)
  - [4.2 安裝步驟](#42-安裝步驟)
  - [4.3 首次啟動與設定](#43-首次啟動與設定)
  - [4.4 MCP Server 啟動](#44-mcp-server-啟動)
  - [4.5 VS Code 擴充設定](#45-vs-code-擴充設定)
  - [4.6 與 Claude Code / Copilot 整合](#46-與-claude-code--copilot-整合)
  - [4.7 Docker Sandbox 部署](#47-docker-sandbox-部署)
  - [4.8 Web 介面啟動](#48-web-介面啟動)
  - [4.9 常見安裝問題與排除](#49-常見安裝問題與排除)
...
- [9. 技能管理（Skill Tracking）](#9-技能管理skill-tracking)
  - [9.1 技能系統概述](#91-技能系統概述)
  - [9.2 技能發現模式](#92-技能發現模式)
  - [9.3 技能評估方式](#93-技能評估方式)
  - [9.4 Agent 健康度監控](#94-agent-健康度監控)
  - [9.5 v2.77 技能增強](#95-v277-技能增強)
- [10. 實戰範例](#10-實戰範例)
...
- [12. 系統維護（Maintenance）](#12-系統維護maintenance)
  ...
  - [12.6 Failure Recovery（v2.28）](#126-failure-recoveryv228)
  - [12.7 Pipeline Architecture（v2.40）](#127-pipeline-architecturev240)
...
- [17. 檢查清單（Checklist）](#17-檢查清單checklist)
  ...
  - [17.6 GSD-2 命令速查表](#176-gsd-2-命令速查表)
- [附錄 A：GSD-2 內建擴充套件一覽](#附錄-agsd-2-內建擴充套件一覽)
- [附錄 B：相關資源](#附錄-b相關資源)
...
GSD-2（Get Shit Done 2）是一套 **Meta-Prompting、Context Engineering 與 Spec-Driven Development** 系統，專為 AI Agent 長時間自主開發而設計。它不是提示框架（Prompt Framework），而是一個獨立的 **TypeScript CLI 應用程式**，建構在 [Pi SDK](https://github.com/badlogic/pi-mono) 之上，能夠真正控制 Agent 的 Context Window、Session 生命週期與 Git 策略。
...
| **Skill Tracking** | 追蹤 AI Agent 在不同任務的技能表現與健康度 |
| **Web Interface** | 瀏覽器介面，視覺化專案管理與即時進度追蹤 |
| **Telegram Remote** | 透過 Telegram 遠端控制 Auto Mode（暫停、恢復、查詢狀態） |
| **Reactive Execution** | 基於依賴圖譜的平行任務執行，同一 Slice 內無衝突任務同時運行 |
| **Hooks System** | Pre-dispatch / Post-unit Hooks，支援自訂自動化流程 |
| **GitHub Sync** | 自動同步 Milestone 到 GitHub Issues、PR 與 Milestones |

### 1.2 與傳統開發模式差異

| 面向 | 傳統開發 | GSD-1（Prompt Framework） | GSD-2（v2.77） |
|------|----------|--------------------------|----------------|
...
> **💡 實務建議**：GSD-2 最適合「一個 Milestone 可拆為 4-10 個 Slice、每個 Slice 1-7 個 Task」的中大型任務。小型單次修改建議使用 `/gsd quick`。

### 1.4 版本演進與生態系

GSD-2 自開源以來已發佈 **113+ 版本**，社群活躍（6,300+ Stars、647 Forks、86 Contributors）。

**版本演進里程碑**：

| 版本 | 重點特性 |
|------|----------|
| v2.26 | Verification Enforcement、Crash Recovery、HTML Reports、Incremental Memory |
| v2.28 | Failure Recovery（原子寫入、OAuth 超時、Blob GC）、Headless Auto-restart |
| v2.38 | Reactive Task Execution（平行任務圖譜）|
| v2.39 | Stuck Detection（滑動窗口）、RUNTIME.md、GitHub Sync、Skill Activation |
| v2.40 | Pipeline Architecture、Post-Mortem Forensics、Real-Time Health Visibility |
| v2.42 | Service Tier（OpenAI Priority / Flex）、`/gsd fast` 切換 |
| v2.43 | Forensics Dedup（自動搜尋重複 Issue）|
| v2.44 | Show Token Cost（逐 Prompt 成本顯示）|
| v2.59 | Context Management（Observation Masking、Compaction）、Capability Routing |
| v2.77 | Context Mode、Memory Architecture ADR-013、9 個新技能、Hook Stack 升級 |

**生態系**：

| 元件 | 說明 |
|------|------|
| `gsd-pi` (npm) | 核心 CLI 套件 |
| VS Code Extension | FluxLabs 發佈，Chat Participant + Sidebar Dashboard |
| Docker Sandbox | 隔離容器執行環境 |
| Web Interface | 瀏覽器專案管理介面 |
| GSD2 Config Utility | 獨立設定管理工具 |
| GSD Orchestrator | 多 Worker 編排引擎 |
| Mintlify Docs | 線上文件站台 |
...
Meta Prompting 指的是「產生 Prompt 的 Prompt」。GSD-2 不是讓開發者手寫每一步的指令，而是透過 **Spec → Roadmap → Slice Plan → Task Plan** 的層級結構，自動為每個任務生成精準的 Dispatch Prompt。
...
Context Engineering 是 GSD-2 的核心差異化能力。每次 Dispatch 時，系統會自動建構最適合的 Context：
...
**v2.77 新增 — Context Mode**：自動拉取最相關的專案 Artifacts、先前 Session 狀態、Milestone/Slice 信號與執行 Metadata，減少手動組裝 Prompt 的需要。新專案預設啟用。
...
GSD-2 使用 `KNOWLEDGE.md` 作為跨 Session 的知識庫：

- **自動學習**：每個 Task 完成後，提取的教訓自動寫入
- **結構化記憶**（v2.77）：`structured_fields` 支援型別化 Metadata
...
Reactive Task Execution（v2.38）是 GSD-2 的平行任務執行機制。當啟用時，GSD 會從任務計畫中的 IO 註解衍生出依賴圖譜，並自動將無衝突的任務平行分發：
...
Context Pressure Monitor（v2.26）監控 Context Window 使用率，防止 Session 在任務中途因 Context 耗盡而無產出：
...
GSD-2 是一個 TypeScript 應用程式，嵌入 Pi Coding Agent SDK：
...
GSD-2 透過 Spec-Driven 方式與 Spring Boot Clean Architecture 整合：
...
GSD-2 支援 Model Context Protocol（MCP），可連接外部 MCP Server 擴展功能：

```mermaid
graph TB
    subgraph "GSD-2 MCP 架構"
        GSD[GSD-2 CLI] --> MCP_CLIENT[MCP Client<br/>內建擴充]
...
GSD-2 可作為 MCP Server 運行，供 Claude Desktop 或 VS Code Copilot 等外部 AI 用戶端使用：
...
GSD-2 提供官方 VS Code 擴充套件（由 FluxLabs 發佈）：
...
### 4.7 Docker Sandbox 部署

GSD-2 提供官方 Docker Sandbox 範本，可在隔離容器中執行 Auto Mode，無需在主機安裝 Node.js：
...
GSD-2 提供瀏覽器介面，適合視覺化專案管理和即時進度追蹤：
...
| 問題 | 解決方案 |
|------|----------|
| `npm ERR! EACCES` | 使用 `sudo npm install -g gsd-pi` 或設定 npm prefix |
...
GSD-2 使用 `.gsd/` 目錄管理所有工作狀態。以下為一個完整的企業專案目錄結構範例：
...
GSD v2.77 支援透過 Telegram 遠端控制 Auto Mode，適合在 Headless / CI 模式下遠端監控和操作：
...
`KNOWLEDGE.md` 是 GSD-2 的長期記憶系統，跨 Session 保持學習成果：
...
GSD-2 的技能系統（Skills）提供領域專業指引，讓 AI Agent 在特定任務中獲得更精準的知識：
...
- **9 個補充技能**：填補端到端工作流程缺口
- **6 個規劃/設計技能**：改善特定領域的規劃品質
- **Chat-frame 渲染**：技能調用在 TUI 中以專用框架樣式呈現
- **Hook Stack 升級**：Layer 0 Shell Hooks 和 Layer 2 Event 擴大擴充整合點

> **💡 實務建議**：建議在專案初始化時，透過 `always_use_skills` 鎖定核心技能（如 `java-spring-boot`、`clean-architecture`），確保 AI 始終遵循團隊的技術棧規範。
...
使用 GSD-2 完整流程建立一個「Spring Boot REST API + Vue UI」的客戶管理系統。
...
GSD-2 自動分類 Provider 錯誤並據此決定恢復策略：
...
### 12.6 Failure Recovery（v2.28）

v2.28 強化了 Auto Mode 的可靠性：
...
### 12.7 Pipeline Architecture（v2.40）
...
| **Schema 遷移** | v2.77 強化了 Schema 版本戳記，升級自動套用遷移 |
| **向下相容** | `.gsd/` 檔案格式保持向下相容，舊專案可直接使用新版本 |
| **雙寫遷移** | v2.77 完成了記憶系統的雙寫遷移，舊資料自動轉換 |

### 13.2 版本升級路徑

```mermaid
graph LR
    V1[GSD v1<br/>.planning/] -->|/gsd migrate| V2[GSD v2.x<br/>.gsd/]
    V2 -->|npm update -g| V277[GSD v2.77<br/>Context Mode]
```
...
GSD-2 的 Prompt 由系統自動管理，開發者不需要手動維護 Prompt 版本。但自定義的 `AGENTS.md` 需要注意：
...
> **💡 實務建議**：開始使用 GSD-2 時，建議先用 `token_profile: quality` 並設定較低的 `budget_ceiling`（如 $10），觀察 AI 行為。熟悉後再調整為 `balanced` 並提高預算。
...
Reactive Execution（v2.38）在 Slice 內建立任務依賴圖，實現安全的平行執行：
...
- [ ] `npm install -g gsd-pi@latest` 已執行
- [ ] `gsd --version` 顯示正確版本
- [ ] Git 已安裝並初始化
...
| 資源 | 連結 |
|------|------|
| GitHub Repository | https://github.com/gsd-build/gsd-2 |
| npm Package | https://www.npmjs.com/package/gsd-pi |
| Getting Started Guide | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/getting-started.md |
| Auto Mode Guide | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/auto-mode.md |
| Configuration Guide | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/configuration.md |
| Token Optimization | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/token-optimization.md |
| Cost Management | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/cost-management.md |
| Git Strategy | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/git-strategy.md |
| Skills Guide | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/skills.md |
| Docker Sandbox | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/docker.md |
| Web Interface | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/web-interface.md |
| Remote Questions | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/remote-questions.md |
| Dynamic Model Routing | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/dynamic-routing.md |
| Parallel Orchestration | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/parallel.md |
| Visualizer | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/visualizer.md |
| Captures & Triage | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/captures.md |
| Troubleshooting | https://github.com/gsd-build/gsd-2/blob/main/docs/user-docs/troubleshooting.md |
| Architecture | https://github.com/gsd-build/gsd-2/blob/main/docs/dev/architecture.md |
...
GSD-2 既是 MCP Client（連線到外部工具），也可作為 MCP Server（暴露自身功能供其他工具呼叫）。
...
GSD-2 的 Universal Config 擴充可自動匯入 Claude Code、Cursor 等工具的 MCP 設定：
...
> **文件維護說明**：本教學手冊基於 GSD-2 v2.77 撰寫。GSD-2 更新頻繁（已發佈 113 個版本），建議定期對照官方 Changelog 更新內容。執行 `gsd update` 可隨時升級到最新版本。
