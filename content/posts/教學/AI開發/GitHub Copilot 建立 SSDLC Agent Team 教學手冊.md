...
- [22. GitHub Copilot 外掛程式（Copilot Plugins / Extensions）](#22-github-copilot-外掛程式copilot-plugins--extensions)
  - [22.1 概述](#221-概述)
  - [22.2 外掛程式核心概念](#222-外掛程式核心概念)
  - [22.3 plugin.json 結構詳解](#223-pluginjson-結構詳解)
  - [22.4 Skills 檔案設計](#224-skills-檔案設計)
  - [22.5 整合 MCP Server](#225-整合-mcp-server)
  - [22.6 整合自訂工具與 API](#226-整合自訂工具與-api)
  - [22.7 SSDLC Agent Team 實作範例](#227-ssdlc-agent-team-實作範例)
  - [22.8 安全考量](#228-安全考量)
  - [22.9 最佳實務](#229-最佳實務)
...
| 前提 | 說明 |
|------|------|
| GitHub 帳號 | 需具備 Copilot Pro / Pro+ / Business / Enterprise 授權（Copilot Free 有限額度亦可使用部分功能） |
| VS Code | 建議 v1.120 或以上（以取得最新 Agent 功能與 Task Optimization） |
...
| **Premium Requests** | 使用進階模型時消耗的計費單位，不同模型有不同倍率。⚠️ 自 2026 年 6 月 1 日起，已全面轉為 **usage-based 計費（AI Credits / per-token）**，每個 credit = $0.01 USD |
| **Auto Model Selection** | Copilot 自動選擇最佳可用模型的功能，VS Code 新增 **Task Optimization**（依任務複雜度智慧路由） |
| **Gate** | SSDLC 流程中需要人工審核與批准的檢查點 |
| **CLI Plugin** | Copilot CLI 的可安裝擴充套件，透過 `plugin.json` 清單檔封裝 Agents、Skills、Hooks、MCP/LSP Server，支援 Marketplace 分發與版本控管 |
...
> ⚠️ **本章內容以 2026 年 5 月官方文件為準**。功能狀態可能隨版本更新而改變，請定期查閱 [GitHub Copilot 官方文件](https://docs.github.com/en/copilot) 確認。
>
> ⚠️ **重要計費變更**：自 2026 年 6 月 1 日起，GitHub Copilot 從 request-based 計費全面轉為 **usage-based（AI Credits / per-token）計費**。本章已反映最新計費模式。
...
| **Auto Model Selection** | Copilot 自動選擇最佳可用模型（含 10% 倍率折扣），涵蓋 Copilot Chat / Cloud Agent / CLI / OpenAI Codex / Anthropic Claude。VS Code 支援 **Task Optimization**（結合即時系統健康與任務複雜度智慧路由） | 降低延遲、減少限速、成本優化 |
...
| **Steering** | Agent session 進行中提供修正指引（每則消耗 AI Credits） | 調整 Agent 行為 |
...
| **CLI Plugins** | Copilot CLI 的可安裝擴充套件，將 Agents、Skills、Hooks、MCP/LSP Server 封裝為單一可分發套件，透過 Marketplace 探索與安裝 | 跨專案重用、團隊標準化、封裝複雜設定 |
...
| CLI Plugins | GA | Copilot Pro / Pro+ / Business / Enterprise | — |
...
| **CLI Plugin** | **`.github/` 手動設定** | Plugin 是可安裝的封裝套件（僅限 CLI），提供跨專案重用、Marketplace 版本管理；`.github/` 是 Repository 級設定（適用 VS Code / CLI / Cloud Agent），範圍限單一 Repo |
| **CLI Plugin** | **Agent Skills** | Plugin 是分發機制（可包含多個 Skills + Agents + Hooks），Skills 是單一可攜能力 |
...
> ⚠️ **計費更新（2026/06/01 起）**：GitHub Copilot 已從 Premium Request Multiplier 轉為 **usage-based per-token 計費（AI Credits）**。以下表格中「成本等級」為相對參考，實際費用依各模型 per-token 定價計算（每 credit = $0.01 USD）。

| 任務類型 | 推薦模型 | 備選模型 | 選用理由 | 成本等級 |
|---------|---------|---------|---------|---------|
| **需求分析 / 架構設計** | Claude Opus 4.7 | Claude Opus 4.6 / GPT-5.5 | 需要深度推理與長上下文理解 | 🔴 高 |
| **程式碼實作** | Claude Sonnet 4.6 | Claude Sonnet 4 / GPT-5.3-Codex | 平衡品質與速度 | 🟡 中 |
| **安全審查** | Claude Opus 4.7 | Claude Opus 4.6 / GPT-5.5 | 需要嚴謹推理，不容許遺漏 | 🔴 高 |
| **測試產生** | Claude Sonnet 4.6 | Claude Sonnet 4 / Gemini 3.1 Pro | 規律性任務，速度重要 | 🟡 中 |
| **文件產生** | Claude Haiku 4.5 | GPT-5.4 mini / Gemini 3.5 Flash | 低複雜度，成本敏感 | 🟢 低 |
| **逆向工程** | Claude Opus 4.7 | Claude Opus 4.6 / GPT-5.5 | 需要最強推理能力 | 🔴 高 |
| **Code Review** | Claude Sonnet 4.6 | Claude Sonnet 4 / GPT-5.4 | 品質與速度均衡 | 🟡 中 |
| **PR / Release** | GPT-5.4 mini | Gemini 3.5 Flash / Gemini 3 Flash | 格式化任務，低成本 | 🟢 低 |
| **專案管理** | Claude Sonnet 4.6 | GPT-5.4 | 需進度分析與風險評估 | 🟡 中 |

### Auto Model Selection 使用原則

> 💡 **VS Code Task Optimization**：VS Code 中的 Auto Model Selection 新增「Task Optimization」功能（GA），會結合即時系統健康狀態與任務複雜度進行智慧路由。其他 IDE（JetBrains / Eclipse / Xcode）的 Auto 僅提供「Reliability / Availability」層級的路由。
...
│  ✓ Gemini 3.1 Pro                       │
│  ✓ Gemini 3 Flash                       │
│  ✓ Raptor mini（fine-tuned，Public Preview）│
│  ✓ Goldeneye（fine-tuned，Public Preview） │
│                                          │
│ Third-party Agents：                     │
...
4. **定期審閱 AI Credits 用量**：建立月度用量報告，識別異常消耗
...
| 項目 | 建議 |
|------|------|
| **VS Code 版本** | v1.120 或以上（建議持續使用最新穩定版） |
| **更新策略** | 啟用自動更新或每月手動更新 |
...
CLI Plugin 是 **Copilot CLI 的可安裝套件**，可將 Custom Agents、Skills、Hooks、MCP Server 設定等元件打包成單一可分發單位。相較於手動複製 `.github/` 目錄結構，Plugin 提供：
...
> ⚠️ **適用範圍**：CLI Plugins 目前僅適用於 **Copilot CLI** 環境。VS Code、JetBrains 等 IDE 的 Agent/Skills 仍使用 `.github/` 目錄結構。
>
...
| **MCP Server 設定** | `.mcp.json` 或 `.github/mcp.json` | Model Context Protocol 整合 |
...
| **適用環境** | VS Code + CLI + Cloud Agent | 僅 CLI |
...
> 💡 **雙軌策略**：建議同時維護 `.github/` 目錄結構（給 VS Code / Cloud Agent 使用）與 Plugin 封裝（給 CLI 使用）。兩者的 Agent 和 Skill 檔案內容可共用，僅需額外維護 `plugin.json`。
...
### 16.4.1 GitHub Copilot 定價模式（2026 年 5 月）
...
| 模型 | Input 價格 | Cached Input 價格 | Output 價格 | 成本等級 |
|------|-----------|------------------|------------|---------|
| **Claude Opus 4.7** | $15.00 | $1.50 | $75.00 | 🔴 極高 |
| **Claude Opus 4.6** | $15.00 | $1.50 | $75.00 | 🔴 極高 |
| **GPT-5.5** | $5.00 | $0.50 | $30.00 | 🔴 高 |
| **GPT-5.3-Codex** | $5.00 | — | $30.00 | 🔴 高 |
| **Claude Sonnet 4.6** | $3.00 | $0.30 | $15.00 | 🟡 中 |
| **Claude Sonnet 4.5** | $3.00 | $0.30 | $15.00 | 🟡 中 |
| **Claude Sonnet 4** | $3.00 | $0.30 | $3.75 / $15.00 | 🟡 中 |
| **GPT-5.4** | $2.50 | $1.25 | $10.00 | 🟡 中 |
| **Gemini 3.1 Pro** | $1.25 | $0.31 | $10.00 | 🟡 中 |
| **Gemini 3.5 Flash** | $1.50 | $0.15 | $9.00 | 🟡 中低 |
| **Gemini 3 Flash** | $0.15 | $0.04 | $0.60 | 🟢 低 |
| **Claude Haiku 4.5** | $0.80 | $0.08 | $4.00 | 🟢 低 |
| **GPT-5.4 mini** | $0.15 | $0.08 | $0.60 | 🟢 極低 |
| **GPT-5.4 nano** | $0.10 | $0.05 | $0.40 | 🟢 極低 |
| **Raptor mini** | $0.15 | — | $0.60 | 🟢 極低 |
| **Goldeneye** | $0.15 | — | $0.60 | 🟢 極低 |
...
```
Agent 模型分配策略（per-token 計費時代）：

低成本任務（使用 GPT-5.4 mini / Gemini 3.5 Flash / Raptor mini）：
- Doc Generator Agent：文件產生不需要最強模型
- JUnit Agent：測試產生用中階模型即可
- PR Checker Agent：檢查項目固定，不需要高推理

標準任務（使用 Claude Sonnet 4.6 / Claude Sonnet 4 / GPT-5.4）：
- Coding Agent：需要良好的程式碼品質
- API Reviewer Agent：需要理解 API 設計

高複雜任務（使用 Claude Opus 4.7 / GPT-5.5）：
...
| **Skills** | 開放標準 | 新 Skill 類型、`gh skill` CLI | Skills |
| **Hooks** | VS Code: Preview、Cloud Agent/CLI: GA | 等待 VS Code GA | Hooks |
| **Memory** | Public Preview | 治理功能改善 | Memory 政策 |
| **MCP** | 擴充中 | 新的 MCP Server | Agent 工具 |
| **Auto Model** | GA | Task Optimization、新模型加入 | 模型設定 |
| **Third-party Agents** | GA | 企業政策控管 | Admin Policies |
| **Agent Management Tab** | GA | 集中管理功能 | 工作流程 |
| **CLI Plugins** | GA | 新 Marketplace、企業 Plugin Standards | Plugin 管理 |
...
| Premium 額度耗盡 | 使用過多高成本模型 | 啟用 Auto Model Selection，檢視 per-token 消耗 |
...
**A**：Hooks 在 VS Code 中仍是 Preview 功能（截至 2026 年 5 月）。需要在 VS Code Settings 中搜尋 `chat.useCustomAgentHooks` 並啟用。如果找不到，請確認 VS Code 和 GitHub Copilot Extension 都已更新至最新版。注意：Hooks 在 Cloud Agent 與 CLI 環境已是 GA。
...
2. **在 Agent Profile 中指定模型**：為不需要高階推理的 Agent 指定低成本模型（如 GPT-5.4 mini、Gemini 3.5 Flash）
...
# 22. GitHub Copilot 外掛程式（Copilot Plugins / Extensions）
...
### 22.5.2 MCP Server 實作（Node.js 範例）
...
本手冊完整介紹了如何使用 GitHub Copilot（2026 年 5 月最新功能）建立企業級 SSDLC Agent Team。從概念理解、環境建置、Agent 設定、到團隊導入與持續改善，涵蓋了完整的生命週期。
...
> 📅 **文件版本**：v1.2.0 | **最後更新**：2026-05-28 | **適用平台版本**：GitHub Copilot 2026 年 5 月
>
> ⚠️ **重要計費變更**：自 2026 年 6 月 1 日起，GitHub Copilot 已從 request-based 計費轉為 usage-based per-token 計費（AI Credits），每 credit = $0.01 USD。詳見 Ch 16.4。
>
