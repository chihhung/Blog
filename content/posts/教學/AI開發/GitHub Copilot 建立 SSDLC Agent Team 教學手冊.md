...
- [10. 設定 Hooks](#10-設定-hooks)
  - [10.1 Hooks 概念](#101-hooks-概念)
  - [10.2 VS Code Hooks 設定（Preview）](#102-vs-code-hooks-設定preview)
  - [10.3 Cloud Agent / CLI Hooks](#103-cloud-agent--cli-hooks)
  - [10.4 Autopilot 風險與護欄設計](#104-autopilot-風險與護欄設計)
- [11. 管理 Copilot Memory](#11-管理-copilot-memory)
  - [11.1 Memory 概念](#111-memory-概念)
  - [11.2 啟用 Memory](#112-啟用-memory)
  - [11.3 Memory 治理](#113-memory-治理)
  - [11.4 Memory 最佳實務](#114-memory-最佳實務)
...
graph TB
    ...
    INST --> REQ & ARCH & BE & FE & TEST & SEC & CR & REL & RE & DOC
    SKILL --> REQ & ARCH & BE & FE & TEST & SEC & CR & REL & RE & DOC
    MEM --> REQ & ARCH & BE & FE & TEST & SEC & CR & REL & RE & DOC

    VSCODE --> REQ & ARCH & BE & FE & TEST & SEC & CR & REL & RE & DOC
    CLOUD --> REQ & ARCH & BE & FE & TEST & SEC & CR & REL & RE & DOC
    CLI --> REQ & ARCH & BE & FE & TEST & SEC & CR & REL & RE & DOC
```
...
1. **Sprint 需求轉程式碼**：Requirements Agent 分析 User Story → Architect Agent 設計 API → Backend Agent 實作 → Test Agent 補測試 → Security Agent 檢查 → Code Review Agent 審查 → Release Agent 產生 PR
2. **舊系統現代化**：Reverse Engineering Agent 分析程式碼 → 產出架構文件與 Business Rules → Architect Agent 設計新架構 → 漸進式遷移
...
> 本章為全文件重點章節。將針對第 3 章定義的 10 個 Agent 角色，逐一提供 VS Code 與 Cloud Agent 兩種格式的完整定義範例。
...
Hooks 是在 Agent 操作前後自動執行的腳本或指令，可用於實作品質門檻、安全檢查與自動化護欄。

### 環境支援狀態

| 環境 | 支援狀態 | 設定方式 |
|------|---------|---------|
| **VS Code** | ⚠️ **Preview**（需 `chat.useCustomAgentHooks` 設定） | Agent frontmatter + `.github/hooks/` |
| **GitHub.com Cloud Agent** | ✓ **GA** | Repository 設定 |
| **GitHub CLI** | ✓ **GA** | Repository 設定 |

### Hook 觸發時機

| Hook 事件 | 觸發時機 | 使用場景 |
|-----------|---------|---------|
| `postCreate` | Agent 建立新檔案後 | 自動格式化、Lint 檢查 |
| `postEdit` | Agent 編輯現有檔案後 | 編譯檢查、靜態分析 |

## 10.2 VS Code Hooks 設定（Preview）
...
在 Agent Profile 的 frontmatter 中定義：
...
## 10.3 Cloud Agent / CLI Hooks

GitHub.com Cloud Agent 和 CLI 的 Hooks 使用 GitHub Actions workflow 觸發，機制與 VS Code 不同。

### 設定方式

Cloud Agent 的自動化護欄主要透過：
1. **GitHub Actions Workflow**：在 PR 建立或更新時觸發
2. **Branch Protection Rules**：要求特定 Check 通過
3. **CODEOWNERS**：要求特定人員審核
...
| **Autopilot（全自動）** | Agent 自行決定並執行所有動作 | 高 | ⚠️ **不建議使用** |
...
```mermaid
graph TD
    A[Agent 執行動作] --> B{Hook 檢查}
    B -->|通過| C[允許執行]
    B -->|失敗| D[阻止執行]
    D --> E[通知開發者]
    C --> F{Branch Protection}
    F -->|通過| G[合併]
    F -->|失敗| H[要求修正]
...
| 護欄層級 | 實作方式 | 作用 |
|---------|---------|------|
| **L1 — Agent 內建** | Agent Profile 的限制條款 | 限制 Agent 行為範圍 |
| **L2 — Hooks** | `postCreate` / `postEdit` hooks | 自動化品質檢查 |
| **L3 — CI/CD** | GitHub Actions Workflow | PR 級別的門檻檢查 |
| **L4 — Branch Protection** | Required reviews, status checks | 合併前的最終門檻 |
| **L5 — CODEOWNERS** | 特定檔案需特定人員審核 | 高風險檔案保護 |
...
Copilot Memory（官方亦稱 **Agentic Memory**）是 **Public Preview** 功能，讓 Copilot 記住使用者和專案的偏好、慣例與上下文，避免在每次對話中重複說明。Memory 具備**跨功能共享**特性：Cloud Agent 儲存的 Memory 會自動被 Code Review 和 CLI 引用，反之亦然。

### Memory 特性

| 特性 | 說明 |
|------|------|
| **範圍** | Repository-scoped（每個 Repository 獨立） |
| **有效期** | 28 天自動過期 |
| **使用環境** | Cloud Agent、Code Review、**CLI**（跨功能共享 Agentic Memory） |
| **引用驗證** | Memory 引用時會自動對比目前程式碼庫驗證正確性（Citations） |
| **權限需求** | 需要 Repository write 權限 |
...
|---------|--------|-------------|
| **儲存方式** | Copilot 自動管理 | 檔案系統（Git 版控） |
| **有效期** | 28 天自動過期 | 永久（除非手動刪除） |
| **可見性** | 不可直接查看完整內容 | 完全可見可編輯 |
| **適用場景** | 動態偏好、對話上下文 | 固定規則、團隊規範 |
| **版本控管** | ✗ 不可 | ✓ 可 |
| **團隊共享** | ✗ 個人 | ✓ 透過 Git |
...
| 實務 | 說明 |
|------|------|
| **了解限制** | Memory 28 天過期，不適合作為永久儲存 |
| **使用 Instructions 替代** | 團隊規範應使用 Instructions，不依賴 Memory |
| **定期教育** | 定期提醒團隊 Memory 的安全使用原則 |
| **不依賴** | 關鍵資訊不應只存在 Memory 中，應有其他紀錄 |
| **敏感資訊檢查** | 建立自我檢查機制，確認未將敏感資訊告訴 Copilot |
...
GitHub.com 新增 **Agents Tab**，提供集中式的 Agent 任務管理介面，與 PR 工作流程緊密整合：

| 功能 | 說明 |
|------|------|
| **啟動任務** | 直接從 Agents Tab 指派任務給 Copilot Coding Agent |
| **監控日誌** | 即時檢視 Agent 執行的操作日誌與工具呼叫 |
| **追蹤會話** | 管理所有進行中與歷史的 Agent 會話 |
| **中途引導** | 在 Agent 執行期間介入，修正方向或補充指示 |
| **開啟至 IDE** | 將 Agent 會話轉移至 VS Code 或 CLI 繼續操作 |

> 💡 **企業用途**：Agents Tab 讓 Tech Lead 可以集中檢視團隊所有 Agent 任務的執行狀態，確保 Agent 行為符合預期。
...
本附錄提供 11 個即用範本，可直接複製到專案中使用。所有範本遵循前面章節的最佳實務。
...
| 4 | Repository Instructions | `.github/copilot-instructions.md` | Ch 8 |
| 5 | Java Coding Standards | `.github/instructions/java-coding.instructions.md` | Ch 8 |
| 6 | Security Review Skill | `.github/skills/security-review/SKILL.md` | Ch 9 |
| 7 | 需求分析 Prompt | `.github/prompts/requirements/analyze-user-story.prompt.md` | Ch 7 |
| 8 | 測試產生 Prompt | `.github/prompts/testing/generate-unit-tests.prompt.md` | Ch 7 |
| 9 | PR Template | `.github/PULL_REQUEST_TEMPLATE.md` | Ch 12 |
| 10 | Copilot Review Instructions | `.github/copilot-review-instructions.md` | Ch 12 |
| 11 | 標準目錄結構 | `.github/` 完整目錄 | Ch 5 |
...
## 21.6 範本 4：Repository Instructions
...
## 21.7 範本 5：Java Coding Standards Instructions
...
## 21.8 範本 6：Security Review Skill
...
## 21.12 範本 10：Copilot Review Instructions
