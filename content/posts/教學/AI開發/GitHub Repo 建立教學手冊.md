...
| **知識編譯** | 未整合 LLM Wiki 模式 | 可搭配 LLM Wiki 進行知識編譯與持久化（詳見第 11 章） |
...
> **進階知識健檢**：上述過時檢測為基於時間與格式的淺層掃描。若需更深層的語意層級健檢（如偵測矛盾陳述、過時技術聲明、孤立頁面、缺失交叉參考），可搭配 LLM Wiki 的 Lint 機制進行知識品質驗證，詳見 [**§11.3 核心操作流程 — Lint（知識健檢）**](#113-核心操作流程)。
...
若團隊已導入 LLM Wiki 知識庫（詳見第 11 章），建議依以下原則與 AI 資產知識庫搭配使用：

1. **職責劃分**：AI 資產 Repo 管理可執行的工具型資產（Agents / Skills / Plugins），LLM Wiki 管理研究型與決策型知識
2. **雙向參照**：Repo 內的教學手冊可引用 LLM Wiki 頁面作為背景知識；LLM Wiki 的 Schema 可引用 Repo 的 AGENTS.md
3. **統一索引**：將 LLM Wiki 的 `index.md` 加入 `llms.txt`，讓 AI Agent 能同時發現兩種知識來源
4. **定期 Lint**：每月執行一次 LLM Wiki Lint，檢測與 Repo 內容的矛盾或過時聲明
...
兩者互補而非互斥：
- **AI 資產知識庫**（本文件）：管理**可執行的工具型資產**（Agents / Skills / Plugins / Hooks 等），重點在「被 AI 工具直接載入使用」
- **LLM Wiki**（第 11 章）：管理**研究型與決策型知識**（技術調查、架構決策、會議結論等），重點在「由 LLM 持續編譯、複合成長的知識資產」

建議小型團隊先建立 AI 資產 Repo，等知識量成長後再導入 LLM Wiki 模式。
...
三步驟：
1. 建立目錄結構：`mkdir -p wiki/{raw,wiki,schema}` 
2. 撰寫 CLAUDE.md（Schema）定義資料夾慣例與工作流
3. 將第一份原始資料放入 `raw/`，以 LLM 執行第一次 Ingest

詳見 [§11.8 實作指引](#118-實作指引)。
...
{{< mermaid >}}
graph LR
    subgraph Repo["AI 資產知識庫<br/>（GitHub Repo）"]
        R1["Agents"]
        R2["Skills"]
        R3["Plugins"]
        R4["Instructions"]
