...
> **2026 年更新**：Bypass Actor 過去僅能指定角色（Role）、Team 或 GitHub App，現已支援直接指定**個別使用者（`"actor_type": "User"`）**，不必再為單一人員或服務帳號另外建立專屬 Team 才能授予 Bypass 權限，設定更為精準且易於稽核。
...
> - 過去若分支受 Organization / Enterprise 層級 Ruleset 保護，重新命名該分支需要 org 層級 Bypass 權限；目前 Repo Admin 只要新名稱仍落在 Ruleset 的 `include` 規則範圍內，即可自行重新命名，不再需要向上申請權限
...
> - 2026 年 4 月曾發生佇列中單一群組包含多筆 PR 且採用 Squash Merge 時，產生錯誤合併結果的短暫異常（已由 GitHub 修復），建議企業訂閱 [GitHub Status](https://www.githubstatus.com/) 通報以掌握類似事件
...
- 公開 Repository 的 Artifact Attestation 產生機制正逐步由「選擇性啟用（opt-in）」轉為**預設開啟**
- `actions/attest-build-provenance` 現已成為底層 `actions/attest` Action 的一層包裝；新專案建議直接採用 `actions/attest`，取得更彈性的 Attestation 產生方式
...
| OIDC Subject Claim 不可變格式 | ⚠️ 即將生效（2026-07-15 後建立的 Repo） | 預設 `sub` claim 改採 `owner_id` + `repo_id` 組成，不再以名稱為基礎，Repo 更名或轉移 Owner 後憑證不受影響 |
...
自 2025 年 4 月起，GitHub 已將原本綁定 Enterprise 方案的 GitHub Advanced Security（GHAS）拆分為兩項可獨立加購的產品——**GitHub Secret Protection**（依每位活躍提交者計費）與 **GitHub Code Security**（依每位活躍提交者計費），兩者皆可在 Free、Team、Enterprise 任一方案上單獨採購，**不再要求企業必須升級至 Enterprise 才能取得 Secret Scanning、Push Protection 或 CodeQL 等能力**：
...
**私有 GitHub Packages Registry 認證**：Dependabot 現已可直接使用內建的 `GITHUB_TOKEN`（搭配 `packages: read` 權限）向組織的私有 GitHub Packages Registry 進行認證並解析依賴，不再需要另外建立與管理專屬的 Personal Access Token：
...
registries:
  acme-internal-packages:
    ...
    url: https://maven.pkg.github.com/acme-bank
...
**2026 年 PVR 生態現況**：隨著 AI 輔助漏洞探勘工具普及，Private Vulnerability Reporting 的使用量與 GitHub Advisory Database 的審核負荷在 2026 年出現顯著成長：
...
- **Model Picker（模型選擇器）**：Agent 面板已可依任務複雜度選擇不同模型——簡單任務（如小型 Bug 修正、補測試）可指定較快速/低成本的模型，複雜重構則指定能力更強的模型，不再是單一固定模型
...
- **Visual Studio 整合擴充**：可直接從 IDE 啟動雲端 Agent Session 並持續追蹤進度，並新增專屬 **Debugger Agent**，會實際執行程式碼、比對執行期行為以驗證修補是否有效，而非僅靜態分析程式碼
...
| **Repository 內建整合** | 除 Marketplace Playground 外，模型目錄現已直接surfaced於 Repo 層級設定中，方便在專案脈絡下快速試用與比較模型 |
...
此設定與 [13.7](#137-github-copilot-extensions-與-mcp) 中 `.vscode/mcp.json` 的專案層級設定是**兩個不同的層次**——前者作用於 IDE 內的 Agent Mode，後者則是 Repo 層級、影響 Copilot Code Review 與 Coding Agent 於雲端執行時可用的工具。
...
Codespaces 資料落地已於 **2026-04-01 正式 GA**，企業可指定 Codespaces 的運算與儲存資料落於特定地理區域，目前支援 **Australia、EU、Japan、US** 四個區域，滿足金融、醫療等對資料主權（Data Sovereignty）有嚴格法規要求的產業需求。
...
> 補充說明：上表儲存空間與 **GitHub Actions Artifacts 共用同一份方案額度**（並非各自獨立計算），規劃容量時應合併評估兩者用量；傳輸頻寬以月為單位重置，儲存空間則以帳單週期連續累計計算。
>
