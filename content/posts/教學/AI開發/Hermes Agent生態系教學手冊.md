...
| 多平台 | Web UI 為主 | CLI + TUI + 20+ 通訊平台（Telegram/Discord/Slack/Matrix/WhatsApp/Signal/Email/SMS/DingTalk/Feishu/WeCom/WeChat/BlueBubbles/Mattermost/Home Assistant/QQBot/Yuanbao/Microsoft Teams/Google Chat/Webhook 等） |
...
| 國際化 | 僅介面語言 | 7 語言 i18n（中/日/德/西/法/烏/土） |
| 授權 | 閉源 | MIT（可商用） |
| 成本 | 按月訂閱 | 自控（用多少付多少 Token） |
| Windows 支援 | 原生 | Native 早期測試版（PowerShell 安裝）+ WSL2 完整支援 |
...
v0.13.0 為 `write_file` 和 `patch` 工具新增自動語法檢查，語法錯誤在寫入當下立即浮出，不再向下游傳遞：
...
graph TB
    ...
    subgraph "Tool Layer（工具層）"
        TERM[Terminal Backends<br/>Local/Docker/SSH/Modal/Daytona/Vercel]
...
| Vercel Sandbox | Vercel API | 雲端沙箱執行 |
...
| 30-35+ | OpenAI-compatible | 任何相容 API 端點 | 全版本 |

**v0.13.0 新增模型**：

| 模型 | Provider | 說明 |
|------|----------|------|
| DeepSeek v4 Pro | DeepSeek | 最新代推理模型 |
| Grok 4.3 | xAI | 升級版 Grok |
| Owl Alpha | OpenRouter | 免費模型 |
| hy3-preview | Tencent | 預覽版模型 |
| Arcee Trinity Large Thinking | Arcee AI | 支援溫度 + 壓縮覆寫 |
