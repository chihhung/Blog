...
> **版本**：v1.2　|　**更新日期**：2026-07-20　|　**適用對象**：資深工程師、架構師、技術主管

## 變更紀錄（v1.1 → v1.2）

- 新增「第十五章　Skill Library」：依 4.3.1 節目錄結構，實作 26 個技能檔案（coding／architecture／testing／security／migration 五大分類，Skill-01～Skill-26）
- 原第十五～十八章（最佳實務 Top 50、常見錯誤 Top 50、導入策略、未來發展）依序遞移為第十六～十九章，並同步更新目錄與全文交叉引用
...
- 擴充第十八章導入策略：新增 18.4～18.6，補齊單一迴圈自主程度模型、七種生產級 Loop 模式目錄、loop-audit／loop-cost 自我檢測
...
- [第十五章　Skill Library](#第十五章skill-library)
- [第十六章　最佳實務 Top 50](#第十六章最佳實務-top-50)
- [第十七章　常見錯誤 Top 50 Anti-patterns](#第十七章常見錯誤-top-50-anti-patterns)
- [第十八章　導入策略](#第十八章導入策略)
- [第十九章　未來發展](#第十九章未來發展)
...
| **Orchestration Tax**（編排稅） | 多 Agent 協作（Multi-Agent、Sub-agents）所需的協調、上下文同步、衝突排解，其成本可能超過單一 Agent 直接執行 | Token 成本／執行時間隨 Agent 數量超線性增加，但產出品質未等比提升 | 採用 loop-audit／loop-cost 等量化工具（見第十八章）評估投入產出比，避免過度編排 |

這四項風險彼此並非獨立：**Cognitive Surrender** 會放大 **Verification Risk**（沒有人想驗證，風險就無法被攔截）；而為了緩解 Verification Risk 所疊加的審查層，又可能墊高 **Orchestration Tax**。因此風險治理必須是系統性的，而非單點補丁——這也是本手冊在第十七章「常見錯誤 Top 50 Anti-patterns」與第十八章「導入策略」中，會反覆對照本節風險來設計具體防範措施的原因（見 [[第十七章]]、[[第十八章]]）。
...
> **定位說明**：截至本手冊撰寫時，Graph Harness 仍主要存在於學術研究與少數前沿工具的實驗性實作中，尚未成為業界主流的生產級架構。本節將其納入手冊，目的是為**第十九章「未來發展」**提供嚴謹的理論依據——它指出了當前 Agent Loop 架構在排程理論上的天花板，也預告了下一代企業 Loop Engineering 平台可能演進的方向（見 [[第十九章]]）。
...
- **Pass^k**：對同一 Task 執行 k 次 Trial，**必須全部成功**才視為通過。適合衡量穩定性與可預測性，例如評估能否安全地用於無人值守的生產迴圈（對應第十八章 L3 成熟度）。
...
11.2 節的六大 Eval 維度主要落在第一層；3.6／3.7 節的 Reviewer／Judge 架構橫跨第一層與第五層；第十八章導入策略中的 loop-audit 概念則對應第二層的持續監測。企業在設計 Loop Engineering 治理機制時，應確保五層防護都有對應的負責人與檢視頻率，而不是只投資在最容易自動化的第一層。
...
第 4.3 節介紹了 Skills 的概念與檔案格式，4.3.1 節列出了一份涵蓋 5 大分類、26 個技能檔案的目錄結構，但當時只給出檔名與一行註解。本章依該目錄逐一落地，把每個技能寫成可直接放入 `skills/` 目錄的完整 Markdown 檔案（frontmatter + 內容），供 AI Agent 在對應情境下載入使用。所有技能均遵循 4.3.2 節範例與附錄 C「Skill Library Template」的統一結構：**適用情境 → 核心原則 → 標準做法 → 禁止事項 → 驗收標準**，並統一編號 Skill-01～Skill-26，方便在 Prompt 或 Sub-agent 設定中引用（例如「載入 Skill-09 後再執行任務」）。

### 15.1　Coding Skills（Skill-01 ～ Skill-06）

對應 `skills/coding/` 目錄，涵蓋後端、前端、程式碼品質、設計模式、API 設計與資料庫優化六項日常編碼最常用的技能。

---

**Skill-01：java-spring-boot.md — Spring Boot 開發技能**
...
**Skill-02：vue3-typescript.md — Vue3 + TypeScript 技能**
...
**Skill-03：clean-code.md — Clean Code 技能**
...
**Skill-04：design-patterns.md — 設計模式應用技能**
...
**Skill-05：api-design.md — RESTful API 設計技能**
...
**Skill-06：database-optimization.md — 資料庫優化技能**
...
### 15.2　Architecture Skills（Skill-07 ～ Skill-11）

對應 `skills/architecture/` 目錄，涵蓋微服務、六角形架構、DDD、事件驅動與雲原生五種架構層級的技能，供 Architect Agent（見 3.1 節 Agent 執行層）在系統設計階段載入。

---

**Skill-07：microservices.md — 微服務架構技能**
...
**Skill-08：hexagonal-architecture.md — 六角形架構技能**
...
**Skill-10：event-driven.md — 事件驅動架構技能**
...
**Skill-11：cloud-native.md — Cloud Native 技能**
...
### 15.3　Testing Skills（Skill-12 ～ Skill-16）

對應 `skills/testing/` 目錄，涵蓋單元測試、整合測試、端對端測試、效能測試與安全測試，供 Tester Agent（見 4.5 節）在不同測試層級載入對應技能。

---

**Skill-12：unit-testing-junit5.md — JUnit 5 單元測試技能**
...
**Skill-13：integration-testing.md — 整合測試技能**
...
**Skill-14：e2e-testing-playwright.md — E2E 測試技能（Playwright）**
...
**Skill-15：performance-testing.md — 效能測試技能**
...
**Skill-16：security-testing.md — 安全測試技能**
...
## 核心原則
1. 對照 OWASP Top 10（見 Skill-17）逐項驗證，而非隨機測試
...
## 參考資料
- OWASP Testing Guide
- Skill-17 OWASP Top 10 技能
````

---

### 15.4　Security Skills（Skill-17 ～ Skill-21）

對應 `skills/security/` 目錄，供 Security Agent（見 4.5 節）在安全審查、掃描與威脅建模時載入。

---

**Skill-17：owasp-top10.md — OWASP Top 10 技能**
...
- 不得僅在前端做權限控制而伺服端不驗證
- 不得將加密金鑰、密碼等機密資訊寫死在原始碼中（見 Skill-20）

## 驗收標準
- [ ] SAST 掃描（見 Skill-18）無 Critical/High 等級的 OWASP 對應弱點
...
**Skill-18：sast-analysis.md — 靜態分析技能（SAST）**
...
## 核心原則
1. Quality Gate 是品質門禁，未通過不得合併（見 4.1.1 節 GitHub Actions 範例）
...
## 參考資料
- 本手冊 4.1.1 節 GitHub Actions Quality Gate 範例
- SonarQube 官方文件 — Quality Gates
````

---

**Skill-19：dependency-scanning.md — 相依套件掃描技能**
...
**Skill-20：secrets-management.md — 秘密管理技能**
...
## 核心原則
1. 機密資訊一律不進入原始碼、不進入 AI 的上下文（對應 16.4 最佳實務 BP-39）
...
**Skill-21：threat-modeling.md — 威脅建模技能**
...
### 15.5　Migration Skills（Skill-22 ～ Skill-26）

對應 `skills/migration/` 目錄，供 Legacy System 逆向工程（第九章）與 Framework 升級（第十章）任務中的 Refactoring Agent 載入。

---

**Skill-22：java6-to-java21.md — Java 版本升級技能**
...
**Skill-23：struts-to-spring-boot.md — Struts 遷移技能**
...
2. Struts Action 中混雜的商業邏輯，遷移時同步拆分到 Service 層（見 Skill-08 六角形架構）
...
**Skill-24：jsp-to-vue3.md — JSP 遷移技能**
...
- [ ] 遷移後頁面之核心功能已通過 E2E 測試（見 Skill-14）
...
**Skill-25：spring-boot-2-to-3.md — Spring Boot 升級技能**
...
## 核心原則
1. 先確認執行環境已達 Java 17+（Spring Boot 3 最低需求），對應 Skill-22 的升級路線圖
...
**Skill-26：monolith-to-microservices.md — 單體到微服務技能**
...
## 適用情境
評估單體應用是否該拆分為微服務，並規劃漸進式拆分路線圖時使用（與 Skill-07 微服務架構搭配使用）。
...
## 第十七章　常見錯誤 Top 50 Anti-patterns

### 17.1　Loop 設計反模式（#1–#15）
...
### 18.3　Loop Engineering 導入 Checklist
...
### 18.4　單一迴圈的自主程度模型（L1-L3）與企業成熟度模型的對應
...
### 18.6　導入前自我檢測：loop-audit 與 loop-cost
...
### 19.1　Self-healing Development（自愈式開發）
...
### 19.2　AI Software Factory（AI 軟體工廠）
...
### 19.4　Multi-Agent Enterprise 架構願景
...
### 19.5　下一代執行引擎：Graph Harness 的展望
...
3. **從不可審計走向形式化可驗證**：排程理論的形式化分析方法，使得企業治理單位（對應第十一章 Evals 與第十八章治理機制）未來有機會對「這個迴圈在什麼條件下會卡住、什麼條件下保證收斂」提出可被證明的保證，而不只是經驗性的信心。
