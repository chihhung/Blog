...
- 支援 **15+ LLM Provider（數百個模型）**（Anthropic、OpenAI、Google Gemini、Google Vertex、Azure OpenAI、Amazon Bedrock、DeepSeek、Mistral、Groq、Cerebras、xAI、Hugging Face、Kimi For Coding、MiniMax、NVIDIA、OpenRouter、Ollama、Vercel AI Gateway、ZAI、OpenCode Zen、OpenCode Go、Cloudflare AI Gateway、Cloudflare Workers AI、Fireworks、Together AI、Xiaomi MiMo 等）
- 支援 **本地模型**：透過 Ollama 或相容 OpenAI API 的本地服務，可完全離線運行
...
> **v0.80.2 新功能摘要**：
> - 完整的容器化支援（Gondolin micro-VM、Plain Docker、OpenShell）
> - Project Trust 機制（信任專案控制）
> - Pi Orchestrator（實驗性多代理協調器）
> - 供應鏈安全強化（npm-shrinkwrap、依賴釘選、CI 安全掃描）
> - Custom Editor API、Overlay 元件、Autocomplete Provider
> - `withFileMutationQueue()` 並行工具檔案安全機制
> - 支援 NVIDIA 與 Ollama 本地模型 Provider
...
| **多模型切換** | ✅ 15+ Provider | ❌ Anthropic only | ✅ 多模型 | ✅ 多模型 |
...
Pi v0.80.2 持續強化供應鏈安全機制，是所有開源 AI Agent 中措施最完善的之一：
...
Pi 可作為 TypeScript 函式庫嵌入到自有系統中。SDK API 需要明確建構認證、模型註冊與 Session 管理元件：
...
#### Project Trust 機制（v0.80+）

Pi 在互動模式啟動時，會對含有 `.pi/` 目錄或 `.agents/skills` 的專案進行信任決策。**未受信任的專案不會載入其本地 Extension、設定與 Skills**，確保安全：

| 情境 | 行為 |
|------|------|
| 首次進入含 `.pi/` 的專案 | 顯示信任提示，等候使用者決定 |
| 已儲存信任決策 | 直接套用（存於 `~/.pi/agent/trust.json`） |
| 非互動模式（`-p`、RPC、JSON） | 依 `defaultProjectTrust` 設定決定（預設 `ask`） |
| CLI 覆蓋 | `--approve` / `-a` 強制信任；`--no-approve` / `-na` 強制拒絕 |

```bash
# 在互動模式中儲存信任決策
/trust

# 全域設定預設行為（settings.json）
{
  "defaultProjectTrust": "ask"  // "ask" | "always" | "never"
}
```

> **企業建議**：在 CI/CD 環境中建議設定 `defaultProjectTrust: "always"` 或使用 `--approve` 旗標，避免自動化流程卡在信任提示。生產環境的安全邊界應由容器化（§16）而非 Trust 機制來保障。
...
#### Extension 生命週期（v0.80+ 完整事件流）

```
pi starts
  ├─► project_trust（決定是否信任專案）
...
| 功能 | 說明 |
|------|------|
| **容器化支援** | 新增 Gondolin（micro-VM）、Plain Docker、OpenShell 三種沙箱模式 |
| **Project Trust** | 專案信任機制，控制是否載入專案本地 Extension 與設定 |
| **Pi Orchestrator（實驗性）** | 多代理協調器，支援自動化任務分派 |
| **Custom Editor API** | 完整的自訂編輯器 API（Vim/Emacs 模式支援） |
| **Overlay 元件** | 浮動面板 UI，支援定位、焦點管理、響應式顯示 |
| **Autocomplete Provider** | 可疊加的自訂自動補全邏輯（如 `#issue` 補全） |
| **withFileMutationQueue** | 並行工具檔案修改佇列，防止 race condition |
| **Supply-chain 強化** | npm-shrinkwrap、依賴釘選、lifecycle scripts 白名單 |
| **Named Startup Sessions** | `--name` / `-n` 在啟動時命名 Session |
| **可點擊檔案路徑** | OSC 8 `file://` 超連結，支援 tmux |
| **NVIDIA / Ollama Provider** | 新增本地模型與 NVIDIA GPU 推論支援 |
| **pi update --all** | 一次更新 Pi 本體與所有已安裝套件 |
| **Timed Dialogs** | 帶倒計時的對話框（`timeout` 選項） |
| **before_provider_request** | 攔截並修改 Provider 請求 payload |
| **after_provider_response** | 接收 HTTP 狀態碼與回應標頭 |
...
Pi **刻意不內建權限系統**。預設情況下，Pi 以啟動它的使用者權限運行，可存取檔案系統、網路與進程。這在開發環境中是合理的，但在以下情境中需要更強的隔離邊界：
...
Gondolin 將 Pi 主進程與 Provider 認證保留在**宿主機**，僅將內建工具（`read`、`write`、`edit`、`bash`）與 `!` 命令路由到本地 Linux micro-VM：
...
#### (3) OpenShell（推薦：高安全企業環境）

OpenShell 提供政策控制的沙箱，支援細粒度的網路、檔案系統與進程存取控制：

```bash
# 使用 OpenShell 沙箱執行 Pi
openshell run --policy strict -- pi -p "Generate unit tests"
```

**特點**：
- 支援 RBAC 風格的政策定義
- 可限制網路出站規則（如僅允許存取 LLM API endpoint）
- 可限制檔案系統存取範圍
- 適合金融、醫療等高合規要求行業
...
> **文件維護**：本手冊隨 Pi Code Agent 版本更新而修訂。最後更新日期：2026 年 7 月 1 日。
