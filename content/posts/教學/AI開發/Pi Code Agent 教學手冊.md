...
- 透過 **Pi Packages** 機制（npm / git）分享與安裝擴充套件
- 支援 **30+ LLM Provider**（Anthropic、OpenAI、Google、Azure、Bedrock、DeepSeek、Mistral、Groq、Cerebras、xAI、Hugging Face、Kimi For Coding、MiniMax、OpenRouter、Vercel AI Gateway、ZAI、OpenCode Zen、OpenCode Go、Google Vertex、Cloudflare AI Gateway、Cloudflare Workers AI、Fireworks、Together AI、Xiaomi MiMo 等）
- npm 套件名稱：`@earendil-works/pi-coding-agent`
- Monorepo 包含 4 個核心套件：`pi-ai`（統一多 Provider LLM API）、`pi-agent-core`（Agent 執行時期與工具呼叫）、`pi-coding-agent`（互動式編碼 Agent CLI）、`pi-tui`（終端 UI 差異渲染函式庫）
- 4 種運行模式：Interactive（互動）、Print / JSON（非互動）、RPC（JSON stdin/stdout）、SDK（程式化嵌入）
...
> **重要澄清**：Pi 不「內建」Plan Mode、Sub-Agents、MCP 等功能，但透過其 Extension 系統皆可實現。官方 `examples/extensions/` 提供了 60+ 個範例，涵蓋 plan-mode、subagent、sandbox 沙箱、SSH 遠端執行、permission-gates 權限閘門、custom-provider、interactive-shell、Git checkpoint 等。Pi 的哲學是**不在核心中堆疊功能**，而是讓使用者按需組裝。你甚至可以直接請 Pi 為你建構 Extension——`pi can create extensions`。
...
Pi v0.77.0 強化了安裝安全機制：

| 安全措施 | 說明 |
|---------|------|
| `--ignore-scripts` 預設 | `npm install` 加入 `--ignore-scripts` 防止 postinstall 惡意腳本 |
| `npm-shrinkwrap.json` | 所有依賴版本完全鎖定，可重現建構 |
| 依賴釘選（pinned deps） | 不使用 floating version，避免被供應鏈攻擊 |
| Pi Packages 權限模型 | Extension 執行於主程序沙箱內，但擁有系統存取權限 |
...
|------|------|
| `PI_OFFLINE` | 完全離線模式 |
| `PI_SKIP_VERSION_CHECK` | 跳過啟動時的版本檢查 |
| `PI_TELEMETRY` | `0` 停用匿名遙測 |
| `PI_CACHE_RETENTION` | 快取保留天數（預設 7） |
| `PI_CODING_AGENT_DIR` | 自訂全域設定目錄（取代 `~/.pi/agent`） |
| `PI_CODING_AGENT_SESSION_DIR` | 自訂 Session 儲存路徑 |
| `PI_PACKAGE_DIR` | 自訂已安裝 Pi Packages 目錄 |
...
const session = await createAgentSession({
  ...
  systemPrompt: "你是團隊的自動化助手，負責程式碼審查與重構。",
});

const result = await session.sendMessage(
  "請審查 src/main/java 目錄下的 Controller 是否符合 RESTful 規範"
);
...
> **實際案例**：[openclaw/openclaw](https://github.com/openclaw/openclaw) 專案使用 Pi SDK 建構了法律文件自動化處理系統。
...
# 從 Git 安裝並 pin 到特定 commit/tag
pi install git:github.com/myteam/pi-enterprise-tools@v1.3.0
...
# 安裝特定版本
npm install -g --ignore-scripts @earendil-works/pi-coding-agent@0.77.0
