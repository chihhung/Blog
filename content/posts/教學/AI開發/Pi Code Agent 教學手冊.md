+++
date = '2026-04-21T14:29:33+08:00'
draft = false
...
13. [Best Practices（企業建議）](#13-best-practices企業建議)
14. [Anti-Patterns（重要）](#14-anti-patterns重要)
15. [結論（Conclusion）](#15-結論conclusion)
...
Pi（又名 Pi Coding Agent）是由 [Mario Zechner（badlogic）](https://github.com/badlogic) 與 [Earendil Inc.](https://earendil.com/) 開發的**極簡終端 AI 程式碼開發工具**，以 MIT License 開源於 [GitHub](https://github.com/badlogic/pi-mono)。其核心定位為：

- **極簡終端控制框架**（Minimal Terminal Coding Harness）
- 可直接操作檔案、執行指令、生成程式碼
- 透過 TypeScript Extensions、Skills、Prompt Templates、Themes 進行擴充
- 支援 **20+ LLM Provider**（Anthropic、OpenAI、Google、Azure、Bedrock、Mistral、Groq、Cerebras、xAI、Hugging Face、Kimi For Coding、MiniMax、OpenRouter、Ollama、Vercel AI Gateway、ZAI、OpenCode Zen、OpenCode Go、Google Vertex 等）
- npm 套件名稱：`@mariozechner/pi-coding-agent`
- Monorepo 包含 7 個核心套件：`pi-ai`、`pi-agent-core`、`pi-coding-agent`、`pi-mom`、`pi-tui`、`pi-web-ui`、`pi-pods`
- 4 種運行模式：Interactive（互動）、Print（非互動）、RPC（JSON stdin/stdout）、SDK（程式化嵌入）

Pi 的設計理念是「**適應你的工作流，而非讓你適應工具**」。
...
| **多模型切換** | ✅ 20+ Provider | ❌ Anthropic only | ❌ 有限 | ✅ 多模型 |
...
> **重要澄清**：Pi 不「內建」Plan Mode、Sub-Agents、MCP 等功能，但透過其 Extension 系統皆可實現。官方 `examples/extensions/` 提供了 50+ 個範例，涵蓋 plan-mode、subagent、MCP 整合、sandbox 沙箱、SSH 遠端執行、permission-gates 權限閘門等。Pi 的哲學是**不在核心中堆疊功能**，而是讓使用者按需組裝。
...
| **No Built-in MCP** | Extension 整合 | MCP 可透過 Extension 實現，保持核心簡潔 |
...
> **平台支援說明**：
> - **Windows**：需使用 Windows Terminal；不支援原生 `Ctrl+Z` 暫停，但其餘功能完整。詳見 [docs/windows.md](https://github.com/badlogic/pi-mono/blob/main/packages/coding-agent/docs/windows.md)。
> - **macOS / Linux**：完整支援，包含 `Ctrl+Z` 暫停/恢復。Linux ARM64（musl）亦支援。
> - **Android（Termux）**：`pkg install nodejs termux-api git && npm install -g @mariozechner/pi-coding-agent`
> - **tmux**：建議搭配使用，需啟用 `set -g extended-keys on`。詳見 [docs/tmux.md](https://github.com/badlogic/pi-mono/blob/main/packages/coding-agent/docs/tmux.md)。
...
| **OpenAI ChatGPT Plus/Pro（Codex）** | 支援 `gpt-5.1`、`gpt-5.2`、`gpt-5.4` 等模型 |
| **GitHub Copilot** | 支援 Copilot Business/Enterprise 帳戶 |
| **Google Gemini CLI** | Google AI Studio 訂閱 |
| **Google Antigravity** | Google 訂閱服務，支援 Claude 和 Gemini 模型 |
...
| `ERR_MODULE_NOT_FOUND` | pnpm 嚴格模式缺少依賴 | 重新安裝 `npm install -g @mariozechner/pi-coding-agent` |
...
| `/share` | 分享 Session 到 pi.dev |
| `/export` | 匯出 Session 為 HTML 或 JSONL |
| `/import` | 匯入 JSONL Session |
| `/copy` | 複製最後回應到剪貼簿 |
| `/config` | 檢視/切換已載入的資源 |
| `/settings` | 開啟設定選單 |
| `/reload` | 熱重載所有資源（AGENTS.md / Extension / Skill 等） |
| `/login` | OAuth 訂閱登入 |
| `/name` | 為當前 Session 命名 |
| `/files` | 列出本次 Session 所有檔案操作 |
...
Extensions 是 TypeScript 模組，擁有完整的 Pi API 存取權限。
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("rollback", {
    ...
    handler: async (args, ctx) => {
      ...
      ctx.notify("使用 git revert <commit> 來回滾特定變更");
    },
  });
}
```
    ...
    if (event.toolName === "write" || event.toolName === "edit") {
      ...
      if (protectedPaths.some((p) => filePath.includes(p))) {
        const confirmed = await ctx.confirm(
          `⚠️ 即將修改受保護檔案：${filePath}\n確定要繼續嗎？`
        );
        if (!confirmed) {
          return { abort: true, message: "操作已取消" };
        }
      }
    }
  });
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    handler: async (args, ctx) => {
      ...
      // 禁止直接部署到 prod
      if (environment === "prod") {
        return { error: "生產環境部署必須透過 CI/CD Pipeline" };
      }

      ctx.notify(`開始部署 v${version} 到 ${environment}...`);

      const result = await ctx.bash(
        `./scripts/deploy.sh --env ${environment} --version ${version}`
      );
...
將 Extension、Skill、Prompt Template 打包為 Pi Package：
...
# 試用（不安裝，僅本次 Session 載入）
pi install npm:@myteam/pi-enterprise-tools -e

# 安裝指定版本（pinned version）
...
3. ✅ 檢查 [CHANGELOG](https://github.com/badlogic/pi-mono/blob/main/packages/coding-agent/CHANGELOG.md) 是否有 Breaking Change
4. ✅ 在非生產環境先測試
...
|------|------|
| 官方網站 | https://pi.dev/ |
| GitHub 源碼 | https://github.com/badlogic/pi-mono |
| npm 套件 | https://www.npmjs.com/package/@mariozechner/pi-coding-agent |
| Discord 社群 | https://discord.com/invite/3cU7Bz4UPx |
| Extension 範例 | https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent/examples/extensions |
| Skills 範例 | https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent/examples/skills |
| Prompt 範例 | https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent/examples/prompts |
| Theme 範例 | https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent/examples/themes |
| 設計哲學部落格 | https://mariozechner.at/posts/2025-11-30-pi-coding-agent/ |
| CHANGELOG | https://github.com/badlogic/pi-mono/blob/main/CHANGELOG.md |
...
- [ ] 安裝 Node.js v20 LTS
- [ ] 安裝 Pi：`npm install -g @mariozechner/pi-coding-agent`
- [ ] 確認版本：`pi --version`
- [ ] 設定 API Key（環境變數）
- [ ] 測試啟動：`pi "Hello, Pi!"`
...
> **文件維護**：本手冊隨 Pi Code Agent 版本更新而修訂。最後更新日期：2026 年 4 月 21 日。
