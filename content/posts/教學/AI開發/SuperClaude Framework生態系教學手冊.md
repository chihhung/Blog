...
- [第四章：30 個 Slash 指令完整指南](#第四章30-個-slash-指令完整指南)
  - [4.10 指令差異比較與選擇指南](#410-指令差異比較與選擇指南)
- [第五章：20 個 AI 代理人使用指南](#第五章20-個-ai-代理人agents使用指南)
  - [5.4 代理人協調模式](#54-代理人協調模式agent-coordination-patterns)
...
- [第九章：Flags 旗標使用指南](#第九章flags-旗標使用指南)
  - [9.4 自動啟動旗標](#94-自動啟動旗標thinking-depth-flags)
  - [9.5 MCP Server 旗標](#95-mcp-server-旗標)
  - [9.6 行為模式旗標](#96-行為模式旗標)
  - [9.7 執行控制旗標](#97-執行控制旗標)
  - [9.8 旗標交互作用與優先規則](#98-旗標交互作用與優先規則)
...
| # | 模式 | 圖示 | 用途 |
|---|------|------|------|
| 1 | Brainstorming Mode | 🧠 | 發散式思考、需求探索、蘇格拉底提問 |
| 2 | Introspection Mode | 🔍 | 元認知分析、錯誤修復、決策透明化 |
| 3 | Deep Research Mode | 🔬 | 深度技術調研、Multi-Hop 推理 |
| 4 | Task Management Mode | 📋 | 系統化任務組織、跨 Session 持久化 |
| 5 | Orchestration Mode | 🎯 | 智慧工具路由、多工具平行執行 |
| 6 | Token-Efficiency Mode | ⚡ | 符號系統壓縮、節省 30-50% Token |
| 7 | Standard Mode | 🎨 | 平衡預設模式、清晰專業溝通 |

> 📝 **說明**：`/sc:business-panel` 是一個**指令**（Expert Panel 功能），而非獨立的行為模式。詳見第四章 §4.7。
...
在實際工作中，部分指令功能看似相近但職責邊界明確。本節提供系統化的比較與選擇邏輯。

### spawn vs task vs implement：任務分解三兄弟

| 比較維度 | `/sc:spawn` | `/sc:task` | `/sc:implement` |
|---------|------------|-----------|-----------------|
| **職責** | 分解複雜專案為階層式子任務 | 管理離散任務的執行與追蹤 | 直接撰寫程式碼 |
| **類比** | 專案經理（PM）拆解 WBS | 工程師從看板拿一張卡片完成 | 程式設計師寫程式 |
| **產出** | 任務階層文件（不含程式碼） | 完成特定任務（可含程式碼） | 程式碼檔案 |
| **複雜度** | 高（跨多模組/多人）| 中（單一明確任務）| 低-中（單一功能實作）|
| **自動結束** | 產出計畫即停止 | 完成任務即停止 | 程式碼通過驗證即停止 |

**選擇流程圖**：
```text
你的需求是什麼？
├─ "把大專案拆成可執行的步驟" → /sc:spawn
├─ "完成一個明確的小任務"     → /sc:task  
└─ "寫出特定功能的程式碼"     → /sc:implement
```

**實戰範例**：
```text
# Step 1：先用 spawn 拆解專案
/sc:spawn "建置完整的電商購物車功能"
# 產出：任務階層（購物車 CRUD、結帳流程、庫存檢查、支付整合...）

# Step 2：用 task 管理逐一執行
/sc:task "實作購物車 CRUD API"

# Step 3：或用 implement 直接寫特定功能
/sc:implement "購物車加入商品 API endpoint" --with-tests
```

### design vs workflow：設計與流程的界線

| 比較維度 | `/sc:design` | `/sc:workflow` |
|---------|-------------|---------------|
| **職責** | 產出架構設計文件 | 產出實作計畫與步驟流程 |
| **思考角度** | 「系統長什麼樣」（What） | 「如何一步步建起來」（How） |
| **產出格式** | 架構圖、元件關係、技術選型 | 有序步驟清單、依賴關係、里程碑 |
| **使用時機** | 技術選型、系統邊界定義 | 專案執行規劃、Sprint 計畫 |

**組合使用**：
```text
# 先設計系統架構
/sc:design "微服務架構 - 訂單管理系統"

# 再規劃實作流程
/sc:workflow "依據上述架構設計，規劃 4 週實作計畫"
```

### index-repo vs Serena：程式碼理解兩把刷

| 比較維度 | `/sc:index-repo` | `--serena` (Serena MCP) |
|---------|-----------------|------------------------|
| **實作方式** | SuperClaude 內建指令 | 外部 MCP Server 整合 |
| **分析方式** | 快速掃描 + 統計摘要 | 語義層級 AST 深度分析 |
| **速度** | 快（秒級完成） | 較慢（需 AST 解析） |
| **適用場景** | 專案初始認識、快速概覽 | 深度重構、依賴追蹤、影響分析 |
| **需要安裝** | 否（內建） | 是（需安裝 Serena MCP Server） |
...
# 準備重構核心模組，需要精確的依賴關係
/sc:analyze "src/core/" --serena --think-hard
```
...
| 場景 | 推薦組合 | 說明 |
|------|---------|------|
| 新專案啟動 | `index-repo` → `design` → `spawn` → `workflow` | 認識 → 設計 → 拆解 → 排程 |
| 功能開發 | `brainstorm` → `implement` → `test` | 探索 → 實作 → 驗證 |
| 品質改善 | `analyze` → `improve` → `cleanup` | 診斷 → 改善 → 清理 |
| 技術調研 | `research` → `design` → `estimate` | 調研 → 設計 → 估算 |
| 故障排除 | `troubleshoot` → `fix` → `test` | 診斷 → 修復 → 驗證 |
...
## 5.4 代理人協調模式（Agent Coordination Patterns）

SuperClaude 的代理人不僅可以單獨使用，更可以組成團隊（Team）以預定義的協調模式進行協作。以下是三種核心協調模式：

### Architecture Teams（架構團隊）

用於系統設計與技術決策，由架構相關代理人組成常備團隊：

| 團隊成員 | 角色職責 |
|---------|---------|
| `@agent-backend` | 後端架構設計、API 規格、資料模型 |
| `@agent-frontend` | 前端架構、元件設計、狀態管理 |
| `@agent-database` | 資料庫架構、索引策略、查詢最佳化 |
| `@agent-devops` | 部署架構、CI/CD、基礎設施即程式碼 |

**觸發時機**：
- `/sc:design` 涉及跨多層（前後端 + DB + 部署）的系統設計
- `--orchestrate` 旗標搭配架構類型任務

**協調流程**：
```text
Developer → /sc:design "分散式訂單系統架構"
  ├─ @agent-backend: 設計微服務邊界與 API Contract
  ├─ @agent-database: 設計分庫分表策略
  ├─ @agent-frontend: 設計 BFF 層與前端狀態同步
  └─ @agent-devops: 設計容器編排與服務網格
  → 整合產出：統一架構設計文件（含取捨說明）
```

### Quality Teams（品質團隊）

用於品質保證與安全驗證，在程式碼提交前進行多維度審查：

| 團隊成員 | 角色職責 |
|---------|---------|
| `@agent-quality-engineer` | 測試策略、覆蓋率分析、品質門檻 |
| `@agent-security` | OWASP 合規、弱點掃描、威脅建模 |
| `@agent-performance` | 效能基準、負載測試、瓶頸分析 |
| `@agent-accessibility` | WCAG 合規、無障礙測試 |

**觸發時機**：
- `/sc:review` 或 `/sc:analyze` 搭配 `--focus quality`
- PR 提交前的自動審查流程

**協調流程**：
```text
Developer → /sc:review "src/checkout/" --orchestrate
  ├─ @agent-quality: 程式碼品質 + 測試覆蓋率檢查
  ├─ @agent-security: 安全弱點掃描（SQL Injection、XSS、CSRF）
  ├─ @agent-performance: 效能熱點識別（N+1 查詢、記憶體洩漏）
  └─ @agent-accessibility: 前端元件無障礙合規
  → 整合產出：品質報告卡 + 改善優先序
```

### Communication Teams（溝通團隊）

用於文件產出與知識管理，確保技術知識的準確傳遞：

| 團隊成員 | 角色職責 |
|---------|---------|
| `@agent-docs` | API 文件、使用者手冊、README |
| `@agent-architect` | 架構決策紀錄（ADR）、技術白皮書 |
| `@agent-quality-engineer` | 測試報告、品質儀表板 |

**觸發時機**：
- `/sc:document` 涉及跨領域的文件產出
- Sprint 結束時的文件更新

### 指令與代理人映射表

以下對照表顯示每個常用指令預設啟動的代理人：

| 指令 | 主要代理人 | 輔助代理人 |
|------|----------|----------|
| `/sc:implement` | backend / frontend | quality-engineer |
| `/sc:design` | architect | backend, frontend, database |
| `/sc:test` | quality-engineer | security（安全測試時） |
| `/sc:review` | quality-engineer | security, performance |
| `/sc:analyze --focus security` | security | quality-engineer |
| `/sc:analyze --focus performance` | performance | backend |
| `/sc:document` | docs | architect |
| `/sc:troubleshoot` | devops | backend |
| `/sc:research` | （無固定代理人） | 依主題動態選擇 |

> 📝 **注意**：代理人協調是自動完成的。開發者只需下達高階指令，系統會依據任務特性自動組建最適合的團隊組合。使用 `--no-delegate` 旗標可禁止協調，強制單一代理人完成任務。
...
| # | 模式名稱 | 中文說明 | 圖示 | 回應風格 | 最佳使用場景 |
|---|---------|---------|------|---------|------------|
| 1 | **Brainstorming Mode** | 腦力激盪模式 | 🧠 | 發散、創意、多方案 | 需求探索、功能規劃初期 |
| 2 | **Introspection Mode** | 自省模式 | 🔍 | 反思、批判、元認知 | 錯誤修復、決策分析、品質回顧 |
| 3 | **Deep Research Mode** | 深度研究模式 | 🔬 | 嚴謹、引用、深度 | 技術調研、競品分析 |
| 4 | **Task Management Mode** | 任務管理模式 | 📋 | 結構化、追蹤、優先序 | 多步驟操作、跨 Session 工作 |
| 5 | **Orchestration Mode** | 編排模式 | 🎯 | 系統性、流程化、協調 | 複雜多工具協調任務 |
| 6 | **Token-Efficiency Mode** | 省 Token 模式 | ⚡ | 精簡、直接、高密度 | 長對話、大型程式庫分析 |
| 7 | **Standard Mode** | 標準模式 | 🎨 | 清晰、專業、適度 | 簡單明確任務、預設模式 |

> 📝 **重要澄清**：`/sc:business-panel` 是一個**指令功能**（Expert Panel，詳見第四章 §4.7），而非獨立的行為模式。早期版本文件可能將其列為模式，此處已依據官方 [modes.md](https://github.com/SuperClaude-Org/SuperClaude_Framework/blob/master/docs/user-guide/modes.md) 修正。
...
### 2. Introspection Mode — 自省模式

**進入方式**：
```text
/sc:reflect
# 或使用旗標
/sc:command --introspect
```

**自動啟動觸發條件**：
- 自我分析請求：「分析我的推理過程」、「反思決策」
- 錯誤修復場景：執行結果不如預期時
- 多種可行方案的複雜問題解決
- 需要優化的模式識別
...
**產出範例**：
```markdown
🧠 Meta-Analysis: 為何選擇結構分析而非功能流分析？
🎯 Decision Logic: 看到類別階層 → 假設 OOP 模式主導
🔄 Alternative Considered: 資料流分析可能揭露隱藏的依賴關係
📊 Evidence Check: import 語句顯示函式組合非 OOP
💡 Learning: 需先分析 import 再選分析策略
⚡ Correction: 切換至函式流分析方法
```
...
/sc:pm "管理專案"
# 或使用旗標
/sc:command --task-manage
```

**自動啟動觸發條件**：
- 操作需要 >3 個協調步驟
- 多檔案/多目錄範圍（>2 目錄 OR >3 檔案）
- 複雜依賴關係需要階段性與檢查點
- 品質改善請求：polish、refine、enhance

**行為特徵**：
- 階層式規劃：將複雜工作拆解為 Plan → Phase → Task → Todo 結構
- Session 持久化：跨中斷維護專案上下文與進度
- 記憶整合：使用 write_memory/read_memory 進行狀態保存
- 進度編排：協調 TodoWrite 與記憶更新進行追蹤
- 品質門檻：在階段轉換之間實作系統化驗證檢查點

---

### 5. Orchestration Mode — 編排模式

**進入方式**：
```text
/sc:workflow "流程定義"
# 或使用旗標
/sc:command --orchestrate
```

**自動啟動觸發條件**：
- 任務涉及 >3 個不同工具或代理人
- 需要並行執行多個獨立子任務
- 複雜的端到端工作流程（設計 → 實作 → 測試 → 部署）

**行為特徵**：
- 智慧工具路由：自動識別最適合的工具/代理人組合
- 並行執行：獨立子任務同時進行以提升效率
- 依賴管理：追蹤任務間的前置條件與交付物
- 結果整合：彙整所有代理人的產出為統一報告
...
### 6. Token-Efficiency Mode — 省 Token 模式
...
**行為特徵**：
- 符號系統壓縮：使用 `→`、`∴`、`≈`、`⊕` 等取代冗長文字
- 結構性壓縮：`impl`→implementation、`fn`→function、`req`→requirement
- 程式碼只展示 diff 格式（變更部分）
- 避免重複已知資訊
- 使用分層省略：先顯示摘要，需要時再展開細節
...
> 💡 **提示**：搭配 MCP Server 使用時，Token-Efficiency Mode 的節省效果最顯著，可達 50%。

---

### 7. Standard Mode — 標準模式

**進入方式**：
```text
# Standard Mode 是預設模式，無需特別啟用
# 從其他模式返回預設：
請回到標準模式
```
...
| **旗標觸發** | `--brainstorm` / `--introspect` / `--orchestrate` | 附加於任何指令後 |
...
| Deep Research | `--think-hard` / `--ultrathink` |
| Task Management | `--task-manage` |
...
| 技術選型 | Deep Research + Introspection | 深度調研 + 決策分析 |
...
> 💡 **提示**：在一個完整的 Sprint 中，建議在規劃日使用 Brainstorming + Deep Research，開發日使用 Task Management，Sprint Review 時使用 Introspection。
...
**建議使用的模式**：Brainstorming Mode → Deep Research Mode → Introspection Mode
...
## 9.4 自動啟動旗標（Thinking Depth Flags）

自動啟動旗標控制 Claude 的推理深度，直接影響回應品質與 Token 消耗量。

| Flag | 別名 | Token 消耗估計 | 使用場景 |
|------|------|--------------|---------|
| `--think` | - | ~4,000 tokens | 一般複雜問題、程式碼審查 |
| `--think-hard` | - | ~10,000 tokens | 架構設計、多步驟推理 |
| `--ultrathink` | - | ~32,000 tokens | 極端複雜問題、安全性深度分析 |

**使用範例**：
```text
# 一般審查
/sc:review "src/auth/" --think

# 架構設計需要深度推理
/sc:design "微服務拆分策略" --think-hard

# 極端複雜的安全性分析
/sc:analyze "整體系統" --focus security --ultrathink
```

**自動觸發規則**：
- 當任務複雜度評估為「高」時，系統自動升級至 `--think`
- 安全性相關請求自動啟用 `--think-hard`
- 使用者可隨時手動覆寫系統判斷

> ⚠️ **注意**：`--ultrathink` 消耗大量 Token，建議僅在關鍵決策點使用。

## 9.5 MCP Server 旗標

MCP Server 旗標用於啟用或指定特定 MCP 工具整合。

| Flag | 啟用的 MCP Server | 功能說明 |
|------|------------------|---------|
| `--c7` | Context7 | 啟用即時技術文件查詢（庫版本、API 參考） |
| `--seq` | Sequential Thinking | 啟用結構化逐步推理引擎 |
| `--magic` | Magic MCP | 啟用 AI 輔助 UI 元件生成 |
| `--play` | Playwright | 啟用瀏覽器自動化測試 |
| `--chrome` | Chrome DevTools | 啟用 Chrome 開發工具偵錯整合 |
| `--tavily` | Tavily Search | 啟用網路搜尋與即時資訊檢索 |
| `--morph` | Morphllm | 啟用多模態 LLM 處理（圖片、PDF） |
| `--serena` | Serena | 啟用語義程式碼分析引擎 |

**組合使用範例**：
```text
# 深度研究需要網路搜尋 + 文件查詢
/sc:research "Kubernetes 1.30 新特性" --tavily --c7

# UI 開發需要元件生成 + 瀏覽器測試
/sc:implement "互動式儀表板" --magic --play

# 程式碼分析需要語義理解
/sc:analyze "src/" --serena --think-hard
```

## 9.6 行為模式旗標

行為模式旗標可將任何指令的執行切換至特定行為模式。

| Flag | 啟用的模式 | 效果說明 |
|------|----------|---------|
| `--brainstorm` | Brainstorming Mode | 發散式多方案回應 |
| `--introspect` | Introspection Mode | 元認知分析、推理透明化 |
| `--task-manage` | Task Management Mode | 結構化任務追蹤 |
| `--orchestrate` | Orchestration Mode | 多工具協調執行 |
| `--uc` | Token-Efficiency Mode | 壓縮符號、精簡輸出 |

**使用範例**：
```text
# 以腦力激盪模式討論 API 設計
/sc:design "RESTful API" --brainstorm

# 以自省模式審查程式碼品質
/sc:review "src/" --introspect
...
執行控制旗標影響指令的執行流程與安全行為。

| Flag | 說明 | 預設值 | 使用場景 |
|------|------|-------|---------|
| `--loop` | 迭代執行直到滿足條件 | false | 測試修復、漸進式重構 |
| `--safe-mode` | 強制安全模式（額外驗證） | false | 生產環境操作 |
| `--validate` | 執行後自動驗證結果 | false | CI/CD 整合 |
| `--delegate` | 允許委派子任務給其他代理人 | true | 複雜多人任務 |
| `--no-delegate` | 禁止委派，強制單一代理人完成 | false | 需要一致性的操作 |

**迭代執行範例**：
```text
# 持續修復直到所有測試通過
/sc:fix "src/auth/" --loop --validate

# 安全模式下進行資料庫遷移
/sc:implement "DB migration" --safe-mode --think-hard
```
...
| 組合 | 效果 |
|------|------|
| `--think-hard` + `--serena` | 深度推理 + 語義分析 = 最佳程式碼理解 |
| `--uc` + `--quiet` | 雙重壓縮 = 最省 Token |
| `--orchestrate` + `--delegate` | 協調模式 + 委派 = 最高效率 |
| `--loop` + `--validate` | 迭代 + 驗證 = 自動修復循環 |
| `--think` + `--tavily` + `--c7` | 推理 + 搜尋 + 文件 = 最佳研究 |

### 衝突組合（系統自動解決）

| 衝突 | 解決規則 |
|------|---------|
| `--verbose` + `--quiet` | `--quiet` 優先（精簡勝出） |
| `--verbose` + `--uc` | `--uc` 優先（省 Token 優先） |
| `--safe-mode` + `--aggressive` | `--safe-mode` 優先（安全優先） |
| `--brainstorm` + `--uc` | `--brainstorm` 優先（創意不可壓縮） |

### 旗標優先權階層

當多個旗標同時存在時，系統依以下優先順序解決衝突：

```text
Safety（安全）  >  Explicit（明確指定）  >  Depth（深度）  >  MCP Control  >  Scope（範圍）
   最高優先                                                                    最低優先
```

**具體規則**：
1. **Safety 層**：`--safe-mode` 永遠優先，覆寫任何可能造成危險的旗標
2. **Explicit 層**：使用者明確指定的旗標優先於自動推斷
3. **Depth 層**：`--ultrathink` > `--think-hard` > `--think` > 預設
4. **MCP Control 層**：MCP Server 旗標之間無衝突，可任意組合
5. **Scope 層**：範圍限定旗標（`--scope`）在最低優先權
...
| 觸發旗標 | 自動啟用 | 原因 |
|---------|---------|------|
| `--ultrathink` | `--think-hard` | 超深度包含深度 |
| `--orchestrate` | `--delegate` | 編排模式需要委派能力 |
| `--safe-mode` | `--validate` | 安全模式必須驗證 |
| `--loop` | `--validate` | 迭代需要驗證才能判斷終止 |
...
| # | 模式 | 圖示 | 觸發方式 | 一句話說明 |
|---|------|------|---------|-----------|
| 1 | Brainstorming | 🧠 | `/sc:brainstorm` / `--brainstorm` | 發散創意，多方案探索 |
| 2 | Introspection | 🔍 | `/sc:reflect` / `--introspect` | 元認知分析，推理透明化 |
| 3 | Deep Research | 🔬 | `/sc:research` / `--think-hard` | 多次迭代深度調研 |
| 4 | Task Management | 📋 | `/sc:task` / `--task-manage` | 結構化任務追蹤 |
| 5 | Orchestration | 🎯 | `/sc:workflow` / `--orchestrate` | 多代理人協調執行 |
| 6 | Token-Efficiency | ⚡ | `--uc` | 符號壓縮，省 30-50% Token |
| 7 | Standard | 🎨 | （預設模式） | 清晰專業，平衡預設 |

> 📝 `/sc:business-panel` 是**指令**（Expert Panel 功能），非行為模式。

### 常用 Flags 速查

| Flag | 常用值 | 說明 |
|------|-------|------|
| `--think` | （布林） | 啟用深度推理（~4K tokens） |
| `--think-hard` | （布林） | 更深推理（~10K tokens） |
| `--ultrathink` | （布林） | 極限推理（~32K tokens） |
...
| `--loop` | （布林） | 迭代執行直到通過 |
| `--safe-mode` | （布林） | 強制安全模式 |
| `--uc` | （布林） | 啟用 Token-Efficiency Mode |
| `--serena` | （布林） | 啟用語義分析 MCP |
| `--tavily` | （布林） | 啟用網路搜尋 MCP |
| `--c7` | （布林） | 啟用技術文件查詢 MCP |
