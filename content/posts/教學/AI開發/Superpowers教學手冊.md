...
> **版本**：v5.0｜**基於**：Superpowers Framework v6.0.3（by Jesse Vincent / obra @ [Prime Radiant](https://primeradiant.com/)）  
> **最後更新**：2026-06-30｜**適用對象**：資深工程師、Tech Lead、AI Agent 團隊｜**語言**：繁體中文  
> **GitHub**：https://github.com/obra/superpowers ⭐ 242k stars · 21.5k forks · 43 Contributors  
...
**Superpowers** 是由 Jesse Vincent（GitHub 帳號 [obra](https://github.com/obra)）與 [Prime Radiant](https://primeradiant.com/) 團隊開發的開源 **完整軟體開發方法論（Complete Software Development Methodology）**，目前是 GitHub 上最受歡迎的 AI 開發方法論專案（⭐ 242k stars、21.5k forks、43 Contributors）。
...
- ❌ 不是單一 IDE 外掛（支援 Claude Code、Cursor、Codex CLI、Codex App、Factory Droid、OpenCode、Gemini CLI、GitHub Copilot CLI、**Antigravity**、**Kimi Code**、**Pi** 等 **11 個平台**）
...
Superpowers v6.0.3 支援 **11 個** AI 編碼平台（Harness）：
...
| **Antigravity** | `agy plugin install` | ⭐⭐⭐⭐⭐ | ✅ 完整 | **v6.0.0 起支援（新增）** |
...
> **章節摘要**：本章說明 Superpowers v6.0.3 的 Plugin 架構、技能自動觸發機制、Hooks 系統、多平台支援策略，以及與 CI/CD 系統的整合模式。

## 2.1 Superpowers 在 AI 開發架構中的位置

Superpowers 位於「AI Agent 的行為規範層」，介於底層 LLM 與上層應用邏輯之間。v5.0 起轉為 **Plugin 架構**，透過各平台的 Plugin Marketplace 安裝，不再需要手動複製檔案。v6.0.0 新增 Antigravity、Kimi Code、Pi 支援，總共 **11 個平台**：
...
graph TB
    ...
    APP --> IDE
    IDE --> CC & CU & AGY & CDX & CDXA & FD & KIMI & OC & GC & GCLI & PI
    CC & CU & AGY & CDX & CDXA & FD & KIMI & OC & GC & GCLI & PI --> SP_HOOK
    SP_HOOK --> SP_CORE
...
1. **偵測平台**：自動辨識 Claude Code / Cursor / Antigravity / Codex CLI / Codex App / Factory Droid / Kimi Code / OpenCode / Gemini CLI / Copilot CLI / Pi
...
> **章節摘要**：本章詳述 Superpowers v6.0.3 的安裝方式（以 Plugin Marketplace 為主）、各平台安裝指南、專案初始化流程，以及升級策略。
...
```bash
# 安裝指定版本
/plugin install superpowers@6.0.3
...
> **章節摘要**：本章深入介紹 Superpowers v6.0.3 的 14+ 個內建技能。按照 The Basic Workflow（7 步驟）的順序組織，並涵蓋每個技能的觸發條件、執行流程、Prompt 範例與注意事項。
...
| Superpowers 版本 | Claude Code | Cursor | Codex CLI | Codex App | Factory Droid | OpenCode | Gemini CLI | Copilot CLI | Antigravity | Kimi Code | Pi |
|-----------------|-------------|--------|-----------|-----------|---------------|----------|------------|-------------|-------------|-----------|-----|
| **v3.x** | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **v4.0–4.2** | ✅ | ⚠️ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **v4.3.x** | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
| 功能 | Claude Code | Cursor | Codex CLI | Codex App | Factory Droid | OpenCode | Gemini CLI | Copilot CLI | Antigravity | Kimi Code | Pi |
|------|-------------|--------|-----------|-----------|---------------|----------|------------|-------------|-------------|-----------|-----|
| **Basic Workflow** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
> **章節摘要**：本章彙整 Superpowers v6.0.3 的核心最佳實務，涵蓋 AI 使用規範、開發流程、架構設計、Subagent 協作、Task Review、Inline Self-Review 等面向。
...
> **章節摘要**：本章系統化列出使用 Superpowers v6.0.3 時最常見的反模式，涵蓋開發、流程、Subagent 協作與組織層面，並提供改正方法。每個反模式都包含具體的錯誤範例與正確做法對照。另可參考官方 `testing-anti-patterns.md`。
...
> **章節摘要**：本章提供一個完整的範例專案，從專案架構到程式碼、測試、開發流程示範，展示 Superpowers v6.0.3 的 7-Step Basic Workflow 在實際專案中的全面應用（含 Subagent-Driven Development、Task Review 與 Inline Self-Review）。
...
> 💡 **提示**：這個範例專案展示了 Superpowers v6.0.3 的 7-Step Basic Workflow 在實際專案中的協同運作。建議讀者根據此範例進行練習，並嘗試使用 Subagent-Driven Development 將 Domain / Application / Presentation 層拆分給不同 Subagent 平行開發。
...
> **用途**：提供 Superpowers v2.0.0 至 v6.0.3 的完整版本變更紀錄快速查閱表，方便企業進行版本評估與升級決策。
...
|---------|-----------|---------|---------|
| v2.x–v3.x | v6.0.3 | 🔴 高 | 需要完整遷移計畫（移除 `.superpowers/`、改用 Plugin、SDD 流程變更） |
| v4.0–v4.2 | v6.0.3 | 🔴 高 | 需處理 Visual Companion port、Inline Self-Review、SDD reviewer 合併 |
| v4.3.x | v6.0.3 | 🟡 中 | 主要需處理 SDD Two-Stage → Single Reviewer 變更 |
| v5.0.0–v5.0.5 | v6.0.3 | 🟡 中 | 需處理 Inline Self-Review、SDD 架構、worktree 路徑變更 |
| v5.0.6–v5.1.0 | v6.0.3 | 🟢 低 | 需注意 SDD reviewer 合併、worktree 路徑、vendor-neutral 用語 |
| v6.0.0–v6.0.2 | v6.0.3 | 🟢 低 | 僅 SDD 暫存目錄路徑變更，向下相容 |
...
> **版本資訊**  
> 本手冊基於 Superpowers Framework v6.0.3（by Jesse Vincent / obra at Prime Radiant）撰寫  
> GitHub：https://github.com/obra/superpowers（⭐ 242k+）  
> 手冊版本：v5.0  
> 最後更新：2026-06-20  
> 維護人員：[Eric Cheng]  
> 
