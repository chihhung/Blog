...
> **版本**：v3.1｜**基於**：Superpowers Framework v5.0.7（by Jesse Vincent / obra @ [Prime Radiant](https://primeradiant.com/)）  
> **最後更新**：2026-04-30｜**適用對象**：資深工程師、Tech Lead、AI Agent 團隊｜**語言**：繁體中文  
> **GitHub**：https://github.com/obra/superpowers ⭐ 174k stars · 15.3k forks · 28 Contributors  
...
**Superpowers** 是由 Jesse Vincent（GitHub 帳號 [obra](https://github.com/obra)）與 [Prime Radiant](https://primeradiant.com/) 團隊開發的開源 **完整軟體開發方法論（Complete Software Development Methodology）**，目前是 GitHub 上最受歡迎的 AI 開發方法論專案（⭐ 174k stars、15.3k forks、28 Contributors）。
...
- ❌ 不是單一 IDE 外掛（支援 Claude Code、Cursor、Codex CLI、**Codex App**、OpenCode、Gemini CLI、**GitHub Copilot CLI** 等 7 個平台）
...
Superpowers v5.0 起支援多個 AI 編碼平台（共 7 個）：
...
| **Codex App（OpenAI）** | 側邊欄 Plugins → Coding | ⭐⭐⭐⭐ | ✅ named agent dispatch | v5.0.6 起支援（新增） |
...
graph TB
    ...
    APP --> IDE
    IDE --> CC & CU & CDX & CDXA & OC & GC & GCLI
    CC & CU & CDX & CDXA & OC & GC & GCLI --> SP_HOOK
    SP_HOOK --> SP_CORE
...
├── agents/                           # 代理定義
│   └── code-reviewer.md
├── commands/                         # 已棄用的 Slash Commands（保留相容）
├── assets/                           # 靜態資源（圖片等）
├── scripts/                          # 建構與安裝腳本
├── tests/                            # 測試套件
├── references/                       # 參考資料
...
├── RELEASE-NOTES.md                  # 版本發行紀錄（非 CHANGELOG.md）
├── CLAUDE.md                         # 貢獻者指南
...
1. **偵測平台**：自動辨識 Claude Code / Cursor / Codex CLI / Codex App / OpenCode / Gemini CLI / Copilot CLI
...
| Superpowers 版本 | Claude Code | Cursor | Codex CLI | Codex App | OpenCode | Gemini CLI | Copilot CLI |
|-----------------|-------------|--------|-----------|-----------|----------|------------|-------------|
| **v3.x** | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **v4.0–4.2** | ✅ | ⚠️ 部分 | ❌ | ❌ | ❌ | ❌ | ❌ |
| **v4.3.x** | ✅ | ✅ | ✅ (AGENTS.md) | ❌ | ❌ | ❌ | ❌ |
...
| 功能 | Claude Code | Cursor | Codex CLI | Codex App | OpenCode | Gemini CLI | Copilot CLI |
|------|-------------|--------|-----------|-----------|----------|------------|-------------|
| **Basic Workflow** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
> GitHub：https://github.com/obra/superpowers（⭐ 174k+）  
> 手冊版本：v3.1  
> 最後更新：2026-04-30  
> 維護人員：[Eric Cheng]  
> 
