+++
date = '2026-03-23T19:30:01+08:00'
draft = false
...
> **版本**：v2.0｜**基於**：Superpowers Framework v5.0.5（by Jesse Vincent / obra @ [Prime Radiant](https://primeradiant.com/)）  
> **最後更新**：2026-03-23｜**適用對象**：資深工程師、Tech Lead、AI Agent 團隊｜**語言**：繁體中文  
> **GitHub**：https://github.com/obra/superpowers ⭐ 107k stars · 8.6k forks  
> **Plugin Marketplace**：https://claude.com/plugins/superpowers  
> **Discord 社群**：https://discord.gg/Jd8Vphy9jq
...
**Superpowers** 是由 Jesse Vincent（GitHub 帳號 [obra](https://github.com/obra)）與 [Prime Radiant](https://primeradiant.com/) 團隊開發的開源 **AI Agent 技能框架（Agentic Skills Framework）**，目前是 GitHub 上最受歡迎的 AI 開發方法論專案之一（⭐ 107k stars、8.6k forks）。
...
- ❌ 不是單一 IDE 外掛（支援 Claude Code、Cursor、Codex、OpenCode、Gemini CLI）
...
> **章節摘要**：本章說明 Superpowers v5.0.5 的 Plugin 架構、技能自動觸發機制、Hooks 系統、多平台支援策略，以及與 CI/CD 系統的整合模式。
...
graph TB
    ...
    subgraph "行為規範層（Superpowers Plugin）"
        ...
        SP_DOC[Document Review<br/>System]
    end
...
graph LR
    ...
    SKILLS --> CC_MKT & CU_PLUG & CDX_AGENTS & OC_INST
    HOOKS --> CC_CLAUDE & CU_HOOK & CDX_AGENTS & OC_INST
    VIS --> CC_MKT & CU_PLUG
```
...
> **章節摘要**：本章詳述 Superpowers v5.0.5 的安裝方式（以 Plugin Marketplace 為主）、各平台安裝指南、專案初始化流程，以及升級策略。
...
參照 [INSTALL.md](https://github.com/obra/superpowers/blob/main/INSTALL.md) 中的 Codex 區段：
...
> **章節摘要**：本章深入介紹 Superpowers v5.0.5 的 15+ 個內建技能。按照 The Basic Workflow（7 步驟）的順序組織，並涵蓋每個技能的觸發條件、執行流程、Prompt 範例與注意事項。
...
| # | 技能名稱 | 類別 | 自動觸發 | 說明 |
|---|---------|------|---------|------|
| 1 | `brainstorming` | 需求 | ✅ | 蘇格拉底式提問 + Visual Companion |
| 2 | `using-git-worktrees` | 隔離 | ✅ | Git Worktree 自動建立與管理 |
| 3 | `writing-plans` | 規劃 | ✅ | 微步驟計畫 + Document Review |
...
| **Document Review** | Spec 經過 Review Loop（v5.0.0 起，最多 3 輪） |
...
graph TD
    ...
    I --> K[使用者回答]
    K --> G
    J --> L[Document Review Loop<br/>最多 3 輪]
    L --> M[需求確認 ✅]
...
> 💡 v5.0.4 將 Review Loop 精簡為最多 **3 輪**，避免過度審查。
...
> **章節摘要**：本章說明 Superpowers 框架本身的升級策略、Plugin Marketplace 版本管理，以及完整版本歷程（v2.0.0 → v5.0.5）。
...
| Superpowers 版本 | Claude Code | Cursor | Codex CLI | OpenCode | Gemini CLI |
|-----------------|-------------|--------|-----------|----------|------------|
| **v3.x** | ✅ | ❌ | ❌ | ❌ | ❌ |
| **v4.0–4.2** | ✅ | ⚠️ 部分 | ❌ | ❌ | ❌ |
| **v4.3.x** | ✅ | ✅ | ✅ (AGENTS.md) | ❌ | ❌ |
...
| 功能 | Claude Code | Cursor | Codex CLI | OpenCode | Gemini CLI |
|------|-------------|--------|-----------|----------|------------|
| **Basic Workflow** | ✅ | ✅ | ✅ | ✅ | ✅ |
...
| 版本 | 日期 | 類型 | 重點變更 |
|------|------|------|---------|
| **v5.0.5** | 2026-03-17 | Patch | ESM/CJS 雙模式相容修復；Windows `process.pid` 修復；依賴更新 |
| **v5.0.4** | 2026-03-10 | Minor | OpenCode 原生一行安裝；Document Review 3 輪上限；效能優化 |
| **v5.0.2** | 2026-02-28 | Minor | Context Isolation（Subagent 間隔離）；Visual Server 零依賴化 |
| **v5.0.0** | 2026-02-15 | **Major** | Visual Brainstorming Companion；Document Review System；Plugin Marketplace 上架 |
| **v4.3.1** | 2026-01-30 | Patch | Cursor 平台適配（`.cursorrules` 生成） |
| **v4.3.0** | 2026-01-20 | Minor | Brainstorming Hard Gates（強制執行門檻） |
| **v4.2.0** | 2026-01-05 | Minor | Git Worktree 強制化（不再可選）；`sp/` 前綴命名規範 |
| **v4.0.0** | 2025-12-15 | **Major** | Two-Stage Code Review；DOT/GraphViz 流程圖；Subagent-Driven Development |
| **v3.5.0** | 2025-11-20 | Minor | `testing-anti-patterns.md` 參考文件；Rationalization Detection 強化 |
| **v3.0.1** | 2025-11-01 | Patch | 技能定義精緻化；觸發條件細化 |
...
- **Document Review**：writing-plans 完成後強制進入 Review Loop，最多 3 輪
...
> **章節摘要**：本章彙整 Superpowers v5.0.5 的核心最佳實務，涵蓋 AI 使用規範、開發流程、架構設計、Subagent 協作、Document Review 等面向。
...
> **章節摘要**：本章系統化列出使用 Superpowers v5.0.5 時最常見的反模式，涵蓋開發、流程、Subagent 協作與組織層面，並提供改正方法。每個反模式都包含具體的錯誤範例與正確做法對照。另可參考官方 `testing-anti-patterns.md`。
...
> **章節摘要**：本章提供 Superpowers v5.0.5 在企業團隊中的導入路線圖，涵蓋 Plugin Marketplace 部署、多平台支援、試點策略、治理機制、Code Review 制度，以及 AI 使用規範。
...
> **章節摘要**：本章提供一個完整的範例專案，從專案架構到程式碼、測試、開發流程示範，展示 Superpowers v5.0.5 的 7-Step Basic Workflow 在實際專案中的全面應用（含 Subagent-Driven Development 與 Document Review）。
...
4. ✅ 包含架構測試（ArchUnit）
5. ✅ 計畫通過 Document Review 後才開始開發
6. ✅ 使用 `finishing-a-development-branch` 技能完成合併
7. ✅ 最後更新文件

> 💡 **提示**：這個範例專案展示了 Superpowers v5.0.5 的 7-Step Basic Workflow 在實際專案中的協同運作。建議讀者根據此範例進行練習，並嘗試使用 Subagent-Driven Development 將 Domain / Application / Presentation 層拆分給不同 Subagent 平行開發。
...
| **Document Review System** | 文件審查系統 | v5.0+ 計畫/規格 Review Loop（≤ 3 輪） |
...
> **用途**：提供 Superpowers v2.0.0 至 v5.0.5 的完整版本變更紀錄快速查閱表，方便企業進行版本評估與升級決策。
...
| v3.0.1 | 2025-11-01 | Patch | ~5k | 1 |
| v3.5.0 | 2025-11-20 | Minor | ~15k | 1 |
| v4.0.0 | 2025-12-15 | **Major** | ~35k | 1 |
| v4.2.0 | 2026-01-05 | Minor | ~50k | 1 |
| v4.3.0 | 2026-01-20 | Minor | ~60k | 1 |
| v4.3.1 | 2026-01-30 | Patch | ~65k | 2（+ Cursor） |
| v5.0.0 | 2026-02-15 | **Major** | ~80k | 5 |
| v5.0.2 | 2026-02-28 | Minor | ~90k | 5 |
| v5.0.4 | 2026-03-10 | Minor | ~100k | 5 |
| v5.0.5 | 2026-03-17 | Patch | **107k** | **5** |
...
### v3.0.1 — 技能精緻化（2025-11-01）
- 技能定義文件格式標準化（YAML frontmatter）
- 觸發條件細化與精準度提升
- 文件勘誤修正

### v3.5.0 — 測試反模式（2025-11-20）
- 新增 `testing-anti-patterns.md` 參考文件
- Rationalization Detection 強化（1% 規則）
- 技能間的依賴關係文件化

### v4.0.0 — Two-Stage Review & Subagent（2025-12-15）
...
|---------|-----------|---------|---------|
| v2.x–v3.x | v5.0.5 | 🔴 高 | 需要完整遷移計畫（移除 `.superpowers/`、改用 Plugin） |
| v4.0–v4.2 | v5.0.5 | 🟡 中 | 需處理 Visual Companion port、Document Review 流程 |
| v4.3.x | v5.0.5 | 🟢 低 | 主要是新增功能，Breaking Changes 少 |
| v5.0.0–v5.0.4 | v5.0.5 | 🟢 極低 | Patch 等級，平滑升級 |

---

> **版本資訊**  
> 本手冊基於 Superpowers Framework v5.0.5（by Jesse Vincent / obra at Prime Radiant）撰寫  
> GitHub：https://github.com/obra/superpowers（⭐ 107k+）  
> 手冊版本：v2.0  
> 最後更新：2026-03  
> 維護人員：[Eric Cheng]  
> 
