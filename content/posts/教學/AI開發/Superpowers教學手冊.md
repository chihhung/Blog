...
> **版本**：v4.0｜**基於**：Superpowers Framework v5.1.0（by Jesse Vincent / obra @ [Prime Radiant](https://primeradiant.com/)）  
> **最後更新**：2026-05-28｜**適用對象**：資深工程師、Tech Lead、AI Agent 團隊｜**語言**：繁體中文  
> **GitHub**：https://github.com/obra/superpowers ⭐ 213k stars · 19k forks · 32 Contributors  
> **Plugin Marketplace**：https://claude.com/plugins/superpowers  
...
**Superpowers** 是由 Jesse Vincent（GitHub 帳號 [obra](https://github.com/obra)）與 [Prime Radiant](https://primeradiant.com/) 團隊開發的開源 **完整軟體開發方法論（Complete Software Development Methodology）**，目前是 GitHub 上最受歡迎的 AI 開發方法論專案（⭐ 213k stars、19k forks、32 Contributors）。
...
- ❌ 不是單一 IDE 外掛（支援 Claude Code、Cursor、Codex CLI、Codex App、**Factory Droid**、OpenCode、Gemini CLI、GitHub Copilot CLI 等 **8 個平台**）
...
Superpowers v5.1.0 支援 **8 個** AI 編碼平台：
...
| **Codex CLI（OpenAI）** | `/plugins` 指令安裝 | ⭐⭐⭐⭐ | ✅ multi_agent | v3.3.0 起支援 |
| **Codex App（OpenAI）** | 側邊欄 Plugins → Coding | ⭐⭐⭐⭐ | ✅ named agent dispatch | v5.0.6 起支援 |
| **Factory Droid** | `droid plugin marketplace add` | ⭐⭐⭐⭐ | ✅ 完整 | **v5.1.0 起支援（新增）** |
...
> **章節摘要**：本章說明 Superpowers v5.1.0 的 Plugin 架構、技能自動觸發機制、Hooks 系統、多平台支援策略，以及與 CI/CD 系統的整合模式。

## 2.1 Superpowers 在 AI 開發架構中的位置

Superpowers 位於「AI Agent 的行為規範層」，介於底層 LLM 與上層應用邏輯之間。v5.0 起轉為 **Plugin 架構**，透過各平台的 Plugin Marketplace 安裝，不再需要手動複製檔案（v5.1.0 新增 Factory Droid 支援，總共 8 個平台）：
...
graph TB
    ...
    APP --> IDE
    IDE --> CC & CU & CDX & CDXA & FD & OC & GC & GCLI
    CC & CU & CDX & CDXA & FD & OC & GC & GCLI --> SP_HOOK
    SP_HOOK --> SP_CORE
...
│   │   ├── SKILL.md
│   │   └── code-reviewer.md           # v5.1.0：從 agents/ 移入，單一來源
│   ├── receiving-code-review.md
│   ├── using-git-worktrees.md         # v5.1.0：重寫（consent + 環境偵測）
...
├── scripts/                          # 建構與安裝腳本
│   └── sync-to-codex-plugin          # v5.1.0：Codex Plugin Mirror 同步工具
├── tests/                            # 測試套件
├── references/                       # 參考資料
...
> ⚠️ **v5.1.0 重大變更**：`agents/` 目錄已移除（`code-reviewer.md` 移入 `skills/requesting-code-review/`）；`commands/` 目錄已移除（legacy slash commands 完全刪除）；`CHANGELOG.md` 已移除（統一使用 `RELEASE-NOTES.md`）。
...
graph LR
    ...
    SKILLS --> CC_MKT & CU_PLUG & CDX_AGENTS & OC_INST & GCLI_MKT & FD_MKT
...
1. **偵測平台**：自動辨識 Claude Code / Cursor / Codex CLI / Codex App / Factory Droid / OpenCode / Gemini CLI / Copilot CLI
...
> **章節摘要**：本章詳述 Superpowers v5.1.0 的安裝方式（以 Plugin Marketplace 為主）、各平台安裝指南、專案初始化流程，以及升級策略。
...
> **章節摘要**：本章深入介紹 Superpowers v5.1.0 的 15+ 個內建技能。按照 The Basic Workflow（7 步驟）的順序組織，並涵蓋每個技能的觸發條件、執行流程、Prompt 範例與注意事項。

> 💡 **v5.1.0 重大變更**：
> - `superpowers:code-reviewer` named agent 已移除，合併至 `skills/requesting-code-review/code-reviewer.md`，改用 `Task (general-purpose)` dispatch。
> - `using-git-worktrees` 技能全面重寫：建立 worktree 前需征得使用者同意（consent-based）、新增環境偵測（`GIT_DIR` vs `GIT_COMMON_DIR`）、卷入源追蹤清理（provenance-based cleanup）、優先使用原生工具。
> - SDD 不再每 3 個任務強制暫停，改為連續執行模式。
...
| 7 | `requesting-code-review` | 審查 | ✅ | 向 Subagent 發起程式碼審查（v5.1.0：自包含，整合 code-reviewer） |
...
> **章節摘要**：本章說明 Superpowers 框架本身的升級策略、Plugin Marketplace 版本管理，以及完整版本歷程（v2.0.0 → v5.1.0）。
...
| Superpowers 版本 | Claude Code | Cursor | Codex CLI | Codex App | Factory Droid | OpenCode | Gemini CLI | Copilot CLI |
|-----------------|-------------|--------|-----------|-----------|---------------|----------|------------|-------------|
| **v3.x** | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **v4.0–4.2** | ✅ | ⚠️ 部分 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **v4.3.x** | ✅ | ✅ | ✅ (AGENTS.md) | ❌ | ❌ | ❌ | ❌ | ❌ |
...
| 功能 | Claude Code | Cursor | Codex CLI | Codex App | Factory Droid | OpenCode | Gemini CLI | Copilot CLI |
|------|-------------|--------|-----------|-----------|---------------|----------|------------|-------------|
| **Basic Workflow** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Subagent-Driven Development** | ✅ | ✅ | ✅ (multi_agent) | ✅ (named dispatch) | ✅ | ✅ | ❌ → fallback | ✅ |
| **Visual Brainstorming Companion** | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
> **章節摘要**：本章彙整 Superpowers v5.1.0 的核心最佳實務，涵蓋 AI 使用規範、開發流程、架構設計、Subagent 協作、Inline Self-Review 等面向。
...
> **章節摘要**：本章系統化列出使用 Superpowers v5.1.0 時最常見的反模式，涵蓋開發、流程、Subagent 協作與組織層面，並提供改正方法。每個反模式都包含具體的錯誤範例與正確做法對照。另可參考官方 `testing-anti-patterns.md`。
...
> **章節摘要**：本章提供 Superpowers v5.1.0 在企業團隊中的導入路線圖，涵蓋 Plugin Marketplace 部署、多平台支援、試點策略、治理機制、Code Review 制度，以及 AI 使用規範。
...
4. Q&A + 規範說明（20 min）
   - 回答問題
   - 說明平台選擇（Claude Code / Cursor / Codex CLI / Codex App / OpenCode / Gemini CLI / Copilot CLI）
   - 說明接下來的試點計畫
```
...
```markdown
# 團隊 AI 使用政策 v2.0（Superpowers v5.1.0）

## 1. 適用範圍
本政策適用於使用 AI 工具（Claude Code、Cursor、Codex CLI、Codex App、OpenCode、Gemini CLI、GitHub Copilot CLI 等）進行開發的所有團隊成員。
...
```markdown
## Superpowers Code Review Checklist（v5.1.0）
...
> **章節摘要**：本章提供一個完整的範例專案，從專案架構到程式碼、測試、開發流程示範，展示 Superpowers v5.1.0 的 7-Step Basic Workflow 在實際專案中的全面應用（含 Subagent-Driven Development 與 Inline Self-Review）。
...
> 💡 **提示**：這個範例專案展示了 Superpowers v5.1.0 的 7-Step Basic Workflow 在實際專案中的協同運作。建議讀者根據此範例進行練習，並嘗試使用 Subagent-Driven Development 將 Domain / Application / Presentation 層拆分給不同 Subagent 平行開發。
...
- [ ] Codex CLI + `/plugins` 指令安裝或 AGENTS.md 配置
...
| **Factory Droid** | Factory Droid 平台 | v5.1.0+ 支援的第八個平台，透過 `droid plugin marketplace add` 安裝 |
...
> **用途**：提供 Superpowers v2.0.0 至 v5.1.0 的完整版本變更紀錄快速查閱表，方便企業進行版本評估與升級決策。
...
|---------|-----------|---------|---------|
| v2.x–v3.x | v5.1.0 | 🔴 高 | 需要完整遷移計畫（移除 `.superpowers/`、改用 Plugin） |
| v4.0–v4.2 | v5.1.0 | 🟡 中 | 需處理 Visual Companion port、Inline Self-Review 流程 |
| v4.3.x | v5.1.0 | 🟢 低 | 主要是新增功能，Breaking Changes 少 |
| v5.0.0–v5.0.5 | v5.1.0 | 🟢 低 | v5.0.6 有 Inline Self-Review 重大變更，但向下相容 |
| v5.0.6 | v5.1.0 | 🟢 低 | 需注意 slash commands 移除與 code-reviewer 變更 |
| v5.0.7 | v5.1.0 | 🟢 低 | 主要新增 Factory Droid、worktree 重寫、SDD 連續執行 |
...
> **版本資訊**  
> 本手冊基於 Superpowers Framework v5.1.0（by Jesse Vincent / obra at Prime Radiant）撰寫  
> GitHub：https://github.com/obra/superpowers（⭐ 213k+）  
> 手冊版本：v4.0  
> 最後更新：2026-05-28  
> 維護人員：[Eric Cheng]  
> 
