...
> **定位**：企業標準技術白皮書  
> **開源倉庫**：[warpdotdev/warp](https://github.com/warpdotdev/warp)（62,700+ Stars）  
...
Warp 是新一代**代理開發環境（Agentic Development Environment, ADE）**，由 Warp 公司以 Rust 語言打造，於 2026 年 4 月 28 日正式開源（AGPL v3 / MIT 雙授權，其中 UI 框架 warpui_core / warpui 兩個 crate 採 MIT，其餘核心程式碼採 AGPL v3），OpenAI 為創始贊助商。截至 2026 年 7 月，GitHub 已取得 **62,700+ Stars**，專案語言組成以 Rust（98.3%）為主體，開源後採「Agent-first」貢獻模式——人類貢獻者的角色從手寫程式碼轉為撰寫規格（Spec）、審查 AI 產出與調度 Agent 艦隊，此模式由 Oz 平台與 GPT 系列模型驅動。
...
| 模型 | model_id | 備註 |
|------|----------|------|
| Claude Sonnet 5 | `claude-5-sonnet-xhigh` / `high` / `low` | 最新旗艦，三種推理強度可選 |
| Claude Opus 4.8 | `claude-4-8-opus-xhigh` / `high` / `max` | 深度推理，適合架構決策 |
| Claude Opus 4.7 | `claude-4-7-opus-xhigh` / `high` / `max` | 高精度推理 |
...
| 模型 | model_id | 備註 |
|------|----------|------|
| Gemini 3.1 Pro | `gemini-3.1-pro` | 大型程式碼庫分析、長上下文 |
| Gemini 3.5 Flash | `gemini-3.5-flash` | 低延遲、輕量任務 |

##### xAI 模型

| 模型 | model_id | 備註 |
|------|----------|------|
| Grok 4.3 | `grok-4.3-high` / `medium` / `low` | 三種推理強度可選 |
| Grok Build 0.1 | `grok-build-0.1` | 程式碼建置導向 |

> xAI 模型不消耗 Warp Credits，須連結個人 **SuperGrok** 訂閱後使用。

##### 開源模型（透過 Fireworks AI 託管）

| 模型 | model_id |
|------|----------|
| GLM 5.2 / 5.1 | `glm-5.2-fireworks` / `glm-5.1-fireworks` |
| Kimi K2.7 Code / K2.6 | `kimi-k27-code-fireworks` / `kimi-k26-fireworks` |
| Minimax 3 / 2.7 | `minimax-3-fireworks` / `minimax-2.7-fireworks` |
| Qwen 3.7 Plus / 3.6 Plus | `qwen-3.7-plus-fireworks` / `qwen-3.6-plus-fireworks` |
...
| **Custom Model Routers**（2026 新增） | 依複雜度（Complexity-based）或自訂規則（Rule-based）將任務路由至指定模型 | Settings > Agents > Model Routers 設定路由規則 |
| **Prompt Queueing**（2026 新增） | Agent 執行中可預先排入後續指令，執行完畢自動接續 | 輸入 `/queue` 或切換排隊模式 Chip |
| **Local-to-Cloud Handoff**（2026 新增） | 本機 Agent 對話可無縫轉移至雲端繼續執行 | macOS 休眠時自動觸發，亦可手動交接 |
| **Git Operations on Remote Sessions**（2026 新增） | 透過 SSH 遠端 Session 執行分支切換、commit、push | 於 SSH Extension 連線的遠端環境中直接操作 |

> **2026 年上半年重點更新**：除上表新增能力外，Warp 亦推出 **Queued Terminal Commands**（終端機指令可排隊循序執行，不限於 Agent Prompt）與 **Git Branch Status Chip**（輸入列常駐顯示目前分支與 ahead/behind commit 數）。這些功能大幅減少多步驟操作時的等待與切換成本，建議團隊在導入時一併納入教育訓練範圍。
...
| **SSH 遠端** | 透過 [SSH Extension](https://docs.warp.dev/terminal/warpify/ssh/) 在遠端主機使用 File Tree、Code Diff，並可執行 Git 分支切換、commit、push 等操作 |

> **重要變更**：早期實驗性的 tmux-based SSH warpify 流程已於 2026 年停用，現行遠端連線一律改用專屬 **SSH Extension**。若團隊文件或 Workflow 中仍引用舊版 tmux 流程，應盡快改用 SSH Extension 以取得完整的遠端 Git 操作與 Code Editor 支援。
...
> **方案限制**：Self-Hosting 屬於 **Enterprise 方案**專屬功能，Free / Build / Max / Business 方案皆不支援將 Cloud Agents 部署於企業自有基礎設施，僅能使用 Warp 官方託管的 Cloud Agents。規劃自託管前應先確認組織的合約方案（詳見[附錄 D](#附錄-dwarp-方案與計費)）。
...
> **BYOK 金鑰管理提醒**：若團隊使用 BYOK（Bring Your Own API Key）串接 Anthropic / OpenAI / Google 等供應商金鑰，應比照 Level 3-4 機密等級管理。務必注意 **BYOK 僅適用於 Local Agent**，金鑰只存放於本機裝置的作業系統安全儲存區，**Cloud Agent 執行時無法取用個人 BYOK 金鑰**，一律改用 Warp Credits 計費（詳見[附錄 D](#附錄-dwarp-方案與計費)）。切勿將 BYOK 金鑰寫入 Cloud Agent 的 Environments / Secrets 設定中，以免誤解其適用範圍。
...
請依 OWASP Top 10:2025（2026 年 1 月正式發布版本）檢查：
1. A01:2025 - Broken Access Control
2. A02:2025 - Security Misconfiguration
3. A03:2025 - Software Supply Chain Failures
4. A04:2025 - Cryptographic Failures
5. A05:2025 - Injection（SQL / XSS / Command）
...
> **OWASP Top 10:2025 重點異動**：相較 2021 版，新增「A03 Software Supply Chain Failures（軟體供應鏈失效）」與「A10 Mishandling of Exceptional Conditions（例外情境處理失當）」兩個全新類別；原本獨立的 SSRF 已併入 A01 Broken Access Control；Security Misconfiguration 由第 5 名躍升至第 2 名。建議團隊將 [11.2 節的相依套件掃描](#112-dependency-scan-相依套件掃描)與 A03 供應鏈風險對應，強化 CI/CD 中的 SBOM（軟體物料清單）盤點。
...
☐ 熟悉 Prompt Queueing（/queue）與 Custom Model Routers 設定
```
...
| 清除畫面 | `Cmd+K` | `Ctrl+K` |
| 指令歷史 | `Cmd+R` | `Ctrl+R` |
| Prompt 排隊（Agent 執行中） | `/queue` 指令 | `/queue` 指令 |
...
> 本節依 [warp.dev/pricing](https://www.warp.dev/pricing) 2026 年 7 月現況整理，Warp 已由早期「Build（免費）/ Max / Business」三方案調整為現行 **五方案**架構，企業導入前應以官網公告為準。

### 方案比較

| 項目 | **Free** | **Build** | **Max** | **Business** | **Enterprise** |
|------|:--------:|:---------:|:-------:|:-------------:|:---------------:|
| **月費** | $0 | $20（年繳 $18） | $200（年繳 $180） | $50/user（年繳 $45/user，上限 25 席） | 客製報價 |
| **月付 Credits** | 有限（新用戶前 2 月 150、之後 75） | 1,500/月 | 18,000/月（Build 的 12 倍） | 每人 1,500/月 | 自訂共享額度 |
| **Local Agent（Warp 內建）** | ✅ 基礎用量 | ✅ 全模型 | ✅ 全模型 | ✅ 團隊用量 | ✅ 無限制 |
| **CLI Agent 工具帶** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **BYOK（自帶 API Key，限 Local Agent）** | ✅ | ✅ | ✅ | ✅（限 10 人以下組織個人使用） | ✅ |
| **Cloud Agents** | 有限使用 | ✅ | ✅ | ✅ | ✅ |
| **Oz Platform（CLI/API/SDK/Web）** | 有限 | ✅ | ✅ | ✅ | ✅ |
| **Self-Hosting（Cloud Agents 自託管）** | ❌ | ❌ | ❌ | ❌ | ✅ |
| **Team Admin Panel / 使用量儀表板** | ❌ | 個人限定 | 個人限定 | ✅ | ✅ |
| **Warp Drive 共享** | 有限 | ✅ 無限（個人/團隊） | ✅ 無限 | ✅ 組織 | ✅ 組織 |
| **Session Sharing** | ❌ | ✅ | ✅ | ✅ | ✅ |
| **SSO / SAML** | ❌ | ❌ | ❌ | ✅ | ✅ |
| **Zero Data Retention（強制）** | — | — | — | ✅ 可強制啟用 | ✅ 可強制啟用 |
| **Audit Log** | ❌ | ❌ | ❌ | 有限 | ✅ 完整 |
| **Bring Your Own LLM（自架模型）** | ❌ | ❌ | ❌ | ❌ | ✅ |
| **技術支援** | 社群 | Email 支援 | Email 支援 | Email 支援 | 專屬客戶經理 + 共用 Slack 頻道 |

### Credits 制度

Warp 使用 **Credits** 做為 AI 用量計量單位，Cloud Agent 執行一律消耗 Credits；Local Agent 若採 BYOK 則不消耗 Credits，改由使用者自行支付對應供應商的 API 費用。不同模型消耗的 Credits 量級不同：
...
| Claude Sonnet 5 / Opus 4.8 Max | 極高 | 最新旗艦，最高精度推理 |
| Grok 4.3 / Grok Build 0.1 | 不消耗 Credits | 需連結個人 SuperGrok 訂閱 |
...
> **BYOK 重要澄清**：Warp **支援 BYOK（Bring Your Own API Key）**，可串接 Anthropic / OpenAI / Google 三家供應商金鑰，個人使用者與 10 人以下組織可直接設定；超過 10 人的組織須升級至 **Business 或 Enterprise** 方案才能為成員開通 BYOK。但 BYOK **僅適用於 Local Agent**——金鑰只存放於本機作業系統的安全儲存區，傳輸過程僅「in-flight」使用後即丟棄，不落地於 Warp 伺服器；**Cloud Agent 執行無法讀取本機金鑰，一律消耗 Warp Credits 計費**。此為原文件先前版本敘述錯誤之處，企業規劃預算與金鑰治理時務必以此區分為準。

> **企業建議**：Business 方案適合 25 人以下、需要 SSO 與集中帳務的團隊；若組織規模超過 25 席、需要 Self-Hosting、BYO LLM 或完整 Audit Log，須採購 **Enterprise** 方案並洽談客製報價。建議先以 Build 或 Max 方案試行，確認效益後再依團隊規模升級至 Business / Enterprise。
