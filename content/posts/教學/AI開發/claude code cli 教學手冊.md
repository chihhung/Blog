...
- [第 4 章 Authentication](#第-4-章-authentication)
  ...
  - [4.5 企業雲端供應商登入](#45-企業雲端供應商登入)
    - [4.5.1 Claude apps gateway：讓第三方雲端也能拿到集中治理](#451-claude-apps-gateway讓第三方雲端也能拿到集中治理)
...
- [第 6 章 CLI 指令大全](#第-6-章-cli-指令大全)
  ...
  - [6.6 權限模式與 Auto Mode（自動權限分類器）](#66-權限模式與-auto-mode自動權限分類器)
    - [6.6.1 Auto Mode 分類器運作方式](#661-auto-mode-分類器運作方式)
    - [6.6.2 啟用條件與企業治理](#662-啟用條件與企業治理)
...
- [第 11 章 CLAUDE.md](#第-11-章-claudemd)
  ...
  - [11.4 維護方式](#114-維護方式)
  - [11.5 Auto Memory（自動記憶）：Claude 自己寫的筆記](#115-auto-memory自動記憶claude-自己寫的筆記)
  - [11.6 `.claude/rules/`：路徑範圍化的模組化規則](#116-clauderules路徑範圍化的模組化規則)
...
- [第 28 章 企業治理進階](#第-28-章-企業治理進階)
  ...
  - [28.3 Network Config 與 LLM Gateway](#283-network-config-與-llm-gateway)
    - [28.3.1 組織層級模型治理（Enterprise）](#2831-組織層級模型治理enterprise)
...
> ⚠️ **注意**：部分周邊功能（Claude Code Web、Routines、GitHub Code Review、Remote Control、Chrome 擴充套件）需要 claude.ai 帳號，單靠 Console API Key 或雲端供應商憑證（Bedrock/Vertex/Foundry）無法使用；企業若僅部署第三方雲端供應商，導入前應規劃工程師是否也需要額外的 Claude for Teams/Enterprise 席位（第4.5章）。
>
...
claude auth login --email you@co   # 登入時預先帶入信箱
claude auth login --sso            # 強制以 SSO 方式登入
claude auth login --console        # 改用 Anthropic Console API Key 計費登入（非訂閱制）
claude auth status                 # 確認登入狀態，預設輸出 JSON；exit code 0=已登入、1=未登入
claude auth status --text          # 以人類可讀格式輸出登入狀態
claude auth logout                 # 登出
claude setup-token                 # 產生長效 OAuth Token，僅印在終端機、不落地儲存（適合 CI/headless 環境，需 Claude 訂閱）
```

> **📌 實務建議**：`claude auth status` 的輸出格式（預設 JSON）很適合寫進健康檢查腳本，用 exit code 判斷是否已登入即可，不需要額外解析文字。
...
export ANTHROPIC_MODEL="claude-sonnet-5"    # 覆寫預設模型（亦可用別名 sonnet/opus/haiku/fable）
```
...
>
> ⚠️ **注意**：透過第三方雲端供應商登入時，部分功能（如特定模型版本上線時間、Auto Mode 等新功能的開放節奏）可能落後於原生 Anthropic 帳號，企業導入前應先確認所需功能在所選供應商上是否已開放。舉例來說，Auto Mode 在 Bedrock／Vertex／Foundry 上預設關閉，且僅支援 Claude Sonnet 5、Opus 4.7、Opus 4.8，需要另外設定 `CLAUDE_CODE_ENABLE_AUTO_MODE=1`（詳見第6.6章）。

### 4.5.1 Claude apps gateway：讓第三方雲端也能拿到集中治理

企業若透過 Bedrock／Vertex／Foundry 登入，原生就無法使用 claude.ai 管理後台的 Server-managed 設定推送（第5.5章），也缺乏依使用者拆分的用量歸屬與花費上限。**Claude apps gateway** 是官方提供、可自架的閘道服務（`claude gateway --config gateway.yaml`，需 v2.1.195 以上），讓部署在第三方雲端的 Claude Code 也能：

- 透過閘道取得與原生 Anthropic 帳號相同的 Server-managed 設定交付（第5.5章）
- 記錄具備 IdP 身分資訊的逐請求稽核紀錄（Request-level Audit Log）
- 設定依使用者的花費上限（Per-user Spend Limits）

> **📌 實務建議**：同時混用多種雲端供應商登入方式的企業，應優先評估部署 Claude apps gateway，讓治理與稽核機制在所有供應商間保持一致，避免「原生帳號有完整治理、第三方雲端卻是治理死角」的落差。
...
- **`policyHelper`**：企業可設定一個由本機執行的輔助程式，動態運算出 Managed 設定內容。一旦設定了 `policyHelper`，它的輸出會**取代**上述四種來源、成為唯一生效的 Managed 設定，優先序凌駕在 Server-managed 之上。
- **跨來源鎖定鍵（Cross-source Lock Keys）**：極少數安全關鍵鍵值（例如 Sandbox 網域白名單鎖定）採「任一 admin 層級來源設定即生效」的邏輯，不受上述四層優先序限制，用意是避免某一層交付管道被繞過就整組防護失效。

另外，多數陣列型設定（如 `permissions.allow` / `permissions.deny`）在不同層級間是**合併（merge）而非取代**——開發者可以在專案/本機層級「增補」規則，但無法移除 Managed 層級已宣告的規則；`fallbackModel` 與 `availableModels` 是**例外**，較高層級會直接**取代**較低層級的值，而非合併陣列內容。

**Server-managed** 設定會在登入時送達裝置，並在 Session 執行期間**每小時自動刷新**一次，不需要額外的端點基礎建設。若企業混用多種雲端供應商（部分走原生 Anthropic 帳號、部分走 Bedrock/Vertex/Foundry），可搭配第4.5.1章的 **Claude apps gateway** 讓第三方雲端供應商也能取得 Server-managed 交付；若不部署 Gateway，則第三方雲端供應商只能透過檔案型或平台原生政策交付 Managed 設定。
...
| `cat file \| claude -p "query"` | 處理管線輸入內容 | `cat error.log \| claude -p "解釋這個錯誤"` |
| `claude -c` | 繼續目前目錄最近一次對話 | `claude -c` |
| `claude -c -p "query"` | 以非互動模式繼續對話 | `claude -c -p "檢查是否有型別錯誤"` |
| `claude -r "<session>" "query"` | 依 ID/名稱回復對話 | `claude -r "auth-refactor" "繼續完成這個 PR"` |
| `claude update` | 更新 CLI 本身至最新版本 | `claude update` |
| `claude install [version]` | 安裝／重新安裝原生執行檔，可指定版本號、`stable` 或 `latest` | `claude install stable` |

> 若指令打錯字，Claude Code 會直接提示最接近的正確指令並結束（例如 `claude udpate` 會印出 `Did you mean claude update?`），不會誤啟動一個以錯字為提示詞的 Session。

## 6.3 Agent 管理

| 指令 | 說明 |
|---|---|
| `claude agents` | 開啟 Agent View，監看與派發背景 Session；常用旗標 `--cwd`（只顯示該目錄啟動的 Session）、`--json`（以 JSON 輸出，加 `--all` 含已結束的）、`--permission-mode`/`--model`/`--effort`/`--agent`（設定派發 Session 的預設值） |
| `claude attach <id>` | 附著到背景執行的 Session |
| `claude logs <id>` | 印出背景 Session 的最新輸出 |
| `claude stop <id>` | 停止背景 Session（別名 `claude kill <id>`） |
| `claude respawn <id>` | 重新啟動背景 Session；加 `--all` 可重啟全部執行中 Session（例如更新 CLI 後套用新版本） |
| `claude rm <id>` | 從清單移除 Session 記錄（對話紀錄仍留在本機，可用 `--resume` 找回） |
| `claude daemon status` | 印出背景 Session 監督程序（Supervisor）狀態、版本、worker 數量，用於診斷 |
| `claude daemon stop --any [--keep-workers]` | 停止 Supervisor；加 `--keep-workers` 只停 Supervisor、保留背景 Session 繼續執行 |
...
| `claude mcp login <name>` | 免開互動式 `/mcp` 面板，直接在終端機完成該 MCP Server 的 OAuth 登入；SSH 遠端可加 `--no-browser` 改印授權網址 |
...
`claude --help` 不會列出全部旗標，未出現在 `--help` 不代表不存在，仍以官方 CLI Reference 為準。以下依用途分類整理：

### Session 與輸出控制

| 旗標 | 說明 |
|---|---|
| `--continue` / `-c` | 繼續目前目錄最近一次對話 |
| `--resume` / `-r` | 依 ID 或名稱回復對話，或開啟互動式選單挑選（背景 Session 會標示 `bg`） |
| `--fork-session` | 搭配 `--resume`／`--continue` 使用，建立新 Session ID 而非沿用原 ID |
| `--name` / `-n` | 設定 Session 顯示名稱，出現在 `--resume` 清單與終端機標題 |
| `--session-id <uuid>` | 指定特定 UUID 作為 Session ID |
| `--from-pr <PR>` | 回復連結到某個 PR 的 Session（接受 PR 編號或 GitHub/GitLab/Bitbucket PR 網址） |
| `--output-format [text\|json\|stream-json]` | 輸出格式（適合腳本解析） |
| `--input-format [text\|stream-json]` | print 模式下的輸入格式 |
| `--json-schema <schema>` | 完成後輸出符合指定 JSON Schema 的驗證結果（僅 print 模式） |
| `--verbose` | 顯示完整逐輪過程 |
| `--bare` | 最小模式：略過 Hooks／Skills／Plugins／MCP／Auto Memory／CLAUDE.md 的自動探索，只保留 Bash/Read/Edit 工具，啟動更快，適合大量腳本化呼叫 |
| `--safe-mode` | 停用所有客製化（CLAUDE.md、Skills、Plugins、Hooks、MCP、自訂指令與 Agent、輸出風格、Workflows、自訂主題／鍵盤綁定等），但認證、模型選擇、內建工具與權限機制照常運作，Managed 政策仍會套用；用於問題排查，與追求啟動速度的 `--bare` 用途不同 |

### Agent 與 Subagent

| 旗標 | 說明 |
|---|---|
| `--agent <name>` | 指定某個 Subagent 定義接管整個主 Session（覆寫 `agent` 設定） |
| `--agents '<json>'` | 以 JSON 動態定義當次 Session 專屬的 Subagent（詳見第10.1章） |
...
| `--permission-prompt-tool` | 指定一個 MCP 工具接手非互動模式下的權限詢問 |

### MCP／Plugin

| 旗標 | 說明 |
|---|---|
| `--mcp-config <file>` | 載入指定 JSON 檔案／字串中的 MCP 設定 |
| `--strict-mcp-config` | 只使用 `--mcp-config` 指定的 MCP Server，忽略其他來源 |
| `--plugin-dir <path>` | 從本機目錄或 `.zip` 載入 Plugin，僅限本次 Session |
| `--plugin-url <url>` | 從 URL 抓取 `.zip` Plugin 封裝，僅限本次 Session |

### 系統提示（System Prompt）

| 旗標 | 行為 | 適用時機 |
|---|---|---|
| `--system-prompt <text>` | 完全取代預設系統提示 | 非「編碼助理」定位的自訂 Agent，且無人值守審查 |
...
| `--model <name>` | 指定模型，可用別名 `sonnet`/`opus`/`haiku`/`fable` 或完整模型 ID |
| `--fallback-model <list>` | 主要模型過載或下架時依序嘗試的備援模型清單（逗號分隔） |
...
| `--advisor <model>` | 為本次 Session 啟用伺服器端 Advisor 工具，指定模型別名 |
...
# 大量腳本化呼叫追求啟動速度，略過自動探索
claude --bare -p "回傳這段輸入的 JSON schema"
```
...
>
> ⚠️ **注意**：`--permission-mode bypassPermissions` 在自動化環境中很方便，但等同於關閉安全閥，務必搭配第19章的 SSDLC 控管與最小權限原則使用；企業可用 `permissions.disableBypassPermissionsMode` 強制關閉此模式。
...
| 模式 | 免詢問即可執行的動作 | 適用情境 |
|---|---|---|
| `default` | 僅讀取類動作 | 剛上手、處理敏感程式碼 |
| `acceptEdits` | 讀取＋檔案編輯＋常見檔案系統指令（`mkdir`/`touch`/`rm`/`mv`/`cp`/`sed` 等，僅限工作目錄或 `additionalDirectories` 範圍內） | 正在盯著編輯器／`git diff` 逐一複核變更，不想每次編輯都按一次 |
| `plan` | 僅讀取類動作 | 修改前先研究、規劃 |
| `auto`（Auto Mode） | 幾乎所有動作，改由背景分類器即時審核 | 長任務、想大幅減少權限詢問次數 |
| `dontAsk` | 只執行符合 `permissions.allow` 規則與唯讀 Bash 指令的動作，其餘一律拒絕（不詢問） | 已完全預先定義好允許清單的 CI／受限腳本環境 |
| `bypassPermissions` | 所有動作皆免詢問 | 僅限容器、VM 等與主機隔離的環境 |

無論哪個模式，`permissions.deny` 與明確的 `ask` 規則永遠生效（包含 `bypassPermissions`）；`permissions.allow` 規則在 `bypassPermissions` 下沒有作用，因為所有動作本來就已放行。另外有一組**受保護路徑（Protected Paths）**——`.git`、`.claude`（除了 `.claude/worktrees`）、`.vscode`、`.idea`、`.husky`、`.cargo`、`.devcontainer`、`.yarn`、`.mvn` 等目錄，以及 `.gitconfig`、Shell 設定檔、套件管理器設定檔等——除了 `bypassPermissions` 之外，任何模式對這些路徑的寫入都不會被自動核准（`auto` 模式下改為交由分類器審核、`dontAsk` 模式下直接拒絕）。

```bash
claude --permission-mode plan          # 啟動時指定模式
claude --permission-mode auto          # 啟動時直接進入 Auto Mode（需帳號/模型/供應商符合條件）
claude auto-mode defaults              # 印出分類器內建規則
claude auto-mode config                # 印出套用目前設定後的實際生效規則
```

### 6.6.1 Auto Mode 分類器運作方式
...
| `Shift + Tab` | 循環切換權限模式（`default`→`acceptEdits`→`plan`，視啟用情況再接續 `bypassPermissions`／`auto`，詳見第6.6章） |
| `Ctrl + G` | 在 Plan 模式下，把目前提議的計畫開啟到預設文字編輯器中直接編修 |
| `Ctrl + B` | 把目前執行中的任務轉為背景執行，不中斷、可繼續在前景輸入新指令 |
...
graph TB
    ...
    Claude["CLAUDE.md + Auto Memory"] --> Window
    Hist["對話歷史"] --> Window
    Files["目前檔案內容"] --> Window
    Tools["工具呼叫結果"] --> Window
...
> Auto Memory（Claude 自己在對話中累積、寫入 `MEMORY.md` 的學習筆記）是 CLAUDE.md 之外另一個會載入 Context 的機制，兩者互補但性質不同：CLAUDE.md 是「你寫給 Claude 的規則」，Auto Memory 是「Claude 自己記下的經驗」。完整機制與設定方式詳見第11章。
...
專案根目錄的 CLAUDE.md 在壓縮後**會被重新從磁碟讀取並注入**對話，因此其中的規則不會因為壓縮而消失；但子目錄中的 CLAUDE.md（隨 Claude 讀取該子目錄檔案時才載入）不會自動重新注入，要等到 Claude 下次讀取該子目錄的檔案才會再次載入。這也是「重要規則優先寫進根目錄 CLAUDE.md」的另一個理由。
...
| 機制 | 這能帶給你什麼 | 何時選用 | 詳見 |
|---|---|---|---|
| Subagent | 單一 Session 內派生的委派工作者，在自己的 Context 中完成側支任務，回傳摘要給主 Agent | 側支任務會用大量搜尋結果、日誌或檔案內容淹沒主對話，且事後不需要再參考細節 | 本章 9.2-9.4、第10章 |
| Agent View（研究預覽） | 一個畫面集中派發／監看多個在背景執行的獨立 Session，以 `claude agents` 開啟 | 手上有多個彼此獨立的任務，想直接交辦、之後再回來檢視狀態，只在需要時介入 | 9.5 |
...
claude agents --json --all  # 以 JSON 輸出所有 Session（含已結束的），適合寫腳本整合
claude --bg -p "..."        # 以背景任務啟動一個新 Session，自動納入 Agent View
```

> Agent View 依賴背景常駐程序（`claude daemon`）運作，可用 `claude daemon status` 檢查其狀態；企業可用 `disableAgentView` 設定鍵整體關閉此功能（同時停用 `--bg`、`/background`）。
>
> `/tasks` 與 `claude agents` 是不同層級的指令：`/tasks` 列出「目前這個 Session 內」正在背景執行的項目（如背景 Subagent、背景 Bash 指令），`claude agents` 則是跨越多個「完全獨立的 Session」的 Agent View。簡言之，`/tasks` 看的是單一 Session 裡面發生了什麼，`claude agents` 看的是有幾個 Session 在跑。
>
...
子 Agent 是一個 Markdown 檔案，搭配 YAML Frontmatter 描述其名稱、說明、可用工具、模型等，本體則是系統提示：

```markdown
---
name: test-writer
description: 針對指定類別/函式撰寫單元測試。修改程式碼後應主動使用。
...
只有 `name` 與 `description` 為必填，其餘欄位皆有預設值。完整支援欄位如下：

| 欄位 | 說明 |
|---|---|
| `name` | 唯一識別碼，僅限小寫字母與連字號；`Hooks` 會以此值作為 `agent_type` |
| `description` | Claude 何時該委派給這個子 Agent；寫得越具體，自動匹配的命中率越高 |
| `tools` | 允許使用的工具清單，省略則繼承主 Session 全部工具 |
| `disallowedTools` | 要從繼承或指定清單中移除的工具 |
| `model` | `sonnet`／`opus`／`haiku`／`fable`、完整模型 ID，或 `inherit`（預設值，沿用主對話模型） |
| `permissionMode` | `default`／`acceptEdits`／`auto`／`dontAsk`／`bypassPermissions`／`plan`；Plugin 提供的子 Agent會忽略此欄位 |
| `maxTurns` | 子 Agent 自動停止前的最大輪數 |
| `skills` | 啟動時預先載入完整內容的 Skill 清單（不只是 description），用於灌入領域知識 |
...
```mermaid
flowchart LR
    Managed["Managed 設定內的 .claude/agents/<br/>(組織層級，最高優先)"] --> Merge["合併註冊表"]
    CliFlag["--agents 旗標<br/>(當次 Session)"] --> Merge
    ProjectDir[".claude/agents/<br/>(專案層級)"] --> Merge
    UserDir["~/.claude/agents/<br/>(使用者層級)"] --> Merge
    PluginDir["Plugin 的 agents/ 目錄<br/>(最低優先)"] --> Merge
    Merge --> Registry["可用子 Agent 清單<br/>(同名以較高優先層級為準)"]
...
**自 v2.1.198 起，`/agents` 不再開啟互動式管理面板**——執行 `/agents` 只會印出提示，告訴你請 Claude 建立，或直接編輯 `.claude/agents/`／`~/.claude/agents/` 底下的檔案（v2.1.197 以前版本仍會開啟含 Running／Library 分頁的舊版面板）。目前建立與呼叫子 Agent 的方式：

```bash
# 1. 直接請 Claude 依自然語言描述建立子 Agent 檔案（推薦）
claude "在 ~/.claude/agents/ 建立一個 code-improver 子 Agent，
負責掃描程式碼並提出可讀性/效能建議，唯讀、使用 Sonnet"

# 2. 對話中以自然語言或 @ 提及指定子 Agent
# 自然語言：Claude 自行判斷是否委派
「使用 test-writer 子 Agent 幫 PaymentService 補測試」
# @ 提及：保證由指定子 Agent 執行
@agent-test-writer 幫 PaymentService 補測試

# 3. 讓整個主 Session 都以某個子 Agent 的設定運作（而非只委派子任務）
claude --agent test-writer -p "為 PaymentService 撰寫單元測試"

# 4. 以 JSON 動態定義當次 Session 專屬、不落地存檔的子 Agent
claude --agents '{"debugger":{"description":"除錯專家","prompt":"你是除錯專家"}}'
```
...
|---|---|---|---|
| Explore | 繼承主對話模型（Anthropic API 上限為 Opus，即主對話用更高階模型時 Explore 最高只會跑在 Opus）| 唯讀工具（無 Write/Edit） | 程式碼搜尋與探索，依「quick／medium／very thorough」三種徹底程度調整範圍 |
...
> ⚠️ **勘誤**：舊版手冊寫 Explore 固定使用 Haiku——這是 v2.1.198 以前的行為。目前 Explore **繼承主對話的模型**（Anthropic API 上限為 Opus）。若團隊仍想讓 Explore 固定跑在低成本模型，可自建一個同名為 `Explore` 的專案／使用者層級子 Agent 並指定 `model: haiku`，會覆蓋內建版本。

Explore 與 Plan 為了保持研究快速、低成本，**不會載入 CLAUDE.md 與 Git 狀態**；其餘所有內建與自訂子 Agent 都會載入完整記憶階層。若要停用某個內建子 Agent，可在 `permissions.deny` 中加入對應規則（如 `Agent(Explore)`）；若要完全禁止任何委派行為，則直接 deny `Agent` 工具本身；只想單純移除 Explore／Plan、改由 Claude 親自讀檔探索，可設定環境變數 `CLAUDE_CODE_DISABLE_EXPLORE_PLAN_AGENTS=1`。

## 10.5 Persistent Memory（子 Agent 持久記憶）

建立子 Agent 時可用 `memory` 欄位啟用持久記憶，讓子 Agent 在跨對話之間累積程式碼慣例、除錯經驗、架構決策等知識，而不是每次從零開始：

| Scope | 路徑 | 適用情境 |
|---|---|---|
| `user` | `~/.claude/agent-memory/<agent名稱>/` | 這個子 Agent 的學習內容適用於你所有專案 |
| `project` | `.claude/agent-memory/<agent名稱>/` | 知識屬於特定專案，且希望團隊透過版控共用（**建議預設選擇**） |
| `local` | `.claude/agent-memory-local/<agent名稱>/` | 知識屬於特定專案，但不適合提交版控 |

啟用後，Read／Write／Edit 工具會自動加入該子 Agent 的可用工具，系統提示也會附上目前 `MEMORY.md`（前 200 行或 25KB，取先達到者）內容。實務上可在子 Agent 的系統提示中直接寫「開始工作前先檢查你的記憶」「完成後把學到的東西存進記憶」，讓子 Agent 主動維護自己的知識庫，而不需要每次手動提醒。
...
> ⚠️ **勘誤**：舊版手冊將本機限定檔案誤植為 `.claude.local.md`，正確檔名應為專案根目錄下的 **`CLAUDE.local.md`**（沒有前導的 `.claude`），本版已修正。

| 層級 | 路徑 | 用途 |
|---|---|---|
| 企業 Managed 政策 | macOS `/Library/Application Support/ClaudeCode/CLAUDE.md`、Linux/WSL `/etc/claude-code/CLAUDE.md`、Windows `C:\Program Files\ClaudeCode\CLAUDE.md`（或直接寫在 `managed-settings.json` 的 `claudeMd` 鍵） | 組織全體強制套用，個別使用者無法排除（詳見11.6） |
| 使用者個人 | `~/.claude/CLAUDE.md` | 個人偏好，跨專案生效 |
| 專案共用 | `./CLAUDE.md` 或 `./.claude/CLAUDE.md` | 團隊共識，建議提交版控 |
| 本機限定 | `./CLAUDE.local.md` | 不提交版控，個人專案限定備註 |

```mermaid
graph TD
    Managed["Managed 政策 CLAUDE.md<br/>(企業強制，最先載入)"] --> Effective["最終生效的<br/>Project Context"]
    User["~/.claude/CLAUDE.md<br/>(個人偏好)"] --> Effective
    Project["./CLAUDE.md 或 ./.claude/CLAUDE.md<br/>(專案共用，版控)"] --> Effective
    Local["./CLAUDE.local.md<br/>(本機限定，不版控)"] --> Effective
```

Claude Code 是從工作目錄逐層往上掃描目錄樹來找 CLAUDE.md／`CLAUDE.local.md`：若在 `foo/bar/` 啟動，會讀到 `foo/bar/CLAUDE.md`、`foo/CLAUDE.md` 及對應的 `CLAUDE.local.md`。這些檔案**全部串接注入 Context，而不是互相覆寫**——由檔案系統根目錄往工作目錄的方向排序，離啟動位置越近的內容越晚出現（越晚出現，模型通常會給予較高權重）；同一目錄內 `CLAUDE.local.md` 會接在 `CLAUDE.md` 之後。子目錄的 CLAUDE.md 則是「Claude 讀到該子目錄檔案時」才隨附載入，不在啟動當下就讀取。若儲存庫已有 `AGENTS.md`（給其他編碼工具用），可在 `CLAUDE.md` 中用 `@AGENTS.md` 匯入取代重複維護。
...
## 11.5 Auto Memory（自動記憶）：Claude 自己寫的筆記

CLAUDE.md 之外，Claude Code 還有第二套跨 Session 記憶機制——**Auto Memory**（v2.1.59+，預設開啟）。兩者互補但角色不同：CLAUDE.md 是「你寫給 Claude 的規則」，Auto Memory 是「Claude 自己在互動中記下的經驗」，兩者都會在每次 Session 開始時載入 Context：

| 比較項目 | CLAUDE.md | Auto Memory |
|---|---|---|
| 誰來寫 | 工程師 | Claude 自己 |
| 內容性質 | 規則、慣例、架構決策 | 學到的建置指令、除錯線索、觀察到的偏好 |
| 範圍 | 專案／使用者／組織 | 每個 Git Repository 一份，所有 Worktree／子目錄共用 |
| 載入方式 | 每次 Session 全文載入 | 每次 Session 載入 `MEMORY.md` 前 200 行或 25KB |
| 適合放什麼 | 建置測試指令、程式碼規範、專案架構 | 只在這個專案發現的建置陷阱、除錯心得、Claude 自行歸納出的偏好 |

Auto Memory 的儲存位置是 `~/.claude/projects/<project>/memory/`（依 Git repo 推導專案名稱，故同一個 repo 的所有 Worktree 共用一份記憶，僅限本機、不會跨機器同步）：

```text
~/.claude/projects/<project>/memory/
├── MEMORY.md          # 索引，每次 Session 開頭載入
├── debugging.md       # 除錯相關細節筆記（隨需讀取，不在啟動時載入）
├── api-conventions.md # API 設計決策
└── ...                # Claude 視需要建立的其他主題檔案
```

```json
// 停用 Auto Memory（專案層級設定）
{ "autoMemoryEnabled": false }
```

```json
// 改變 Auto Memory 儲存位置（任何設定層級皆可）
{ "autoMemoryDirectory": "~/my-custom-memory-dir" }
```

Session 內執行 `/memory` 可列出目前載入的 CLAUDE.md／`CLAUDE.local.md`／Rules 檔案，切換 Auto Memory 開關，並直接開啟 Auto Memory 資料夾檢視。當你對 Claude 說「以後都用 pnpm，不要用 npm」這類要求時，Claude 通常會直接寫進 Auto Memory；若想確保寫進**團隊共用**的 CLAUDE.md，應明確要求「把這個寫進 CLAUDE.md」。

> **📌 實務建議**：Auto Memory 適合「這個特定專案才有」的瑣碎經驗（例如某個測試在 CI 上偶爾會因為 timezone 設定失敗），CLAUDE.md 則保留給「每個新加入的工程師都該知道」的正式規範；把這條分界講清楚，可以避免兩份記憶內容互相重複或衝突。
>
> ⚠️ **注意**：Auto Memory 是 Context（模型會盡量遵循但無強制力），不是強制設定；需要「不管 Claude 怎麼判斷都必須擋下」的規則，仍應使用 PreToolUse Hook（第12章），而不是寄望寫進任何一種記憶檔案。

## 11.6 `.claude/rules/`：路徑範圍化的模組化規則

大型專案的 CLAUDE.md 若塞入過多規則，會拖累每次 Session 的基礎 Token 成本、也可能降低模型遵循的準確度（官方建議單一 CLAUDE.md 控制在 200 行以內）。`.claude/rules/` 讓你把規則拆成多個主題檔案，並可選擇只在 Claude 讀取符合特定路徑的檔案時才載入：
...
│   ├── CLAUDE.md           # 主要專案指令
│   └── rules/
│       ├── code-style.md   # 沒有 paths frontmatter：每次 Session 都載入
│       ├── testing.md
│       └── frontend/
│           └── react.md    # 支援巢狀子目錄
```
...
沒有 `paths` 欄位的規則檔會在啟動時無條件載入（優先序與 `.claude/CLAUDE.md` 相同）；有 `paths` 的規則檔則只在 Claude **讀取**符合該 glob 樣式的檔案時才載入，不是每次工具呼叫都觸發。`.claude/rules/` 也支援 symlink，可讓多個專案共用同一份規則庫；`~/.claude/rules/` 則是套用到你本機所有專案的個人規則，載入順序在專案規則之前（優先序較低）。

> **📌 實務建議**：若發現 CLAUDE.md 裡有一段內容只跟前端或後端其中一邊有關，優先移到 `.claude/rules/` 底下對應的路徑範圍檔案，既能縮小每次 Session 的基礎 Context，也讓規則跟隨實際工作的檔案類型自動生效。
...
Claude Code 提供一系列生命週期事件供 Hook 介入，依作用範圍分類：

| 分類 | 事件 |
|---|---|
| Session 層級 | SessionStart、SessionEnd |
| 對話輪層級 | UserPromptSubmit、Stop、StopFailure |
| 工具層級 | PreToolUse、PostToolUse、PermissionRequest（自訂權限詢問邏輯，與 PreToolUse 的差異是可直接介入核准/拒絕的 UI 呈現） |
| 記憶／指令載入 | InstructionsLoaded（記錄實際載入了哪些 CLAUDE.md／Rules／Skill 檔案，除錯路徑範圍化規則時很有用，詳見第11.6章） |
| Subagent | SubagentStart、SubagentStop（Subagent 定義檔內的 `Stop` 會在委派情境下自動轉換為 `SubagentStop`） |
| 其他治理 | Setup、ConfigChange |

## 12.2 Handler 類型

- `command`：執行 Shell 腳本；Windows 上若要以 PowerShell 撰寫，需在 Hook 設定中加上 `shell: powershell`
...
claude mcp login github      # 對需要 OAuth 登入的 Server（如上例），免開互動面板完成授權（第6.4章）
```
...
> 需要 OAuth 授權的 MCP Server（HTTP／SSE／claude.ai Connector），可用 `claude mcp login <name>` 直接在終端機完成登入；SSH 遠端環境加 `--no-browser` 改印授權網址、貼回終端機即可，不需要開啟 `/mcp` 互動面板。要清除已儲存的憑證則用 `claude mcp logout <name>`。
...
- **整合測試**：協助配置測試替身/容器化依賴
- **E2E 測試**：搭配 Playwright 等工具，依使用者流程描述生成腳本；也可搭配第1.6章提到的 Claude in Chrome 擴充套件（`--chrome`），讓 Claude 直接在瀏覽器中操作頁面驗證行為，再回頭產生對應的自動化腳本
...
互動式 Session 會顯示當次成本；Team/Enterprise 方案可在管理後台查看團隊整體用量與依 Skill/Subagent/Plugin/MCP 拆分的成本明細。若企業透過第三方雲端供應商部署且有部署 Claude apps gateway（第4.5.1章），還能取得依使用者拆分的花費上限（Per-user Spend Limits），彌補原生第三方雲端帳單通常只能看到總量、無法拆到個人的落差。
...
- 設定優先順序與 Managed 強制設定，含 `policyHelper` 動態運算與跨來源鎖定鍵（第5章）
- Hook 強制落實安全底線（第12章）
- MCP Server 分級與白名單（第13章）
- 子 Agent 註冊與審查（第10章）
- 第三方雲端供應商部署可透過 Claude apps gateway 取得與原生帳號一致的集中治理與稽核（第4.5.1章）
- Enterprise 帳號可額外啟用組織層級模型治理（模型限制、預設模型、Effort 上限，由伺服器端強制），但目前不涵蓋 Bedrock／Vertex／Foundry，那些供應商仍需以 `availableModels` 控管（第5.4章）
...
- 用 `--teammate-mode` 決定隊友顯示方式（`in-process` 預設整合在同一畫面、`tmux`／`iterm2` 各自獨立分頁，方便盯著多個隊友的即時輸出）。
- 隊友的工作範圍應手動切分、避免同時觸及相同檔案（Agent Teams 目前不像 Subagent／Agent View 那樣自動套用 Worktree 隔離）。

> ⚠️ **注意**：Agent Teams 用來直接建立/刪除團隊的內部工具（`TeamCreate`／`TeamDelete`）已於近期版本移除，改為由 Claude 依任務描述動態管理隊友生命週期；若團隊文件或腳本中仍引用這兩個工具名稱，應更新為目前的委派方式（直接以自然語言請 Claude 組成團隊）。
...
透過 claude.ai 或 Anthropic API 認證的 Enterprise 組織，可在不部署任何設定檔的前提下，於管理後台直接進行伺服器端的模型治理：**組織模型限制**（停用特定模型，使用者選不到）、**組織預設模型**（新 Session 起始模型，除非組織強制否則使用者仍可自行切換）、**組織 Effort 上限**（依角色限制可用的推理強度等級）。三者皆需 Enterprise 方案，且開放對象為有限度的組織（實際可用性請洽 Anthropic 業務窗口）。

> ⚠️ **注意**：這套伺服器端治理**不涵蓋** Amazon Bedrock、Google Cloud Agent Platform、Microsoft Foundry 或 Claude Platform on AWS 上執行的 Session；在這些供應商上，模型限制仍須透過 Managed 設定的 `availableModels` 執行，預設模型則以 Managed 設定的 `model` 鍵指定（詳見第5.4章）。
...
**Q12：Auto Memory 跟 CLAUDE.md 該怎麼選？重複了嗎？**
不重複，兩者角色不同：CLAUDE.md 是你寫給 Claude 的規則，Auto Memory 是 Claude 自己在互動中歸納出的經驗筆記，兩者都會載入每次 Session 的 Context。團隊都該知道的正式規範放 CLAUDE.md；只在這個專案才會遇到的瑣碎經驗，讓 Claude 自己記到 Auto Memory 即可，詳見第11.5章。

**Q13：執行 `/agents` 沒有跳出管理面板，是不是壞掉了？**
...
- [ ] 已了解 Auto Memory 與 CLAUDE.md 的分工，並知道用 `/memory` 檢視目前載入內容（第11.5章）
