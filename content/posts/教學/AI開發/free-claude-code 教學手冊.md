...
> **版本**：2026-05 ｜ **對應專案**：[Alishahryar1/free-claude-code](https://github.com/Alishahryar1/free-claude-code)
> **適用對象**：資深工程師 / 架構師 / DevOps / AI 開發團隊
> **授權**：MIT License
> **定位**：企業級 AI 輔助開發實戰手冊  
> **語言**：繁體中文  
> **文件更新日期**：2026 年 5 月 2 日
...
free-claude-code 是一個開源 **Anthropic Messages API 相容代理伺服器**（Proxy），以 Python FastAPI 建構，目前在 GitHub 上擁有超過 19,000 顆星。它攔截 Claude Code CLI、VS Code Extension 或 JetBrains ACP 發出的 API 請求，將其轉發至其他 LLM 後端（NVIDIA NIM、OpenRouter、DeepSeek、LM Studio、llama.cpp、Ollama），讓開發者能以**免費或低成本**方式使用 Claude Code 的完整開發體驗。

**核心特色：**

- **六大 Provider 後端**：NVIDIA NIM、OpenRouter、DeepSeek、LM Studio、llama.cpp、Ollama
...
- **Discord / Telegram Bot** 遠端 Coding Session（含對話分支、任務停止與清除）
...
graph TB
    ...
    FCC -->|OpenAI Chat Translation| NIM
    FCC -->|Anthropic Messages| OR
    FCC -->|Anthropic Messages| DS
    FCC -->|Anthropic Messages| LMS
    FCC -->|Anthropic Messages| LC
    FCC -->|Anthropic Messages| OL
```

> **實務建議**：初次導入建議先使用 NVIDIA NIM（免費點數）驗證整體流程，確認可行後再切換至正式 Provider。
...
graph LR
    ...
    subgraph "Provider Layer"
        C1[OpenAI Chat Transport<br/>NVIDIA NIM]
        C2[Anthropic Messages Transport<br/>OpenRouter / DeepSeek / LM Studio / llama.cpp / Ollama]
    end
...
├── cli/                   # Package entry points 與 Claude process management
...
free-claude-code  # 啟動 proxy
```

> `fcc-init` 會從專案根目錄的 `.env.example` 建立使用者設定檔。
...
graph TD
    subgraph "開發環境建置流程"
        ...
        S3 --> S4[Clone free-claude-code]
        S4 --> S5[設定 .env]
        S5 --> S6[啟動 Proxy :8082]
        S6 --> S6A[Startup Validation<br/>自動驗證模型設定]
        S6A --> S7[設定環境變數<br/>ANTHROPIC_BASE_URL]
        S7 --> S8[啟動 Claude Code]
        S8 --> S9[執行 /model 驗證]
    end
```

> **實務建議**：建議將啟動指令包裝成 shell script 或 PowerShell script，統一團隊啟動方式。
...
| 類別 | 關鍵變數 | 必填 |
|------|---------|------|
| 核心 | `MODEL`、`ANTHROPIC_AUTH_TOKEN` | ✅ |
| 模型路由 | `MODEL_OPUS`、`MODEL_SONNET`、`MODEL_HAIKU` | ❌ |
| Thinking | `ENABLE_MODEL_THINKING`、`ENABLE_*_THINKING` | ❌ |
| Provider Key | `NVIDIA_NIM_API_KEY`、`OPENROUTER_API_KEY`、`DEEPSEEK_API_KEY` | 至少一個 |
...
- [ ] free-claude-code 已 clone
- [ ] `.env` 已從 `.env.example` 複製並設定
- [ ] 至少一個 Provider API Key 已設定
- [ ] `ANTHROPIC_AUTH_TOKEN` 已設定
- [ ] Proxy 啟動成功（`uv run uvicorn server:app --port 8082`）
- [ ] 啟動驗證日誌無錯誤警告
