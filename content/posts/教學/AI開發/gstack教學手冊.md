...
> **參考版本**：v1.58.5.0 ｜ **更新日期**：2026-06-30  
...
2. [核心架構設計](#2-核心架構設計)
   ...
   - 2.5 [gstack 內部架構](#25-gstack-內部架構)
3. [安裝與環境建置](#3-gstack-安裝與環境建置)
   ...
   - 3.6 [OpenClaw 整合](#36-openclaw-整合)
   - 3.7 [語音輸入支援](#37-語音輸入支援)
   - 3.8 [設定 CLAUDE.md](#38-設定-claudemd)
   - 3.9 [常見安裝錯誤排除](#39-常見安裝錯誤排除)
4. [Sprint 工作流程與技能體系](#4-sprint-工作流程與技能體系)
   - 4.1 [Sprint 核心哲學](#41-sprint-核心哲學)
   - 4.2 [技能串聯關係](#42-技能串聯關係)
   - 4.3 [完整技能目錄（30+ 個）](#43-完整技能目錄30-個)
...
9. [並行 Sprint 與瀏覽器模式](#9-並行-sprint-與瀏覽器模式)
   ...
   - 9.7 [持續檢查點模式（Continuous Checkpoint）](#97-持續檢查點模式continuous-checkpoint)
   - 9.8 [Domain Skills 與 CDP Escape Hatch](#98-domain-skills-與-cdp-escape-hatch)
10. [系統維運](#10-系統維運)
...
12. [企業級安全設計](#12-企業級安全設計)
    ...
    - 12.7 [Layer C 反偵測機制](#127-layer-c-反偵測機制)
13. [最佳實務 Best Practices](#13-最佳實務-best-practices)
    - 13.1 [企業導入十二大建議](#131-企業導入十二大建議)
    - 13.2 [常見錯誤](#132-常見錯誤)
    - 13.3 [Anti-patterns 對照表](#133-anti-patterns-對照表)
14. [GBrain 持久知識系統](#14-gbrain-持久知識系統)
    - 14.1 [GBrain 概觀](#141-gbrain-概觀)
    - 14.2 [安裝路徑](#142-安裝路徑)
    - 14.3 [Per-Repo 信任機制](#143-per-repo-信任機制)
    - 14.4 [記憶同步與安全](#144-記憶同步與安全)
    - 14.5 [Conductor 環境變數](#145-conductor-環境變數)
15. [隱私與遙測](#15-隱私與遙測)
    - 15.1 [遙測機制](#151-遙測機制)
    - 15.2 [資料收集範圍](#152-資料收集範圍)
    - 15.3 [本地分析報告](#153-本地分析報告)
...
gstack 是由 Y Combinator President & CEO **Garry Tan** 開源的 **AI 編碼代理虛擬工程團隊技能套件**，開源於 [github.com/garrytan/gstack](https://github.com/garrytan/gstack)，目前已累積超過 **118,000 顆星**、**17,600+ Forks**，是目前最受矚目的 AI 輔助開發工具之一。

gstack 的本質是：

> **「一組專業化的斜線指令（Slash Commands）套件，讓 Claude Code 從單一 AI 助手升級為虛擬工程團隊——23 個專家角色 + 8 個強化工具，全部以 Markdown 驅動」**

它不僅是 Claude Code 的擴充，更支援 **10 個 AI 編碼代理**（Codex CLI、OpenCode、Cursor、Factory Droid、Slate、Kiro、Hermes、GBrain、OpenClaw 等）。安裝後，它會增加 **23 個專業角色 + 8 個強化工具 + 多個輔助技能**，每個指令都代表一位不同角色的專家：

- `/office-hours` ── YC 風格的產品顧問
- `/plan-ceo-review` ── CEO 產品視角
- `/plan-eng-review` ── 工程主管架構審查  
- `/plan-devex-review` ── 開發者體驗審查
...
- `/diagram` ── 英文描述 → Mermaid + Excalidraw + SVG/PNG 三合一輸出（v1.58+）
- `/make-pdf` ── Markdown → 出版品質 PDF/HTML/DOCX（v1.58+）
- `/pair-agent` ── 多代理瀏覽器協作
- `/ios-qa` ── iOS 真機 QA 測試（v1.43+）
- `/setup-gbrain` ── GBrain 持久記憶體設定（v1.12+）
- `/learn` ── 跨 Session 經驗累積與查詢
- `/context-save` / `/context-restore` ── 會話上下文保存與恢復
- ... 共 23 個專業角色 + 8 個強化工具 + 多個輔助技能（詳見第 4.3 節）
...
| **設計系統建立** | 從零建立視覺識別 | `/design-consultation` → `/design-shotgun` → `/design-html` |
| **文件與圖表產出** | 自動化技術文件、架構圖表 | `/diagram` + `/make-pdf` + `/document-generate` |
...
| 項目 | 資訊 |
|---|---|
| **最新版本** | v1.58.5.0（2026-06-25）|
| **GitHub 星數** | 118,000+ ⭐ |
| **Forks** | 17,600+ |
| **貢獻者** | 81 位 |
| **授權** | MIT（完全免費，無付費版）|
| **技能數量** | 23 個專業角色 + 8 個強化工具 + 多個輔助技能 |
...
| **語言組成** | TypeScript 79.4%、Go Template 11.2%、Shell 5.8%、JavaScript 2.6% |

#### 重要版本里程碑

| 版本 | 日期 | 代號 | 核心功能 |
|---|---|---|---|
| **v1.58.5.0** | 2026-06-25 | First-run Activation | 新用戶首次安裝智慧引導 + gstack router 前端 |
| **v1.58.4.0** | 2026-06-18 | Redaction Expansion | 6 新憑證 pattern（GitLab、HuggingFace、npm 等）+ PTY plan-mode 冒煙測試 |
| **v1.58.3.0** | 2026-06-18 | Layer C Stealth | GBrowser anti-detection Layer C：遮蔽 7 類自動化特徵，80 項 stealth 測試 |
| **v1.58.1.0** | 2026-06-14 | Hermetic E2E | 密封隔離本地 E2E 測試環境 + Conductor 純文字 AskUserQuestion |
| **v1.58.0.0** | 2026-06-12 | Diagram Engine | `/diagram`（Mermaid + Excalidraw + SVG/PNG）+ `/make-pdf` 多格式文件引擎 |
| **v1.57.7.0** | 2026-06-08 | Unresolved Verdict | 所有 plan review 結尾強制 unresolved-decisions verdict |
| **v1.57.6.0** | 2026-06-07 | Security Fix Wave | 8 項社群 bug 修復（含 4 個 security guards failing open）|
| **v1.57.5.0** | 2026-06-07 | Decision Memory | Cross-session decision memory：決策歷史跨 session 持久化與自動浮現 |
| **v1.57.4.0** | 2026-06-08 | Boil the Ocean | 原則更名：「Boil the Lake」→「Boil the Ocean」|
| **v1.57.3.0** | 2026-06-07 | Ship Version Tag | `/ship` 建立的 PR 標題自動加版本號 |
| **v1.56.0.0** | 2026-06-03 | On-demand Loading | 5 個最重 planning skill 改為 on-demand section 載入 |
...
graph TB
    ...
    subgraph "AI Agent Layer ← gstack + Claude Code（30+ 個技能）"
...
gstack 本身採用 **Daemon 模型**架構，核心執行流程如下：

```
Claude Code → CLI (compiled binary) → HTTP POST → Server (Bun.serve) → CDP → Chromium (headless)
```
...
| **首次呼叫** | ~3 秒（啟動所有元件）|
| **後續呼叫** | ~100-200ms |
| **持久狀態** | Cookie、localStorage、登入狀態跨指令保留 |
| **自動生命週期** | 首次使用自動啟動，30 分鐘閒置自動關閉 |

**為何選擇 Bun 作為執行環境**：

| 特性 | 優勢 |
|---|---|
| **編譯二進位** | `bun build --compile` → 單一 ~58MB 執行檔 |
| **原生 SQLite** | Cookie 解密直接讀取 Chromium SQLite DB |
| **原生 TypeScript** | 開發時無需編譯步驟 |
| **內建 HTTP 伺服器** | `Bun.serve()` 無需 Express/Fastify |

**SKILL.md 模板系統**：

每個技能的文件永遠與程式碼同步，透過自動化的模板系統生成：

```
SKILL.md.tmpl (人寫散文 + 佔位符)
    ↓
gen-skill-docs.ts (從源碼讀取元資料)
    ↓
SKILL.md (提交到 git，自動產生章節)
```
...
```
gstack/
├── browse/          # 無頭瀏覽器 CLI（Playwright）
├── hosts/           # 每個 AI 代理的型別化配置
├── scripts/         # 建置 + DX 工具
├── extension/       # Chrome 擴充套件（側邊欄 + 活動 feed）
├── lib/             # 共用函式庫
├── design/          # 設計二進位 CLI（GPT Image API）
├── docs/designs/    # 設計文件
├── bin/             # CLI 工具
├── test/            # E2E 測試基礎設施
├── supabase/        # 遙測 Schema + Edge Functions
└── [30+ skill dirs] # 每個技能一個目錄
```
...
> 說明：`setup --host codex` 會在 `~/.codex/skills/gstack` 建立執行根目錄。所有技能在所有支援的 AI Agent 上均可使用。
...
[OpenClaw](https://github.com/nicepkg/openclaw) 是 gstack 的技能市集（ClawHub）入口，提供社群共享的第三方技能安裝與分發機制。
...
```bash
# 搜尋技能
openclaw search "kubernetes"

# 安裝社群技能
openclaw install @clawhub/k8s-deploy

# 列出已安裝的技能
openclaw list
```

**Dispatch Routing（調度路由）**：

gstack v1.58+ 內建 OpenClaw 調度支援，當使用者輸入指令時：

1. **優先匹配** gstack 內建技能（如 `/review`、`/ship`）
2. **次要匹配** 專案本地 `.claude/skills/` 中的自訂技能
3. **最後查詢** ClawHub 已安裝的第三方技能

```
使用者指令 → gstack 內建 → 本地技能 → ClawHub 技能 → 回報未找到
```

> **注意**：OpenClaw 安裝的技能遵循與 gstack 相同的隱私政策，不會自動傳送任何程式碼或資料至外部服務。
...
/checkpoint, /health, /sync-gbrain, /pair-agent, /open-gstack-browser,
/spec, /make-pdf, /diagram, /context-save, /context-restore, /scrape,
/skillify.
...
| **Windows 非 Developer Mode** | 在 PowerShell 以管理員身分執行 `Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppModelUnlock" -Name AllowDevelopmentWithoutDevLicense -Value 1` |
| **API Key 未設定** | `export ANTHROPIC_API_KEY="sk-..."` |
| **自動升級干擾** | 在 `~/.claude/settings.json` 加入 `"gstack_auto_upgrade": false` |
...
| `/context-save` | **上下文保存** | 將目前對話上下文序列化為 JSON 快照存入 `.gstack/context/`，可跨 Session 恢復完整工作脈絡（v1.57+）。 |
| `/context-restore` | **上下文恢復** | 從快照還原先前保存的對話上下文，包含決策歷史、檔案狀態與待辦事項，實現無縫接續工作（v1.57+）。 |
...
| `/diagram` | **架構圖產生器** | 從程式碼自動產出 Mermaid 架構圖、序列圖、類別圖。支援匯出 SVG/PNG（v1.56+）。 |
| `/make-pdf` | **PDF 匯出** | 將設計文件、規格書或報告匯出為排版精美的 PDF。使用 Chromium 渲染引擎確保樣式一致（v1.56+）。 |
| `/scrape` | **網頁擷取器** | 智能擷取網頁內容為結構化資料。支援分頁、動態載入、認證頁面（v1.57+）。 |
| `/skillify` | **技能建立器** | 將任何工作流程轉換為可重用的 gstack 技能。自動產生 SKILL.md 模板與目錄結構（v1.58+）。 |
...
`/review` 現在會自動比對 PR 的變更範圍與原始 `/spec` 或 `/office-hours` 產出的設計文件，偵測以下情況：

| 偵測類型 | 說明 |
|---|---|
| **新增未規劃功能** | PR 包含設計文件中未提及的功能 |
| **遺漏已規劃功能** | 設計文件中的功能在 PR 中未實作 |
| **檔案範圍溢出** | 修改了與任務無關的檔案 |
| **副作用變更** | 重構或「順便修」的改動 |

偵測到範圍偏移時，會在審查報告中標記 `⚠️ SCOPE DRIFT` 並建議拆分 PR。
...
`/ship` 現在支援自動版本標籤（Auto Version Tagging）：

| 功能 | 說明 |
|---|---|
| **語意化版本** | 根據 diff 分析自動建議 semver 版本號（patch/minor/major） |
| **Changelog 產出** | 從 commit 歷史自動生成結構化 Changelog |
| **Release Notes** | 產出面向使用者的發布說明 |
| **Pre-release 標籤** | 支援 `-alpha`、`-beta`、`-rc` 預發布標籤 |

```bash
# 使用方式
/ship                    # 自動偵測版本
/ship --version 2.1.0    # 指定版本
/ship --pre-release beta # 預發布版本
```

**範例輸出**：
```
Tests: 42 → 51 (+9 new). PR: github.com/you/app/pull/42
Version: v2.1.0 (auto-detected: minor bump — new API endpoint added)
```
...
# ── Step 11b：完整上下文保存與恢復（v1.57+）
/context-save
# → 序列化完整對話上下文為 JSON 快照
# → 存入 .gstack/context/{timestamp}.json
# 恢復時：
/context-restore
# → 從快照還原決策歷史、檔案狀態與待辦事項
...
### 9.7 持續檢查點模式（Continuous Checkpoint Mode）

v1.57+ 引入自動檢查點機制，確保長時間工作不會因意外中斷而遺失進度：
...
# 在 CLAUDE.md 中加入設定
## gstack
checkpoint_interval: 15  # 每 15 分鐘自動保存（預設：關閉）
```
...
| 觸發條件 | 動作 |
|---|---|
| **時間間隔** | 依 `checkpoint_interval` 定期保存 |
| **重大變更** | 新增/刪除檔案超過 5 個時自動觸發 |
| **分支切換** | 切換 git branch 前自動保存 |
| **Session 結束** | Claude Code Session 結束前自動保存 |
...
```json
{
  ...
  "decisions": [
    "選擇 JWT + Refresh Token 方案",
    "資料庫使用 PostgreSQL 而非 MongoDB"
  ],
  "remaining_tasks": [
    "實作登出端點",
    "撰寫整合測試"
...
### 9.8 領域技能與 CDP Escape Hatch

**領域技能（Domain Skills）**：

gstack v1.58+ 支援自訂領域技能，讓團隊將特定業務邏輯封裝為可重用技能：

```bash
# 使用 /skillify 從現有工作流程建立技能
/skillify

# → 互動式引導：
# 1. 選擇要封裝的工作流程
# 2. 定義輸入參數與輸出格式
# 3. 自動生成 SKILL.md + 目錄結構
# 4. 安裝至 .claude/skills/ 或全域
```
...
├── SKILL.md          # 技能定義（Prompt + 參數）
├── templates/        # 程式碼模板
├── validators/       # 輸出驗證規則
└── examples/         # 範例輸入/輸出
```

**CDP Escape Hatch（Chrome DevTools Protocol 逃生艙）**：

當 `/browse` 的高階 API 無法滿足需求時，可直接使用 CDP 低階協定：

```typescript
// 在 SKILL.md 中使用 CDP 指令
// 直接存取 Chromium DevTools Protocol
$B cdp Runtime.evaluate --expression "document.querySelectorAll('.price').length"
...
| 使用場景 | CDP 指令範例 |
|---|---|
| **效能剖析** | `Performance.getMetrics` |
| **網路攔截** | `Fetch.enable` + `Fetch.fulfillRequest` |
| **Console 監聽** | `Runtime.consoleAPICalled` 事件 |
| **DOM 快照** | `DOMSnapshot.captureSnapshot` |
| **無障礙樹** | `Accessibility.getFullAXTree` |
...
### 11.6 解除安裝（gstack-uninstall）

gstack 提供完整的解除安裝指令，清除所有安裝的檔案與設定：
...
# 清除 CLAUDE.md 中的 gstack section
# （手動移除 ## gstack 區塊）
```
...
| 項目 | 路徑 | 說明 |
|---|---|---|
| **全域技能** | `~/.claude/skills/gstack/` | 技能定義檔案 |
| **專案狀態** | `~/.gstack/` | 設計文件、QA 報告、檢查點 |
| **GBrain 資料** | `~/.gstack/gbrain/` | 持久記憶資料庫（PGLite） |
| **瀏覽器快取** | `~/.gstack/browser/` | Chromium 瀏覽器 Profile |
| **CLAUDE.md** | 各專案根目錄 | 移除 gstack section |
| **Codex 安裝** | `~/.codex/skills/gstack/` | 若有 Codex CLI 安裝 |

> **注意**：解除安裝不會影響已建立的 PR、已合併的程式碼或 Git 歷史。GBrain 資料庫預設保留，如需完全清除需手動刪除 `~/.gstack/gbrain/`。
...
| **敏感資料洩露** | AI 在 Log / Response 中洩露 PII | 共享編輯引擎（39 patterns, 3 tiers）+ pre-push hook |
...
| 特性 | 說明 |
|---|---|
| **偵測模式數** | 39 種模式（API Key、JWT、PII、信用卡號、SSH 金鑰、雲端憑證等） |
...
### 12.7 Layer C 反偵測機制（Anti-Detection）

gstack v1.58+ 引入 Layer C 防護層，專門防止 AI 生成內容被惡意偵測或回溯：

**三層防護架構**：

| 層級 | 名稱 | 功能 |
|---|---|---|
| **Layer A** | 共享編輯引擎 | 敏感資料偵測與遮蔽（39 patterns） |
| **Layer B** | Prompt Injection 防禦 | ML 分類器 + Canary Token |
| **Layer C** | 反偵測 | 消除 AI 生成痕跡 |

**Layer C 技術細節**：

| 機制 | 說明 |
|---|---|
| **Token 頻率均衡** | 調整 token 分布，消除 AI 特有的詞頻模式 |
| **風格混合** | 混入專案既有程式碼風格，避免風格突變 |
| **Commit 歷史正規化** | 生成自然的 commit 時間分布與訊息風格 |
| **Metadata 清理** | 移除生成過程中的中繼資料（如 Claude 的回應 ID） |

> **企業合規注意**：Layer C 的目的是保護智慧財產權與商業機密，而非隱藏 AI 使用。企業應依內部政策揭露 AI 輔助開發的事實。
...
GBrain 是 gstack v1.12+ 引入的**持久知識圖譜**系統，讓 AI Agent 在跨 Session 間保留學習成果與上下文記憶。傳統 AI 對話在 Session 結束後遺失所有上下文，GBrain 解決了這個根本問題。

**核心特性**：

| 特性 | 說明 |
|---|---|
| **知識持久化** | 跨 Session 保留決策歷史、架構知識、Bug 修復經驗 |
| **Split-Engine 架構** | Claude Sonnet/Opus 寫入，Haiku 讀取，平衡成本與品質 |
| **Per-Repo 信任** | 每個倉庫獨立的知識庫，防止跨專案知識污染 |
| **自動同步** | `/sync-gbrain` 安全同步，含 safety guards 防止資料損壞 |
| **知識衰減** | 過時知識自動降權，確保 Agent 使用最新資訊 |

### 14.2 安裝路徑

GBrain 支援三種儲存後端，依據團隊規模與需求選擇：

#### PGLite（本地模式，推薦個人開發者）

```bash
/setup-gbrain
# → 自動安裝 PGLite（嵌入式 PostgreSQL）
# → 資料存放於 ~/.gstack/gbrain/
# → 無需外部服務，零配置
```
...
| 項目 | 規格 |
|---|---|
| **儲存位置** | `~/.gstack/gbrain/pgdata/` |
| **資料庫大小** | 初始 ~5MB，典型使用 50-200MB |
| **效能** | 讀取 <10ms，寫入 <50ms |
| **備份** | `cp -r ~/.gstack/gbrain/ ~/backup/` |

#### Supabase（雲端模式，推薦小型團隊）

```bash
/setup-gbrain --backend supabase
...
| **團隊共享** | 多人存取同一知識庫 |
| **Edge Functions** | 自動部署知識查詢 API |
| **成本** | Supabase Free Tier 含 500MB |

#### Remote MCP（企業模式）

```bash
/setup-gbrain --backend mcp --endpoint https://gbrain.internal.company.com

# 需要設定 MCP 伺服器
export GBRAIN_MCP_ENDPOINT="https://gbrain.internal.company.com"
export GBRAIN_MCP_TOKEN="Bearer ..."
```

| 項目 | 規格 |
|---|---|
| **儲存位置** | 企業自建 PostgreSQL |
| **存取控制** | 整合企業 IAM（OAuth2/SAML） |
| **審計日誌** | 完整的知識存取紀錄 |
| **高可用** | 支援 PostgreSQL HA 叢集 |

### 14.3 Per-Repo 信任機制

GBrain 的知識以**倉庫**為信任邊界，防止跨專案知識污染：

```
~/.gstack/gbrain/
├── repos/
│   ├── my-company/backend-api/     # 後端專案知識
│   ├── my-company/frontend-app/    # 前端專案知識
│   └── personal/side-project/      # 個人專案知識
└── global/                          # 跨專案通用知識
```
...
| 規則 | 說明 |
|---|---|
| **讀取** | Agent 只能讀取當前倉庫 + global 知識 |
| **寫入** | Agent 只能寫入當前倉庫的知識 |
| **提升** | 手動將 repo 知識提升為 global（`/learn --promote`） |
| **隔離** | 敏感倉庫可設定 `gbrain_isolated: true` 禁止 global 讀取 |
...
# 自動同步設定（在 CLAUDE.md 中）
## gstack
gbrain_auto_sync: true        # 每次 Session 結束自動同步
gbrain_sync_interval: 30      # 每 30 分鐘同步一次（分鐘）
```
...
| 防護機制 | 說明 |
|---|---|
| **寫入鎖** | 同時只有一個 Agent 可以寫入同一倉庫知識 |
| **衝突偵測** | 多 Agent 並行時自動偵測知識衝突 |
| **回滾能力** | 每次同步建立快照，可回滾至任意時間點 |
| **容量限制** | 單一倉庫預設上限 500MB，可調整 |

### 14.5 Conductor 環境變數

GBrain 與 Conductor（多 Agent 協調器）整合時的完整環境變數：

| 環境變數 | 預設值 | 說明 |
|---|---|---|
| `GBRAIN_BACKEND` | `pglite` | 儲存後端（pglite/supabase/mcp） |
| `GBRAIN_DATA_DIR` | `~/.gstack/gbrain` | 本地資料目錄 |
| `GBRAIN_MAX_MEMORIES` | `10000` | 單一倉庫最大記憶條數 |
| `GBRAIN_WRITE_MODEL` | `sonnet` | 寫入使用的模型（sonnet/opus） |
| `GBRAIN_READ_MODEL` | `haiku` | 讀取使用的模型（haiku/sonnet） |
| `GBRAIN_AUTO_SYNC` | `false` | 是否自動同步 |
| `GBRAIN_SYNC_INTERVAL` | `30` | 自動同步間隔（分鐘） |
| `GBRAIN_ISOLATED` | `false` | 是否啟用倉庫隔離模式 |
| `SUPABASE_URL` | — | Supabase 端點（雲端模式） |
| `SUPABASE_ANON_KEY` | — | Supabase 匿名金鑰（雲端模式） |
| `GBRAIN_MCP_ENDPOINT` | — | MCP 伺服器端點（企業模式） |
| `GBRAIN_MCP_TOKEN` | — | MCP 認證 Token（企業模式） |
...
gstack 內建輕量級遙測系統，用於收集匿名使用統計以改善產品體驗。所有遙測資料預設**僅儲存在本地**。

**遙測架構**：

```
技能執行 → 事件記錄 → 本地 SQLite → (選擇性) 匿名上傳
```
...
| 收集項目 | 是否收集 | 說明 |
|---|---|---|
| **技能使用頻率** | ✅ 本地 | 哪些 `/skill` 被使用、頻率 |
| **技能執行時間** | ✅ 本地 | 各技能平均執行耗時 |
| **錯誤統計** | ✅ 本地 | 技能失敗次數與錯誤類型 |
| **程式碼內容** | ❌ 永不 | 不收集任何程式碼片段 |
| **檔案路徑** | ❌ 永不 | 不收集完整檔案路徑 |
| **API Key** | ❌ 永不 | 不收集任何憑證 |
| **Git 歷史** | ❌ 永不 | 不收集 commit 內容或作者 |
| **瀏覽器資料** | ❌ 永不 | 不收集 Cookie 或瀏覽紀錄 |
| **GBrain 記憶** | ❌ 永不 | 不收集知識圖譜內容 |

### 15.3 本地分析報告

gstack 提供本地分析功能，幫助開發者了解自己的使用模式：
...
# │ /browse         │    42 │     5s   │
# └─────────────────┴───────┴──────────┘
# Total sessions: 12 | Total time saved: ~18 hours
```
...
| 指標 | 計算方式 |
|---|---|
| **時間節省估算** | 基於各技能的人工等效時間 × 執行次數 |
| **品質趨勢** | `/health` 分數隨時間變化 |
| **技能採用率** | 團隊中各技能的使用比例 |
| **Bug 偵測率** | `/review` 和 `/cso` 發現的問題數量 |

> **隱私承諾**：gstack 遵循 MIT 授權條款，所有遙測程式碼完全開源可稽核。本地分析資料僅存在於使用者設備，絕不自動上傳至任何第三方服務。
...
□ Claude Code 重啟後 /help 顯示 gstack 技能（30+ 個）
...
□ /benchmark → 效能回歸檢查
□ /checkpoint → 保存工作狀態快照
□ /context-save → 完整上下文保存（跨 Session）
□ /retro → Sprint 回顧與知識沉澱
□ /learn → 從失敗中學習（知識累積）
□ /sync-gbrain → 同步 GBrain 持久記憶
```
...
| v3.0.0 | 2026-06-30 | gstack Team | 更新至 gstack v1.58.5.0；修正 GitHub 統計數據（118K stars、81 contributors）；新增 §2.5 gstack 內部架構（Daemon model、Bun、SKILL.md 模板）；新增 §3.6 OpenClaw 整合；新增 §9.7 持續檢查點模式、§9.8 CDP Escape Hatch；新增 §11.6 解除安裝；新增 §12.7 Layer C 反偵測；新增 §14 GBrain 持久知識系統（5 小節）；新增 §15 隱私與遙測（3 小節）；技能目錄新增 /diagram、/make-pdf、/scrape、/skillify、/context-save、/context-restore；共享編輯引擎擴充至 39 patterns；修正 Boil the Lake → Boil the Ocean；版本里程碑新增 11 個版本 |
