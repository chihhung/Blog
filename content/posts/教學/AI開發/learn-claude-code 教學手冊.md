+++
date = '2026-07-01T17:00:16+08:00'
draft = false
...
> **參考專案**：[shareAI-lab/learn-claude-code](https://github.com/shareAI-lab/learn-claude-code)（69.3k+ Stars，MIT License）  
...
- [3. Agent 核心機制解析（20 Sessions）](#3-agent-核心機制解析20-sessions)
  - [3.1 Agent Loop（s01）](#31-agent-loops01)
  - [3.2 Tool Use 與 Dispatch Map（s02）](#32-tool-use-與-dispatch-maps02)
  - [3.3 Permission 三層防護（s03）](#33-permission-三層防護s03)
  - [3.4 Hooks 擴展點機制（s04）](#34-hooks-擴展點機制s04)
  - [3.5 Planning / Reflection — TodoWrite（s05）](#35-planning--reflection--todowrite-s05)
  - [3.6 Subagent 機制（s06）](#36-subagent-機制s06)
  - [3.7 Skills 按需載入（s07）](#37-skills-按需載入s07)
  - [3.8 Context Compact — 壓縮策略（s08）](#38-context-compact--壓縮策略s08)
  - [3.9 Memory System 記憶系統（s09）](#39-memory-system-記憶系統s09)
  - [3.10 System Prompt 動態組裝（s10）](#310-system-prompt-動態組裝s10)
  - [3.11 Error Recovery 錯誤恢復（s11）](#311-error-recovery-錯誤恢復s11)
  - [3.12 Task 持久化與相依管理（s12）](#312-task-持久化與相依管理s12)
  - [3.13 Background Tasks（s13）](#313-background-taskss13)
  - [3.14 Cron Scheduler 排程機制（s14）](#314-cron-scheduler-排程機制s14)
  - [3.15 Agent Teams 與 Mailbox（s15-s16）](#315-agent-teams-與-mailboxs15-s16)
  - [3.16 Autonomous Agent 自治（s17）](#316-autonomous-agent-自治s17)
  - [3.17 Worktree + Task Isolation（s18）](#317-worktree--task-isolations18)
  - [3.18 MCP Plugin 外部工具整合（s19）](#318-mcp-plugin-外部工具整合s19)
  - [3.19 完整 Agent 整合（s20）](#319-完整-agent-整合s20)
- [4. Harness Engineering（核心競爭力）](#4-harness-engineering核心競爭力)
  ...
  - [4.6 CLAUDE.md 系統設計](#46-claudemd-系統設計)
- [5. 實作教學（Step-by-step）](#5-實作教學step-by-step)
  ...
  - [5.8 加入 Hooks 機制](#58-加入-hooks-機制)
...
- [9. 與企業架構整合](#9-與企業架構整合)
  ...
  - [9.6 MCP Server 整合](#96-mcp-server-整合)
...
- [12. 最佳實踐（Best Practices）](#12-最佳實踐best-practices)
  ...
  - [12.4 成本控制](#124-成本控制)
  - [12.5 Context Engineering 最佳實踐](#125-context-engineering-最佳實踐)
...
- [14. 延伸資源與姊妹專案](#14-延伸資源與姊妹專案)
  ...
  - [14.4 OpenClaw — 社群生態](#144-openclaw--社群生態)
- [15. 附錄](#15-附錄)
...
**learn-claude-code** 是由 shareAI-lab 開源的 AI Coding Agent 教學專案（GitHub **69.3k+ Stars**，MIT License，創立於 2025-06），核心理念為「**Harness Engineering for Real Agents**」。專案副標題精確概括了其定位：

> *Bash is all you need — A nano claude code-like 「agent harness」, built from 0 to 1*

該專案透過 **20 個遞進式實作單元（Sessions）**，從零開始教你構建一個功能完整的 AI Coding Agent 系統。每個 Session 新增一個 Harness 機制，且都附帶一句設計格言：

| 階段 | Sessions | 核心學習 | 新增機制 |
|------|----------|----------|---------|
| **Stage 1：核心能力** | s01-s06 | Agent Loop / Tool Dispatch / Permission / Hooks / TodoWrite / Subagent | 基礎 6 大機制 |
| **Stage 2：複雜任務** | s07-s11 | Skills / Context Compact / Memory / System Prompt / Error Recovery | 知識管理與容錯 |
| **Stage 3：長期運作** | s12-s14 | Task DAG / Background Tasks / Cron Scheduler | 持久化與排程 |
| **Stage 4：多 Agent** | s15-s18 | Agent Teams / Protocols / Autonomous / Worktree | 團隊協作 |
| **Stage 5：整合延伸** | s19-s20 | MCP Plugin / 完整整合 | 外部工具 + 系統整合 |

**各 Session 格言速覽**：

| Session | 格言 | 核心機制 |
|---------|------|---------|
| s01 | "One loop & Bash is all you need" | Agent Loop |
| s02 | "Adding a tool means adding one handler" | Tool Dispatch Map |
| s03 | "Set boundaries first, then grant freedom" | Permission 三閘門 |
| s04 | "Hook around the loop, never rewrite the loop" | Hooks 擴展點 |
| s05 | "An agent without a plan drifts" | TodoWrite |
| s06 | "Break big tasks down; each subtask gets clean context" | Subagent |
| s07 | "Load knowledge on demand, not upfront" | Skills 按需載入 |
| s08 | "Context always fills up — have a way to make room" | 四層 Context Compact |
| s09 | "Remember what matters, forget what doesn't" | Memory System |
| s10 | "Prompts are assembled at runtime, not hardcoded" | 動態 System Prompt |
| s11 | "Errors aren't the end, they're the start of a retry" | Error Recovery |
| s12 | "Break big goals into small tasks, order them, persist" | Task DAG |
| s13 | "Slow ops go background, agent keeps thinking" | Background Tasks |
| s14 | "Fire on schedule, no human kick needed" | Cron Scheduler |
| s15 | "Too big for one agent — delegate to teammates" | Agent Teams |
| s16 | "Teammates need shared communication rules" | Team Protocols |
| s17 | "Teammates check the board, claim work themselves" | Autonomous Agents |
| s18 | "Each works in its own directory, no interference" | Worktree Isolation |
| s19 | "Not enough capability? Plug in more via MCP" | MCP Plugin |
| s20 | "Many mechanisms, one loop" | 完整整合 |

專案配套提供三語文件（English / 中文 / 日本語）、互動學習平台（[learn.shareai.run](https://learn.shareai.run/)），以及基於 pytest 的 CI 煙霧測試。
...
### 3.3 Permission 三層防護（s03）

> **格言**：`"Set boundaries first, then grant freedom"` — 先設邊界，再授予自由

Permission 系統是 Agent 安全邊界的核心，採用三閘門管線設計：
...
class PermissionGateway:
    ...
    DENY_LIST = [
        ...
        r"curl.*\|\s*bash",        # 遠端程式碼執行
        r"chmod\s+777\s+/",        # 授予全域寫入
        r">\s*/etc/passwd",        # 覆蓋系統檔案
    ]
    
    # 第二層：規則引擎（根據上下文決定）
    RULE_ENGINE = [
        # 格式：(pattern, condition, verdict)
        (r"git push",        "branch == 'main'",    "REQUIRE_APPROVAL"),
        (r"kubectl apply",   "namespace == 'prod'", "REQUIRE_APPROVAL"),
        (r"\.env",           "operation == 'write'","DENY"),
        (r"secrets/",        "any",                 "DENY"),
    ]
    
    def check(self, tool_name: str, tool_input: dict, context: dict) -> tuple:
        """三閘門權限檢查，回傳 (allowed: bool, reason: str)"""
        
        # Gate 1: 硬性拒絕清單
        cmd = tool_input.get("command", "") or tool_input.get("path", "")
        for pattern in self.DENY_LIST:
            if re.search(pattern, cmd, re.IGNORECASE):
                return False, f"[DENY_LIST] 禁止操作: {pattern}"
        
        # Gate 2: 規則引擎
        for pattern, condition, verdict in self.RULE_ENGINE:
            if re.search(pattern, cmd, re.IGNORECASE):
                if self._eval_condition(condition, context):
                    if verdict == "DENY":
                        return False, f"[RULE] 規則禁止: {condition}"
                    elif verdict == "REQUIRE_APPROVAL":
                        return self._request_human_approval(cmd)
        
        # Gate 3: 預設允許（白名單外需人工確認）
        return True, "OK"
    
    def _request_human_approval(self, command: str) -> tuple:
        """非同步人工審批"""
        print(f"\n⚠️ 需要審批：{command}")
        decision = input("允許此操作？[y/n]: ").strip().lower()
        return (True, "已核准") if decision == "y" else (False, "已拒絕")
```
...
└─────────────┬───────────────────────────┘
              ...
              ▼
┌─────────────────────────────────────────┐
│  Gate 2: Rule Engine（情境規則）          │
│  • 根據上下文（分支、環境、操作類型）     │
│  • 可設定、可擴展                         │
│  匹配 → DENY 或 REQUIRE_APPROVAL         │
└─────────────┬───────────────────────────┘
              │ 通過
              ▼
┌─────────────────────────────────────────┐
│  Gate 3: Human Approval（人工審批）       │
│  • 高風險操作需人工確認                   │
│  • 非同步等待決策                         │
│  批准 → ALLOWED / 拒絕 → BLOCKED         │
└─────────────────────────────────────────┘
```
...
| 場景 | 建議策略 |
|------|----------|
| 開發環境 | Gate 1 + Gate 2（規則寬鬆）|
| 測試環境 | Gate 1 + Gate 2（中等嚴格）|
| 生產環境 | Gate 1 + Gate 2 + Gate 3（全部啟用）|
| CI/CD 自動化 | Gate 1 + Gate 2（無 Gate 3，改用審批 PR）|

> **格言的深意**：安全不是禁止 Agent 做任何事，而是確保它只做被授權的事。過度限制會讓 Agent 無法完成工作；缺乏限制會造成災難。三層防護找到這個平衡點。

### 3.4 Hooks 擴展點機制（s04）

> **格言**：`"Hook around the loop, never rewrite the loop"` — 在迴圈外掛鉤，永遠不要重寫迴圈

Hooks 是 Agent Loop 的擴展點系統，讓你在不修改核心迴圈的情況下注入額外行為：
...
class HookSystem:
    """Agent Loop 的 Hooks 管理器"""
    
    def __init__(self):
        self._hooks: dict[str, list[Callable]] = {
            "UserPromptSubmit":  [],   # 使用者提交 Prompt 後
            "PreToolUse":        [],   # Tool 執行前
    ...
    def fire(self, event: str, ctx: HookContext) -> Optional[str]:
        """觸發 Hook，回傳 None 表示繼續，回傳字串表示覆寫結果"""
        for handler in self._hooks.get(event, []):
            result = handler(ctx)
            if result is not None:
                return result  # 第一個非 None 結果覆寫後續
        return None

# === 企業級 Hook 範例 ===

# 1. PreToolUse：自動安全掃描
def security_scan_hook(ctx: HookContext) -> Optional[str]:
    """在 bash 工具執行前掃描危險命令"""
    if ctx.tool_name == "bash":
        cmd = ctx.tool_input.get("command", "")
        if "DROP TABLE" in cmd.upper():
            return "HOOK_BLOCKED: 偵測到 DROP TABLE，已阻止執行"
    return None  # 允許繼續

# 2. PostToolUse：成本追蹤
def cost_tracking_hook(ctx: HookContext):
    """在每次 Tool 執行後記錄成本"""
    import json, time
    entry = {
        "timestamp": time.time(),
        ...
        "result_len": len(ctx.tool_result or ""),
    }
...
def notification_hook(ctx: HookContext):
    """Agent 完成任務時發送 Slack 通知"""
    import subprocess
    subprocess.run([
        "curl", "-X", "POST", "-H", "Content-type: application/json",
        "--data", f'{{"text": "Agent [{ctx.agent_name}] 已完成任務"}}',
        os.getenv("SLACK_WEBHOOK_URL", "")
    ], capture_output=True)

# === 整合到 Agent Loop ===
hooks = HookSystem()
hooks.register("PreToolUse",  security_scan_hook)
hooks.register("PostToolUse", cost_tracking_hook)
hooks.register("Stop",        notification_hook)

def agent_loop_with_hooks(user_message: str):
    messages = [{"role": "user", "content": user_message}]
    ...
    while True:
        ...
        if response.stop_reason != "tool_use":
            # 觸發 Stop Hook
            hooks.fire("Stop", HookContext(tool_name="", tool_input={}))
            break
        
        results = []
        for block in response.content:
            if block.type == "tool_use":
                ctx = HookContext(tool_name=block.name, tool_input=block.input)
                
                # 觸發 PreToolUse Hook
                blocked = hooks.fire("PreToolUse", ctx)
                if blocked:
                    output = blocked  # Hook 阻止執行
                else:
                    output = TOOL_HANDLERS[block.name](**block.input)
                
                # 觸發 PostToolUse Hook
                ctx.tool_result = output
                hooks.fire("PostToolUse", ctx)
...
| Hook 事件 | 觸發時機 | 典型用途 |
|-----------|----------|----------|
| `UserPromptSubmit` | 使用者提交 Prompt 後 | Prompt Injection 掃描、意圖分類 |
| `PreToolUse` | Tool 執行前 | 安全檢查、命令審查、權限驗證 |
| `PostToolUse` | Tool 執行後 | 成本追蹤、輸出清洗、Audit Log |
| `Stop` | Agent 正常停止 | 完成通知、結果報告、資源清理 |
| `SubagentStop` | Subagent 停止 | 子任務完成彙報、結果聚合 |

> **核心設計原則**：Hooks 不應該替代核心迴圈邏輯，而是在迴圈的關鍵節點提供觀察和干預能力。過度使用 Hooks 會讓系統難以追蹤；適度使用則提供強大的可擴展性而不破壞主流程的清晰度。

### 3.5 Planning / Reflection — TodoWrite（s05）
...
### 3.9 Memory System 記憶系統（s09）

> **格言**：`"Remember what matters, forget what doesn't"` — 記住重要的，遺忘不重要的

Memory System 讓 Agent 在長任務中持續保持對關鍵資訊的記憶，由三個子系統組成：
...
class MemorySystem:
    ...
    def should_remember(self, content: str) -> bool:
        """判斷此內容是否值得記住"""
        return any(trigger in content for trigger in self.MEMORY_TRIGGERS)
    
    # --- 第二層：擷取（Extraction）---
    # 決定「以何種形式儲存」
    def extract_and_store(self, content: str, category: str):
        """擷取關鍵資訊並結構化存儲"""
        entry = {
            "category": category,          # 類別
            "content": content[:500],       # 內容摘要
            "created_at": datetime.now().isoformat(),
            "access_count": 0,             # 存取次數（用於遺忘策略）
        }
        self.memories.setdefault(category, []).append(entry)
        self._save()
    
    # --- 第三層：整合（Consolidation）---
    # 定期合併相似記憶，防止記憶庫膨脹
    def consolidate(self):
        ...
        for category, entries in self.memories.items():
            if len(entries) > 10:
                # 使用 LLM 摘要並合併
                summary_prompt = f"請整合以下 {category} 類別的記憶，保留關鍵資訊：\n"
                for e in entries:
                    summary_prompt += f"- {e['content']}\n"
                
                consolidated = client.messages.create(
                    model=MODEL,
                    system="你是記憶整合助手，請將多條記憶合併為簡潔的摘要。",
                    max_tokens=500,
    ...
    def recall(self, query: str) -> str:
        ...
        # 簡單關鍵字匹配（生產環境可用向量搜尋）
        relevant = [m for m in all_memories if any(w in m["content"] for w in query.split())]
...
Extraction（以何種結構儲存？）
    │
    ▼
Storage（存入 .agent_memory.json）
    │
    ▼                         ← 定期執行
Consolidation（記憶超過 10 條時整合）
    │
    ▼
Recall（按需從記憶庫取回注入 Context）
```

### 3.10 System Prompt 動態組裝（s10）

> **格言**：`"Prompts are assembled at runtime, not hardcoded"` — System Prompt 在執行時動態組裝，而非硬編碼

傳統做法是把 System Prompt 寫死，但高效能的 Agent 會在執行時根據狀態動態組裝：

```python
class SystemPromptBuilder:
    """動態 System Prompt 組裝器"""
    ...
    def __init__(self):
        self.sections = {
            ...
            "skills":      "",   # 按需載入的技能（動態）
            "memory":      "",   # 長期記憶摘要（動態）
            "constraints": "",   # 當前約束條件（動態）
        }
    
    def set_identity(self, role: str, expertise: list):
        """設定角色身份（通常固定）"""
        self.sections["identity"] = f"""你是 {role}。
專業領域：{', '.join(expertise)}。
你的目標是高效、安全地完成使用者交付的任務。"""
    
    def set_active_skills(self, skill_names: list):
        """注入當前激活的技能知識"""
        skill_content = []
    ...
    def inject_memory(self, memory: MemorySystem, current_task: str):
        """注入相關記憶"""
        recalled = memory.recall(current_task)
        self.sections["memory"] = recalled
    
    def set_task_context(self, task_desc: str, progress: str):
        """設定當前任務上下文"""
        self.sections["context"] = f"""## 當前任務
{task_desc}
...
{progress}"""
    ...
    def build(self) -> str:
        ...
        # 固定部分（永遠包含）
        if self.sections["identity"]:
            parts.append(self.sections["identity"])
        if self.sections["tools"]:
            parts.append(self.sections["tools"])
        
        # 動態部分（非空時包含）
        for section in ["context", "skills", "memory", "constraints"]:
...
builder.set_identity("資深 Java 工程師", ["Spring Boot", "Microservices", "TDD"])
builder.set_active_skills(["spring-boot-3", "security-checklist"])
builder.inject_memory(memory_system, current_task="實作使用者登入 API")
builder.set_task_context("實作 JWT 認證模組", "已完成 Entity 層，正在實作 Service 層")

DYNAMIC_SYSTEM = builder.build()
```

**靜態 vs 動態 System Prompt 對比**：

| 比較項目 | 靜態 System Prompt | 動態組裝 |
|----------|-------------------|---------|
| Token 消耗 | 固定（包含所有知識） | 按需（只含當前需要的） |
| 靈活性 | 低 | 高 |
| 維護成本 | 低 | 中 |
| 適用場景 | 簡單 Agent | 複雜、長任務 Agent |
| 知識覆蓋 | 有限（受 Token 限制） | 無限（按需載入） |

> **設計原則**：System Prompt 應該像衣服，不是像皮膚。根據任務穿上適合的裝備，而不是把所有知識都塞進去。

### 3.12 Task 持久化與相依管理（s12）
...
### 3.14 Cron Scheduler 排程機制（s14）

> **格言**：`"Fire on schedule, no human kick needed"` — 按排程觸發，無需人工介入

Cron Scheduler 讓 Agent 能夠自動定時執行任務，實現真正的「常駐式」運作：
...
class CronJob:
    """持久化排程任務定義"""
    def __init__(self, job_id: str, cron_expr: str, task: str, enabled: bool = True):
        self.id = job_id
        self.cron_expr = cron_expr   # "0 2 * * *" = 每天凌晨 2 點
        self.task = task             # 要執行的任務描述
        self.enabled = enabled
        self.last_run = None
        self.next_run = None

class CronScheduler:
    """耐久性排程器：基於檔案的任務佇列 + 輪詢執行"""
    ...
    def add_job(self, cron_expr: str, task: str) -> str:
        """新增排程任務"""
            ...
            while self._running:
                ...
                for job_id, job in list(self.jobs.items()):
                    if not job.get("enabled"):
                        continue
                    if job.get("next_run") and job["next_run"] <= now:
                        print(f"[Cron] 觸發排程任務: {job['task']}")
    ...
    def _calc_next_run(self, cron_expr: str) -> str:
        """解析 cron 表達式，計算下次執行時間（簡化版）"""
        # 生產環境建議使用 croniter 套件
        # 這裡用簡單的 daily 示意
...
# 每天凌晨 2 點掃描安全漏洞
scheduler.add_job("0 2 * * *", "對整個程式碼庫執行 SAST 掃描，產出安全報告")

# 每週一早上 9 點產生技術債報告
scheduler.add_job("0 9 * * 1", "分析所有 TODO/FIXME 評論，產出技術債報告")

# 每小時檢查 CI 狀態
scheduler.add_job("0 * * * *", "檢查最近 1 小時內失敗的 CI 建置，並嘗試自動修復")

# 啟動排程器
scheduler.start(agent_executor=lambda task: agent_loop(task))
```
...
| 排程 | Cron 表達式 | 任務 |
|------|-------------|------|
| 每日安全掃描 | `0 2 * * *` | SAST + 依賴弱掃 |
| 週報產生 | `0 9 * * 1` | 程式碼品質 + 技術債彙整 |
| 每小時 CI 監控 | `0 * * * *` | 失敗建置自動修復 |
| 每月合規檢查 | `0 0 1 * *` | 授權合規 + 證書到期檢查 |
| 每日文件同步 | `0 18 * * *` | API 文件自動更新 |

> **Cron vs Background Task**：Background Task（s13）是「單次非同步執行」，Cron Scheduler（s14）是「週期性定時觸發」。前者用於長時間的單一任務；後者用於建立常態性的 Agent 運作節奏。兩者可以組合：Cron 觸發 → Background Task 執行。

### 3.15 Agent Teams 與 Mailbox（s15-s16）
...
### 3.17 Worktree + Task Isolation（s18）
...
### 3.11 Error Recovery 錯誤恢復（s11）

> **格言**：`"Errors aren't the end, they're the start of a retry"` — 錯誤不是終點，而是重試的起點
...
### 3.18 MCP Plugin 外部工具整合（s19）

> **格言**：`"Not enough capability? Plug in more via MCP"` — 能力不足？透過 MCP 插入更多

MCP（Model Context Protocol）是 Anthropic 制定的開放標準，讓 Agent 能以統一介面連接任意外部系統，無需為每個系統手工撰寫 Tool handler：
...
class MCPClient:
    ...
    def _send(self, method: str, params: dict = None) -> dict:
        """透過 JSON-RPC 2.0 與 MCP Server 通訊"""
        self._msg_id += 1
        request = {
            "jsonrpc": "2.0",
            "id": self._msg_id,
    ...
    def _initialize(self):
        ...
        self._send("initialize", {
            ...
            "clientInfo": {"name": "learn-claude-code", "version": "1.0"},
        })
...
class MCPToolPool:
    """統一工具池：內建 Tool + MCP 動態 Tool 的無縫整合"""
    
    NAMING_PREFIX = "mcp__{server}__{tool}"
    
    def __init__(self, builtin_tools: list, builtin_handlers: dict):
        self.tools = list(builtin_tools)         # 內建工具定義
        self.handlers = dict(builtin_handlers)   # 內建工具處理器
        self.mcp_clients = {}
    
    def connect_mcp_server(self, server_name: str, command: str):
        """連接 MCP Server 並整合其工具"""
        ...
        for tool in mcp_tools:
            # 使用命名前綴避免衝突：mcp__github__create_issue
            safe_name = self.NAMING_PREFIX.format(
                server=server_name,
                tool=self._normalize_name(tool["name"])
            )
            ...
            # 動態建立 Handler
            def make_handler(client, original_name):
                return lambda **kwargs: client.call_tool(original_name, kwargs)
            
            self.handlers[safe_name] = make_handler(client, tool["name"])
...
# 企業 MCP Server 整合範例
tool_pool = MCPToolPool(TOOLS, TOOL_HANDLERS)

# 連接 GitHub MCP Server（管理 Issue / PR）
tool_pool.connect_mcp_server("github", "npx @modelcontextprotocol/server-github")

# 連接資料庫 MCP Server
tool_pool.connect_mcp_server("postgres", "npx @modelcontextprotocol/server-postgres")

# 連接 Jira MCP Server（企業自建）
tool_pool.connect_mcp_server("jira", "python mcp_servers/jira_server.py")

# Agent Loop 無需任何改動，自動支援所有 MCP 工具
print(f"可用工具數量：{len(tool_pool.tools)} 個")
# 輸出：可用工具數量：42 個（5 內建 + 37 MCP 動態）
```
...
| MCP Server | 功能 | 適用場景 |
|------------|------|----------|
| GitHub MCP | Issue、PR、Code Review 管理 | DevOps 自動化 |
| PostgreSQL MCP | 資料庫查詢與 Schema 管理 | 資料分析任務 |
| Filesystem MCP | 增強型檔案操作 | 複雜檔案管理 |
| Slack MCP | 頻道訊息、通知 | 團隊協作整合 |
| Jira MCP | Ticket 管理、Sprint 追蹤 | 專案管理整合 |
| Puppeteer MCP | 瀏覽器自動化 | E2E 測試、爬蟲 |
| 企業自建 MCP | 對接內部系統 | ERP、OA、資安平台 |

**自建 MCP vs 手工 Tool 的選擇時機**：

```
選 MCP Server：
✅ 需要與多個 Agent / 多個團隊共享工具
✅ 工具需要獨立部署、版本管理
✅ 整合的系統已有 MCP Server 生態
✅ 需要支援多種傳輸協議（stdio / HTTP / WebSocket）
...
### 3.19 完整 Agent 整合（s20）

> **格言**：`"Many mechanisms, one loop"` — 眾多機制，一個迴圈

s20 是 learn-claude-code 的最終整合課程，將前 19 個機制整合進單一可運作的 Harness。整合後的 Agent 具備 27+ 內建工具 + 動態 MCP 工具：

```
s20 完整 Agent 工具清單（27+ 內建）：

核心工具（s01-s02）：
  bash, read_file, write_file, edit_file, glob, grep

計畫與隔離（s05-s06）：
  todo_write, todo_update, subagent_spawn

知識管理（s07, s10）：
  load_skill, list_skills, update_system_prompt

記憶（s09）：
  memory_store, memory_recall, memory_consolidate

任務系統（s12）：
  task_create, task_update, task_list, task_get_ready
...
團隊協作（s15-s16）：
  team_send, team_read, team_register

工作區（s18）：
  worktree_create, worktree_remove

MCP 整合（s19）：
  mcp_connect, mcp_list_tools（動態）
...
使用者 Prompt
    │
    ▼ UserPromptSubmit Hook（Prompt 掃描）
┌──────────────────────────────────────────────────┐
│                  Agent Loop (s01)                │
│                                                  │
│  ┌─────────────────────────────────────────────┐ │
│  │ System Prompt（動態組裝 s10）                │ │
│  │ = Identity + Skills + Memory + Context      │ │
│  └─────────────────────────────────────────────┘ │
│                                                  │
│  ┌─────────────────────────────────────────────┐ │
│  │ Tool Pool（s02 + s19）                       │ │
│  │ = 27 內建工具 + N 個 MCP 動態工具            │ │
│  └─────────────────────────────────────────────┘ │
│                  │                               │
│                  ▼ PreToolUse Hook               │
│          Permission Gateway (s03)                │
│          Deny List → Rules → Approval            │
│                  │                               │
│                  ▼                               │
│          Tool Execution                          │
│                  │                               │
...
│                                                  │
│  Memory System (s09) ← 自動記憶關鍵資訊          │
│  Context Compact (s08) ← Token > 70% 時觸發     │
│  Task Manager (s12) ← 追蹤多步驟任務            │
│  Cron Scheduler (s14) ← 定時觸發                │
│  Agent Teams (s15-16) ← 複雜任務委派            │
│  Worktree (s18) ← 每任務獨立目錄                │
└──────────────────────────────────────────────────┘
    │
    ▼ Stop Hook（完成通知、資源清理）
Response to User
...
> **s20 的核心啟示**：20 個機制，每一個都圍繞著同一個核心迴圈。迴圈從未改變。這正是 Harness Engineering 的精髓 — 透過分層的機制為模型建構越來越豐富的運作環境，而不是試圖讓迴圈本身變得複雜。
...
CLAUDE.md 是 Claude Code 實現「跨 Session 持久化指令」的核心機制，讓 Harness 工程師可以在專案層面永久定義 Agent 的行為規範，無需每次重新 Prompt。
...
1. 全域設定（~/.claude/CLAUDE.md）
   └── 適用所有專案的個人偏好、通用規則

2. 專案根目錄（./CLAUDE.md）
   └── 整個專案的 Agent 行為規範

3. 子目錄（./src/CLAUDE.md、./tests/CLAUDE.md）
   └── 特定模組的局部覆蓋規則
...
## 專案背景
這是一個金融業核心交易系統，使用 Java 21 + Spring Boot 3.2。
所有程式碼變更必須符合 PCI-DSS 合規要求。

## 技術規範
- 架構：Clean Architecture（Controller → Service → Repository）
- 框架：Spring Boot 3.2、Spring Security 6、JPA/Hibernate 6
- 測試：JUnit 5、Mockito、AssertJ（覆蓋率 >= 80%）
- 程式碼風格：Google Java Style Guide

## 強制性行為規則
1. **閱讀優先**：修改任何檔案前必須先用 read_file 了解現有實作
2. **先計畫後執行**：複雜任務必須先用 todo_write 列出步驟
3. **修改後必測**：每次程式碼修改後執行 `mvn test -q`
4. **禁止刪除測試**：不可刪除現有測試，只能新增

## 安全紅線（絕對禁止）
- 不可修改 .env、application-prod.yml 等機密配置
- SQL 查詢必須使用參數化，禁止字串拼接
- 不可在程式碼中硬編碼 API Key、密碼、Token
- 不可直接操作生產資料庫

## 命名規範
- 類別：PascalCase
- 方法：camelCase，動詞開頭（findById, createOrder）
- 常數：UPPER_SNAKE_CASE
- DB 欄位：snake_case
...
## 常用指令
- 編譯：`mvn clean compile -q`
- 測試：`mvn test -q`
- 安全掃描：`mvn dependency-check:check -q`
```

#### CLAUDE.md vs Skills vs System Prompt 的職責分工

| 機制 | 作用範圍 | 更新頻率 | 適合存放 |
|------|----------|----------|----------|
| **CLAUDE.md** | 跨 Session，持久化 | 低（專案層面） | 不變的規則、禁止事項、技術規範 |
| **Skills** | 按需載入，暫時 | 中（知識更新） | 技術文件、最佳實踐、領域知識 |
| **System Prompt** | 單次 Session | 高（動態組裝） | 當前任務上下文、記憶摘要 |
...
❌ 不適合放入 CLAUDE.md：
  - 詳細的 API 文件（應放 Skills）
  - 當前任務說明（應放 Prompt）
  - 頻繁更動的配置
  - 過長的內容（建議 < 500 行）
```

> **與 Skills 的關鍵差異**：CLAUDE.md 是「強制規則」，Agent **必須**遵守；Skills 是「參考知識」，Agent 在需要時主動查詢。前者定義邊界，後者擴充能力。
...
在 Agent 具備基本的規劃與錯誤修復能力後，下一步是加入 Hooks 機制，讓系統具備可擴展的監控與安全防護能力，而無需修改核心迴圈。
...
"""
實作 Hooks 機制（對應 s04）
讓 Agent Loop 具備 PreToolUse / PostToolUse / Stop 三種擴展點
"""
...
@on("PreToolUse")
def security_guard(tool_name: str, tool_input: dict, **_) -> dict | None:
    """安全防護：阻止危險命令"""
    if tool_name == "bash":
        cmd = tool_input.get("command", "")
        DANGEROUS = ["rm -rf /", "DROP DATABASE", "chmod 777 /", "curl | bash"]
        for d in DANGEROUS:
            if d.lower() in cmd.lower():
                print(f"🔴 [Security] 阻止危險命令: {cmd}")
                return {"blocked": True, "reason": f"危險命令: {d}"}
    return None

@on("PreToolUse")
def audit_logger(tool_name: str, tool_input: dict, **_):
    """審計日誌：記錄所有工具呼叫"""
    entry = {
        "timestamp": time.time(),
        "event": "PRE_TOOL",
...
@on("PostToolUse")
def token_tracker(tool_name: str, result: str, **_):
    """成本追蹤：累計 Token 使用量（估算）"""
...
@on("Stop")
def completion_notifier(**_):
    """任務完成通知（可整合 Slack / Teams / Email）"""
    print("✅ [Hook] Agent 任務完成")
    # 實際整合：發送 Slack Webhook、觸發下游系統等

# =========================================================
# Step 3: 整合到 Agent Loop（僅需在工具呼叫前後插入 fire()）
# =========================================================
...
def agent_loop_with_hooks(user_message: str):
    ...
    while True:
        ...
        for block in response.content:
            ...
            # ↓ PreToolUse Hook
            hook_result = fire("PreToolUse",
                               tool_name=block.name,
                               tool_input=block.input)
            
            if hook_result.get("blocked"):
                output = f"BLOCKED: {hook_result.get('reason', '安全防護阻止')}"
            else:
                output = TOOL_HANDLERS[block.name](**block.input)
            
            # ↓ PostToolUse Hook
            fire("PostToolUse", tool_name=block.name, result=output)
...
if __name__ == "__main__":
    agent_loop_with_hooks("列出當前目錄下的所有 Python 檔案並統計行數")
```

**Hooks 機制的架構效益**：

```
未使用 Hooks：                    使用 Hooks 後：
┌──────────────┐                 ┌──────────────┐
│ Agent Loop   │                 │ Agent Loop   │
│              │                 │ ↕ PreToolUse │
│  if cmd dangerous:             │   ├─ security_guard
│    return error                │   └─ audit_logger
│  if cmd ran:                   │ ↕ PostToolUse│
│    log_to_file(...)            │   └─ token_tracker
│  if done:                      │ ↕ Stop       │
│    send_notification(...)      │   └─ completion_notifier
└──────────────┘                 └──────────────┘
迴圈程式碼越來越複雜              迴圈程式碼保持乾淨
新增功能需修改核心迴圈             新增功能只需掛鉤
```

> **設計原則**：所有橫切面關注點（安全、審計、監控、通知）都透過 Hooks 實現，而非嵌入核心迴圈。這讓每一段程式碼各司其職，可以獨立測試、獨立部署。
...
Model Context Protocol（MCP）是 Anthropic 於 2024 年推出的開放標準，在 2025-2026 年已成為企業 Agent 生態整合的主流方式。相比手工撰寫 Tool handler，MCP 提供標準化的服務發現、工具協商和安全邊界。
...
│  Agent Runtime                                   │
│  ┌─────────────────────────────────────────────┐ │
│  │ Tool Pool（MCPToolPool）                      │ │
│  │ ├── 內建工具（bash, read_file, etc.）         │ │
│  │ └── MCP 動態工具（按需發現）                  │ │
│  └──────────────────┬──────────────────────────┘ │
...
│  [GitHub]       [PostgreSQL]    [Jira 企業版]    │
│  - 建立 Issue   - 執行查詢     - 建立 Ticket     │
│  - Code Review  - Schema 瀏覽  - 更新 Sprint     │
│  - PR 管理      - Migration    - 讀取任務板      │
└──────────────────────────────────────────────────┘
```
...
```python
# mcp_servers/internal_db_server.py
# 企業自建 MCP Server：封裝內部資料庫存取
...
# 建立 MCP Server 實例
server = Server("internal-db-server")

@server.list_tools()
async def handle_list_tools() -> list[types.Tool]:
    """宣告此 MCP Server 提供的工具清單"""
    return [
        types.Tool(
            name="query_app_db",
            description="查詢應用程式資料庫（唯讀 SELECT）",
            inputSchema={
                "type": "object",
                "properties": {
                    "sql": {
                    ...
                    "limit": {
                        "type": "integer",
                        "description": "最大回傳筆數（預設 50，最大 200）",
                        "default": 50
                    }
                },
                "required": ["sql"]
...
async def handle_call_tool(
    ...
    if name == "query_app_db":
        ...
        # 執行查詢（使用連線池）
        # results = await db_pool.fetch(sql)
        # return [types.TextContent(type="text", text=json.dumps(results))]
        return [types.TextContent(type="text", text="（資料庫查詢結果）")]
        ...
        ORDER BY ordinal_position
        """
        # results = await db_pool.fetch(schema_sql)
        return [types.TextContent(type="text", text=f"Schema for {table}")]
    ...
    async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
        await server.run(
            ...
            InitializationOptions(
                server_name="internal-db-server",
...
```json
{
  "mcpServers": {
    "internal-db": {
      "command": "python",
      "args": ["mcp_servers/internal_db_server.py"],
      "env": {
        "DB_HOST": "internal-postgres.company.com",
        "DB_PORT": "5432",
        "DB_NAME": "app_db",
...
| 風險點 | 防護措施 |
|--------|----------|
| 工具命名衝突 | 使用 `mcp__{server}__{tool}` 命名前綴 |
| 機密資訊洩漏 | MCP Server 內建存取控制，不直接暴露連線字串 |
| 未授權工具呼叫 | 在 PreToolUse Hook 中驗證 MCP 工具白名單 |
| Server 不可用 | 實作 fallback 機制，降級使用內建工具 |
| 供應鏈攻擊 | 僅使用官方或自建 MCP Server，審計第三方套件 |

> **導入建議**：企業初期建議優先自建 MCP Server 封裝內部系統（資料庫、Jira、內部 API），再逐步引入官方 MCP Server（GitHub、Slack 等）。所有 MCP Server 應部署於企業內網，使用服務帳號而非個人 Token。
...
### 12.5 Context Engineering 最佳實踐

Context Engineering 是 2025-2026 年 Agent 工程的核心差異化能力。與傳統 Prompt Engineering（單一輸入最佳化）不同，Context Engineering 涵蓋**跨 Session 狀態管理、多層次指令優先序、動態 Context 壓縮**三個維度，決定了 Agent 在企業長期運行的品質與成本。

#### CLAUDE.md 撰寫最佳實踐

CLAUDE.md 是 Claude Code 跨 Session 持久化指令的載體，其內容在每次新 Session 啟動時自動注入 System Prompt。錯誤的 CLAUDE.md 設計是企業 Agent 品質不穩定的最常見來源。

```markdown
# CLAUDE.md 企業模板（最佳實踐版）

## 角色定位（Role）
你是 [公司名稱] 的企業級 AI 開發助理，專注於 [領域]。
本地 repo 路徑：/home/agent/workspace
主要程式語言：TypeScript + Python

## 硬性規則（Hard Rules）— 永遠遵守，不得例外
- 禁止直接推送到 main/master 分支
- 禁止刪除 production 環境資源
- 所有外部 API 呼叫必須使用 .env 中的配置，不得硬編碼憑證
- 資料庫操作前必須確認備份存在

## 工作流程（Workflow）
1. 接收任務時，先閱讀 docs/architecture.md 了解系統架構
2. 修改前先執行 git status 確認工作目錄乾淨
3. 每完成一個子任務，更新 TODO.md 中的進度
4. PR 前必須通過 npm run lint && npm run test

## 目錄結構慣例（Project Structure）
src/
  api/         # REST API 路由層
  services/    # 業務邏輯層（禁止在此層直接呼叫 DB）
  repositories/# 資料存取層
  models/      # TypeORM Entity 定義

## 常用指令備忘（Quick Commands）
- 啟動開發環境：docker-compose up -d
- 執行測試：npm run test:watch
- 查看 log：docker logs -f app --tail=100
```

> **反模式警告**：CLAUDE.md 不應包含「請盡力做好每件事」、「保持創意」等模糊指令。每一條指令應可驗證（有明確的成功/失敗判斷標準）。

#### System Prompt 快取策略（Prompt Caching）

Anthropic API 支援 Prompt Caching，當 System Prompt 超過 1024 tokens 時，快取可將後續請求的 input token 成本降低 **90%**，並減少首個 token 延遲。

```python
# prompt_cache_config.py
# 正確設定 Prompt Caching（企業 Harness 應在此層實作）

import anthropic

client = anthropic.Anthropic()

def build_cached_request(user_message: str, tools: list) -> dict:
    """建構帶 Prompt Cache 標記的請求"""
    
    # System Prompt 主體（超過 1024 tokens 才觸發快取）
    system_content = [
        {
            "type": "text",
            "text": load_base_system_prompt(),  # 基礎角色定義（穩定、不變）
            "cache_control": {"type": "ephemeral"}  # 標記為可快取
        },
        {
            "type": "text",
            "text": load_project_context(),  # 專案背景（每日更新一次）
            "cache_control": {"type": "ephemeral"}  # 另一個快取層
        },
        {
            "type": "text",
            "text": f"今日日期：{get_today()}｜任務背景：{get_session_context()}"
            # 不加 cache_control：每次請求都變化，不值得快取
        }
    ]
...
def load_base_system_prompt() -> str:
    """載入穩定的基礎 System Prompt（快取命中率高）"""
    # 這部分應包含：角色定義、工具使用規則、輸出格式要求
    # 避免包含任何動態內容（時間、用戶名稱、任務ID）
    with open("prompts/base_system.md") as f:
        return f.read()

def load_project_context() -> str:
    """載入專案背景（每日重新快取）"""
    # 這部分包含：架構摘要、程式碼慣例、API 端點清單
    # 可以在 CI/CD 時自動更新
    with open("prompts/project_context.md") as f:
        return f.read()
```
...
# 企業 Agent 的 Token 預算控制層

class TokenBudgetManager:
    """
    Token 預算三層架構：
    - Session 預算：單次 Agent 任務的上限
    - Daily 預算：每日總消耗上限（接入帳務系統）
    - 模型選擇策略：依任務複雜度動態選擇模型
    """
    ...
    def select_model(self, task_complexity: str) -> str:
        """依複雜度選擇最符合成本效益的模型"""
        mapping = {
            "high":   "claude-opus-4-8",         # 架構設計、複雜推理
            "medium": "claude-sonnet-4-6",        # 一般開發任務
            "low":    "claude-haiku-4-5-20251001" # 格式轉換、文件生成
        }
        return mapping.get(task_complexity, "claude-sonnet-4-6")
    ...
    def record_usage(self, model: str, input_tokens: int, output_tokens: int):
        ...
        if self.spent > self.session_budget * 0.8:
            print(f"[警告] 已消耗 {self.spent:.4f} USD，接近預算上限")
    
    def check_budget(self) -> bool:
        """檢查是否仍有剩餘預算"""
        return self.spent < self.session_budget
    
    def summary(self) -> dict:
        return {
            "total_calls": self.call_count,
            "total_spent_usd": round(self.spent, 4),
            "budget_remaining_usd": round(self.session_budget - self.spent, 4),
            "budget_utilization_pct": round(self.spent / self.session_budget * 100, 1)
        }
```
...
| 策略 | 觸發時機 | 效果 | 風險 |
|------|----------|------|------|
| **Prompt Caching** | System Prompt > 1024 tokens | 節省 90% input 成本 | 快取失效時成本回升 |
| **Context Compact** | Context 達 50% 時 | 維持 Session 可用 | 可能遺失早期細節 |
| **Subagent 隔離** | 獨立子任務 | 避免 Context 膨脹 | 子任務間需要明確傳遞狀態 |
| **行號範圍讀取** | 讀取大型檔案 | 減少無效 Context | 需要事先知道目標行號 |
| **CLAUDE.md 摘要** | 重複性背景資訊 | 減少每次描述成本 | 需定期更新維護 |

> **核心原則**：Context 的品質 > Context 的量。50 個精確相關的 Token 比 5000 個「可能有用」的 Token 更能提升 Agent 輸出品質。定期審視 System Prompt 中哪些內容 Agent 真正「用到了」，刪除未被引用的背景資訊。
...
> - 最後更新：2026-07-01
> - 授權：MIT License
> - 參考專案：[shareAI-lab/learn-claude-code](https://github.com/shareAI-lab/learn-claude-code)（69.3k+ Stars）
