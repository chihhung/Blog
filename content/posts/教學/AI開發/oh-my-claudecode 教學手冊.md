+++
date = '2026-05-02T17:01:55+08:00'
draft = false
...
OMC 由韓國開發者 Yeachan Heo（@Yeachan-Heo）創建，目前在 GitHub 上擁有超過 32,000 顆星，106 位貢獻者，已發佈超過 223 個版本。npm 套件名稱為 `oh-my-claude-sisyphus`。
...
| omc team (CLI) | tmux CLI workers | Codex/Gemini CLI 任務，按需生成 |
| ccg | /ask codex + /ask gemini → Claude 合成 | 混合後端+UI 工作 |
...
> **注意**：`omc autoresearch` CLI 指令已硬棄用為 shim，請改用 in-session skill。
...
| `/ccg` | /ask codex + /ask gemini | 三模型顧問合成 |
...
`OMC_ASK_*` 為正式環境變數。Phase-1 仍接受 `OMX_ASK_ADVISOR_SCRIPT` 和 `OMX_ASK_ORIGINAL_TASK`（附帶棄用警告）。**預定硬日落日期：2026-06-30**。

#### Native Team Worktree Mode（Opt-In，未釋出）

`omc team` runtime-v2 正在引入選配的 worker worktree 模式。Worker 在專屬 git worktree 中運作，而任務生命週期、mailbox、狀態和 manifest 保留在 leader workspace 的 team 協調根目錄下。
...
> **文件維護**：本手冊基於 oh-my-claudecode v4.13.5（2026-04-28）撰寫，最後更新日期 2026-05-02。建議隨 OMC 版本升級同步更新本文件。
> 
> **專案統計**（截至 v4.13.5）：32,000+ GitHub Stars、3,000+ Forks、106 Contributors、223 Releases
> 
> **官方資源**：
