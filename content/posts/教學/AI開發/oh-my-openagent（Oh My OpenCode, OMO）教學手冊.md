...
> **版本**：v5.0｜**最後更新**：2026-05-31  
> **對應 OMO 版本**：v4.5.12（187 releases）  
> **適用對象**：資深工程師、架構師、技術主管  
...
| 主要語言 | TypeScript (91.9%), HTML (5.2%), JavaScript (1.6%), Python (1.0%) |
| OpenCode 最低版本 | v1.14.0（v4.5.1+ 支援 OpenCode 1.15） |
| 官方網站 | [omo.dev](https://omo.dev/) |
| CLI 別名 | `oh-my-opencode`、`oh-my-openagent`、`omo`（推薦）、`lazycodex` |
...
| Kimi K2.6 Prompt | Sisyphus 新增 Kimi K2.6 原生 Prompt 變體（v4.0.0+ 升級至 K2.6） |
...
| Codex CLI Light Edition（v4.5.12+） | 可攜式 OMO 元件運行於 OpenAI Codex CLI。安裝：`bunx omo install --platform=codex` 或 `bunx lazycodex install` |
...
| Kimi K2.6 | 編排替代方案（Sisyphus fallback，v4.0.0+ 升級至 K2.6 原生 Prompt） |
...
graph TB
    ...
    subgraph OMO["oh-my-openagent Plugin"]
        ...
        subgraph Features["核心功能"]
            ...
            Hooks["🪝 Hooks (54+ / 61 with Team Mode)"]
            MCPs["🔌 Built-in MCPs"]
...
|-------|--------|---------|------|------|
| **Sisyphus** | Claude Opus 4.7 | Kimi K2.5 / K2.x / GLM-5 | 主編排器，規劃、委派、驅動任務完成 | 積極平行執行，不會中途停止。v3.17.6+ 新增 Opus 4.7 和 Kimi K2.x 原生 Prompt |
| **Hephaestus** | GPT-5.5 | GPT-5.4 / GPT-5.3-codex (fallback) | 自主深度工作者，端到端執行 | v3.17.6+ 預設從 GPT-5.4 升級至 GPT-5.5，以 outcome-first delegation 重寫 Prompt |
| **Prometheus** | Claude Opus 4.7 | Kimi K2.5 / GLM-5 | 戰略規劃者，面談式需求分析 | /start-work 觸發，建立完整計劃 |
...
Sisyphus 主鏈：
```
claude-opus-4-7 max → kimi-k2.6 → gpt-5.5 medium → glm-5.1 → qwen3.5-plus → big-pickle
```
...
graph LR
    ...
    subgraph OMO_Enhancements["OMO 增強"]
        ...
        OMOHooks["54+ OMO Hooks (61 w/ Team Mode)"]
...
| Kimi Code Subscription | $19 | Kimi K2.6（Sisyphus 備選，v4.0.0 升級） |
...
| 版本 | 安裝指令 | 說明 |
|------|---------|------|
| **Ultimate**（OpenCode） | `bunx omo install` | 完整版：11 個 Agent、54+ hooks、5 個內建 MCP、Team Mode、ulw-loop、ultrawork、hashline |
| **Light**（Codex CLI） | `bunx omo install --platform=codex`<br/>或 `bunx lazycodex install` | 可攜版：rules、comment-checker、LSP、ultrawork、ulw-loop、start-work continuation、telemetry |
| **兩者皆裝** | `bunx omo install --platform=both` | 同時安裝 Ultimate 與 Light |

`--platform` 預設為 `opencode`（Ultimate）。`bunx lazycodex install` 是 `bunx omo install --platform=codex` 的快捷別名。

> **CLI 別名說明**：`oh-my-opencode`、`oh-my-openagent`、`omo`、`lazycodex` 四個 `bunx` 別名都呼叫同一個編譯後的 CLI。`omo` 是推薦的簡短形式。`lazycodex` 僅用於 Light 版安裝快捷方式。
...
| 備選模型 | Kimi K2.6 → GPT-5.5 medium → GLM-5.1 |
...
| Kimi K2.6 | 編排替代 | Sisyphus 備選（K2.x 原生 prompt，v4.0.0 升級） |
...
| 模型更新 | kimi-k2.5→k2.6、glm-5→glm-5.1、minimax-m2.7→qwen3.5-plus |
...
| **Light Edition** | v4.5.12 新增 Codex CLI 可攜版（`bunx omo install --platform=codex`） |

### 12.3 Migration Notes（v3.14.0 → v4.5.12）

#### v4.5.0 → v4.5.12 重要變更

| 變更項目 | 說明 |
|----------|------|
| **Light Edition** | v4.5.12 新增 Codex CLI Light 版本（`bunx omo install --platform=codex` 或 `bunx lazycodex install`），提供可攜式子集：rules、comment-checker、LSP、ultrawork、ulw-loop |
...
| **Model updates** | kimi-k2.5→k2.6、glm-5→glm-5.1、minimax-m2.7→qwen3.5-plus |
...
| 規劃 | Claude Opus 4.7 / Kimi K2.5 | 結構化思維 |
...
| Agent | 主模型 | 角色 |
|-------|--------|------|
| Sisyphus | Claude Opus 4.7（+ Kimi K2.6 variant） | 主編排器 |
| Hephaestus | GPT-5.5（fallback: GPT-5.4） | 深度自主工作者（outcome-first delegation） |
| Prometheus | Claude Opus 4.7（+ Kimi K2.6 / GLM-5.1 fallback） | 戰略規劃者 |
...
| `deep` | GPT-5.5（v3.17.6+） | GPT-5.4 → kimi-k2.6 | 自主研究 + 執行 |
...
| `writing` | Claude Opus 4.7 | kimi-k2.6 | 文件撰寫 |

### 18.4 Hook 一覽（54 內建 / 61 with Team Mode）
