...
> **版本**：v3.2｜**最後更新**：2026-04-30  
> **對應 OMO 版本**：v3.17.10（168 releases）  
> **適用對象**：資深工程師、架構師、技術主管  
...
| Kimi K2.x Prompt | Sisyphus 新增 Kimi K2.x 原生 Prompt 變體（v3.17.6+） |
...
Sisyphus 主鏈：
```
claude-opus-4-7 max → k2p5 → kimi-k2.5 → gpt-5.5 medium → glm-5 → big-pickle
```

> **設計原理**：Agent 說「需要什麼類型的工作」，Harness 挑選對應模型。`ultrabrain` 類別現在預設路由到 GPT-5.5 xhigh（v3.17.6+），`quick` 類別預設路由到 GPT-5.4-mini（v3.13.0+），`deep` 類別預設路由到 GPT-5.5（v3.17.6+）。使用者無需手動配置。若 Provider 回傳 insufficient balance/credits/forbidden 錯誤，runtime 會自動將其視為 quota exhaustion 並降級至下一個模型（v3.17.10+）。
...
| Kimi Code Subscription | $19 | Kimi K2.5 / K2.x（Sisyphus 備選） |
...
### 10.3 Migration Notes（v3.14.0 → v3.17.10）
...
| `deep` | GPT-5.5（v3.17.6+） | 自主研究 + 執行 |
| `quick` | GPT-5.4-mini | 單檔修改 |
| `ultrabrain` | GPT-5.5 xhigh（v3.17.6+） | 困難邏輯 / 架構 |
| `unspecified-high` | GPT-5.5 high（v3.17.6+） | 一般高強度 |
...
> 基於 GitHub Repository v3.17.10 版本資訊撰寫。  
