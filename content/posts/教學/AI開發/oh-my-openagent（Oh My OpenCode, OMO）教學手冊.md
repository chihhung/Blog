...
> **版本**：v4.0｜**最後更新**：2026-05-14  
> **對應 OMO 版本**：v4.1.2（177 releases）  
> **適用對象**：資深工程師、架構師、技術主管  
...
| Boulder 追蹤（v4.1 新增） | 多 Session 工作追蹤，per-task 計時器、完成偵測、耗時提示。`bunx oh-my-opencode boulder` CLI 儀表板 |
...
| 匿名遙測 | PostHog 匿名 DAU 追蹤（可停用），v3.17.0+ |
...
| Kimi K2.5 / K2.x | 編排替代方案（Sisyphus fallback，v3.17.6+ 新增 K2.x 原生 Prompt） |
| GLM-5 | 編排備選（unspecified-high 預設） |
| GPT-5.5 | 推理、深度工作、驗證（Oracle / Hephaestus / deep 類別預設，v3.17.6+ 從 GPT-5.4 自動遷移） |
...
> OMO 預設啟用匿名遙測（PostHog），僅追蹤 DAU/WAU/MAU，使用 hashed installation identifier（非原始 hostname），且不建立 PostHog person profiles。單一事件 `omo_daily_active` 每個 UTC 日每台機器最多傳送一次。v3.17.13+ 已縮減至僅傳送 `omo_daily_active` 事件，移除了 `plugin_loaded` 事件和 feature flags。可透過以下環境變數停用：
...
| 備選模型 | Kimi K2.5 / K2.x → GPT-5.5 medium → GLM-5 |
...
| Kimi K2.5 / K2.x | 編排替代 | Sisyphus 備選（K2.x 原生 prompt） |
...
### 12.3 Migration Notes（v3.14.0 → v4.1.2）
...
v3.17.0 開始，OMO 預設啟用 **PostHog 匿名遙測**。v3.17.13 起範圍大幅收窄，目前**僅收集 `omo_daily_active` 單一事件**（每日活躍計數），不含任何個人資訊、lifecycle 事件或 exception 回報。
...
v3.17.0 起，OMO 預設啟用 **PostHog 匿名遙測系統**。v3.17.13 起大幅收窄範圍，目前**僅收集單一事件 `omo_daily_active`**（每日活躍使用者計數），不包含任何功能使用統計、lifecycle 事件或 exception 回報。

**遙測架構**（v3.17.13+ / v4.0.0+）：
...
flowchart TD
    ...
    Events --> E1["omo_daily_active<br/>（每日一次）"]
    
    PostHog --> Dashboard["DAU 計數儀表板"]
```

> 📌 **v4.0.0 變更**：`plugin_loaded` 事件移除、feature flags 停用。v3.17.13 起移除 `cli_run_start`、`cli_run_end`、`install_command`、`activity_state` 等所有 lifecycle 事件及 `captureException`。
...
| `omo_daily_active` | 每日活躍使用者計數（每日僅觸發一次） | ❌ |
| 原始碼內容 | — | ❌ 不收集 |
| API Key / Secrets | — | ❌ 不收集 |
| 使用者身份 | — | ❌ 不收集 |
...
| `boulder` | Boulder 追蹤儀表板（v4.1.0+，CLI：`bunx oh-my-opencode boulder`） |

### 18.2 Agent 一覽表

| Agent | 主模型 | 角色 |
|-------|--------|------|
| Sisyphus | Claude Opus 4.7（+ Kimi K2.x variant） | 主編排器 |
| Hephaestus | GPT-5.5（fallback: GPT-5.4） | 深度自主工作者（outcome-first delegation） |
| Prometheus | Claude Opus 4.7 | 戰略規劃者 |
