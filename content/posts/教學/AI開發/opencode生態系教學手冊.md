...
> **版本**：基於 OpenCode v1.4.3（2026 年 4 月）  
...
- [第十章：Oh My OpenAgent（OmO）生態系](#第十章oh-my-openagentomo生態系)
  ...
  - [10.29 OmO v3.14-v3.17 更新摘要](#1029-omo-v314-v317-更新摘要)
- [第十一章：OpenWork 桌面應用與協作平台](#第十一章openwork-桌面應用與協作平台)
  ...
  - [11.18 OpenWork v0.11.192-v0.11.206 更新摘要](#1118-openwork-v011192-v011206-更新摘要)
...
| 語言/技術 | 佔比 | 用途 |
|-----------|------|------|
| TypeScript | 51.6% | 核心引擎、工具系統 |
| MDX | 34.0% | 文件系統 |
| JavaScript | 11.1% | 相容層、工具鏈 |
| CSS | 2.6% | TUI 樣式 |
...
graph TB
    subgraph "OpenCode 生態系"
        subgraph "核心層"
            OC["OpenCode Core<br/>v1.4.3<br/>142k ⭐ | MIT"]
        end

        subgraph "增強層"
            OmO["Oh My OpenAgent (OmO)<br/>v3.17.0<br/>50.7k ⭐ | SUL"]
            OW["OpenWork<br/>v0.11.206<br/>13.6k ⭐ | FSL-1.1-MIT"]
        end
...
|------|------|--------|------|-------|------|
| **OpenCode** | AI 編碼代理核心 | [anomalyco/opencode](https://github.com/anomalyco/opencode) | v1.4.3 | 142k | MIT |
| **Oh My OpenAgent** | 進階外掛（多代理協作） | [code-yeongyu/oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent) | v3.17.0 | 50.7k | SUL |
| **OpenWork** | 桌面 GUI 協作平台 | [different-ai/openwork](https://github.com/different-ai/openwork) | v0.11.206 | 13.6k | FSL-1.1-MIT |
...
2. **內建工具系統**：bash、edit、write、read、grep、glob、list、patch、lsp、skill、todowrite、webfetch、websearch、question 等 14+ 工具讓 AI 能夠直接操作開發環境
...
| v1.4.3 | 2026-04-10 | 最新穩定版；OpenCode Go 訂閱、Docker 安裝、Ollama Cloud、40+ 提供商支援、VS Code SDK v1.4.3 |
...
graph TB
    subgraph "介面層（Clients）"
        TUI["TUI（終端機介面）"]
        Desktop["Desktop App"]
        IDE["IDE Extension<br/>（VS Code SDK v1.4.3）"]
        Web["Web 介面"]
        Mobile["遠端操控<br/>（行動裝置）"]
    ...
    subgraph "OpenCode Server 層"
        ...
        Tools["內建工具 14+<br/>bash/edit/write/read/grep..."]
...
OpenCode 內建 14+ 工具，讓 AI Agent 能直接操作開發環境：
...
> **注意**：`write`、`patch` 工具均受 `edit` 權限控制，該權限涵蓋所有檔案修改操作。`websearch` 無需 API 金鑰，可直接連接 Exa AI 託管 MCP 服務。底層搜尋工具（`grep`、`glob`、`list`）使用 [ripgrep](https://github.com/BurntSushi/ripgrep)，預設遵循 `.gitignore` 規則。
...
OpenCode 提供 **VS Code 擴充（SDK v1.4.3）**：
...
| 知識產權問題 | 授權模式審查（MIT / FSL-1.1-MIT）+ 法務確認 |
...
OmO 自 v3.13 至 v3.17 持續快速迭代，以下為關鍵更新：
...
| v3.17 | Model Fallbacks 混合格式（純字串與物件設定共存）；Skills 新增 `frontend-ui-ux`；效能改進 |
...
| 貢獻者 | **53+** |
| 版本發佈 | **1,055+** |
| 授權 | **FSL-1.1-MIT**（Functional Source License，轉換為 MIT） |
| 技術堆疊 | TypeScript 82.8% / JavaScript 7.6% / Rust 5.2% / CSS 3.0% / Shell 0.8%（Tauri） |
...
### 11.18 OpenWork v0.11.192-v0.11.206 更新摘要
...
OpenWork 自 v0.11.204 起將授權從 MIT 變更為 **FSL-1.1-MIT**（Functional Source License 1.1）：

| 項目 | 說明 |
|------|------|
| 意義 | 原始碼開放可閱讀，但有特定使用限制 |
| 兩年轉換 | 每個版本發佈兩年後自動轉換為 MIT 授權 |
| 企業影響 | 內部使用不受限，但競品式服務需留意 |
...
> **文件結束**  
> 本文件涵蓋 OpenCode v1.4.3、Oh My OpenAgent v3.17.0、OpenWork v0.11.206 的完整生態系教學。  
