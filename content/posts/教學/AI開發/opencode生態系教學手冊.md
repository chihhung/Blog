...
> **版本**：基於 OpenCode v1.15.13（2026 年 5 月）  
...
- [第十一章：OpenWork 桌面應用與協作平台](#第十一章openwork-桌面應用與協作平台)
  ...
  - [11.19 企業導入 OpenWork 建議](#1119-企業導入-openwork-建議)
...
OpenCode 是一個 **100% 開源** 的 AI 編碼代理（AI Coding Agent），由 Anomaly 團隊開發維護。截至 2026 年 5 月，OpenCode 已達到以下里程碑：
...
| 語言/技術 | 佔比 | 用途 |
|-----------|------|------|
| TypeScript | 66.5% | 核心引擎、工具系統、代理系統 |
| MDX | 29.9% | 文件系統 |
| CSS | 3.1% | TUI 樣式、Desktop App |
| HTML | 0.2% | 網頁模板 |
| Astro | 0.1% | 文件網站 |
...
graph TB
    subgraph "OpenCode 生態系"
        subgraph "核心層"
            OC["OpenCode Core<br/>v1.15.13<br/>168k ⭐ | MIT"]
        end

        subgraph "增強層"
            OmO["Oh My OpenAgent (OmO)<br/>v4.5.12<br/>60.4k ⭐ | SUL"]
            OW["OpenWork<br/>v0.14.0<br/>15.7k ⭐ | MIT"]
        end

        subgraph "整合層"
            SDK["OpenCode SDK<br/>v1.15.x"]
...
|------|------|--------|------|-------|------|
| **OpenCode** | AI 編碼代理核心 | [anomalyco/opencode](https://github.com/anomalyco/opencode) | v1.15.13 | 168k | MIT |
| **Oh My OpenAgent** | 進階外掛（多代理協作） | [code-yeongyu/oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent) | v4.5.12 | 60.4k | SUL |
| **OpenWork** | 桌面 GUI 協作平台 | [different-ai/openwork](https://github.com/different-ai/openwork) | v0.14.0 | 15.7k | MIT |
...
| v1.15.13 | 2026-05-31 | **最新穩定版**；168k Stars、921 貢獻者、815 releases；database schema 重構（migration 機制優化）、keymap fallback 改進、gcp metadata 修正、stats 路由功能新增、nix flake 更新、OpenCode Zen 供應商支援、ACP 支援、external_directory/doom_loop 權限機制 |
...
| v4.5.12 | **最新版**，187 releases、60.4k ⭐、268 貢獻者；**Multi-Harness Agent OS 重構**——支援 OpenCode/Codex CLI/Pi 多平台、兩個版本（Ultimate for OpenCode、Light for Codex CLI）、lazycodex 別名、omo-codex Light 可攜版、security-review 內建技能強化、54+ 生命週期掛鉤（61 with Team Mode）、.agents 目錄完整遷移 |
...
| v0.14.0 | **最新版**，1,346 releases、15.7k ⭐、62 貢獻者；定位轉型為「open source alternative to Claude Cowork/Codex」、Google Workspace OAuth 身份驗證、Voice mode CDP 音訊檢查、pnpm v11 遷移、內建擴充市集（marketplace）、SCIM/SSO 企業管理流程、新版訊息列表 UI、embedded OpenCode 升級至 v1.15.12、Windows 付費支援計畫 |
...
graph TB
    subgraph "介面層（Clients）"
        TUI["TUI（終端機介面）"]
        Desktop["Desktop App"]
        IDE["IDE Extension<br/>（VS Code SDK v1.15.13）"]
        Web["Web 介面"]
        Mobile["遠端操控<br/>（行動裝置）"]
...
OpenCode 提供 **VS Code 擴充（SDK v1.15.13）**：
...
| 架構 | **Multi-Harness Agent OS**（支援 OpenCode/Codex CLI/Pi） |
...
> **重要**：這一重構代表 OmO 不再僅是 OpenCode 的外掛，而是演進為狨立的 Agent OS 平台，可在多個 AI 編碼工具上運行。
...
| 版本發佈 | **1,346+** |
| 授權 | **MIT** |
| 技術堆疊 | TypeScript 85.6% / JavaScript 9.2% / CSS 2.0% / Swift 1.3% / Shell 1.3% / MDX 0.6% |
| OpenCode 版本 PIN | 透過 `constants.json` 固定（當前內嵌 v1.15.12） |
| 官方網站 | [openworklabs.com](https://openworklabs.com) |
| Desktop Framework | **Electron**（從 Tauri 遷移至 Electron，v0.13.0） |
| 應用程式語言 | 英文、法文、西班牙文、加泰隆尼亞文、巴西葡萄牙文、日文、簡體中文、泰文、越南文、俄文（10 語系） |
| Windows 支援 | 透過付費支援計畫提供 |
| 企業方案 | 提供 Enterprise 版本，含 SCIM/SSO 管理 |
...
| v0.14.0 | **最新版**（1,346 releases、15.7k ⭐、62 貢獻者）；定位轉型為「open source alternative to Claude Cowork/Codex」、**Google Workspace OAuth** 身份驗證、**Voice mode CDP** 音訊檢查功能、**pnpm v11** 遷移、內建擴充市集（built-in extensions marketplace）、**SCIM/SSO** 企業管理流程、新版訊息列表 UI、embedded OpenCode 升級至 v1.15.12、Windows 透過付費支援計畫提供 |
...
> **文件結束**  
> 本文件涵蓋 OpenCode v1.15.13、Oh My OpenAgent v4.5.12、OpenWork v0.14.0 的完整生態系教學。  
