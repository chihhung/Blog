...
> **版本**：基於 OpenCode v1.14.50（2026 年 5 月）  
...
- [第十章：Oh My OpenAgent（OmO）生態系](#第十章oh-my-openagentomo生態系)
  ...
  - [10.25 GPT-5.4 xhigh 路由與 ultrabrain 模式（v3.13+ 新增）](#1025-gpt-54-xhigh-路由與-ultrabrain-模式v313-新增)
  ...
  - [10.30 OmO v4.0-v4.1.2 更新摘要（Team Mode 重大版本）](#1030-omo-v40-v412-更新摘要team-mode-重大版本)
- [第十一章：OpenWork 桌面應用與協作平台](#第十一章openwork-桌面應用與協作平台)
  ...
  - [11.18 OpenWork v0.11.192-v0.13.0 更新摘要](#1118-openwork-v011192-v0130-更新摘要)
...
| 語言/技術 | 佔比 | 用途 |
|-----------|------|------|
| TypeScript | 57.4% | 核心引擎、工具系統 |
| MDX | 29.5% | 文件系統 |
| JavaScript | 10.4% | 安裝腳本、工具、測試 |
| CSS | 2.3% | TUI 樣式 |
| HTML | 0.2% | 網頁模板 |
| Astro | 0.1% | 文件網站 |
| Other | <0.1% | Rust、Shell、CI/CD |
...
graph TB
    subgraph "OpenCode 生態系"
        subgraph "核心層"
            OC["OpenCode Core<br/>v1.14.50<br/>160k ⭐ | MIT"]
        end

        subgraph "增強層"
            OmO["Oh My OpenAgent (OmO)<br/>v4.1.2<br/>57.7k ⭐ | SUL"]
            OW["OpenWork<br/>v0.13.8<br/>15.2k ⭐ | MIT"]
        end

        subgraph "整合層"
            SDK["OpenCode SDK<br/>v1.14.x"]
...
|------|------|--------|------|-------|------|
| **OpenCode** | AI 編碼代理核心 | [anomalyco/opencode](https://github.com/anomalyco/opencode) | v1.14.50 | 160k | MIT |
| **Oh My OpenAgent** | 進階外掛（多代理協作） | [code-yeongyu/oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent) | v4.1.2 | 57.7k | SUL |
| **OpenWork** | 桌面 GUI 協作平台 | [different-ai/openwork](https://github.com/different-ai/openwork) | v0.13.8 | 15.2k | MIT |
...
2. **內建工具系統**：bash、edit、write、read、grep、glob、list、patch、multiedit、lsp、skill、todowrite、webfetch、websearch、question 等 15+ 工具讓 AI 能夠直接操作開發環境
...
| v1.14.50 | 2026-05-14 | **最新穩定版**；160k Stars、896 貢獻者、800 releases；Scout SubAgent 新增（外部文件與依賴研究唯讀代理）、native LLM core foundation、TUI notifications 與 attention sounds、Electron Desktop CI native arm64 runner、keymap fallback priority 與 TUI config diagnostics、v2 model listing API、session usage totals tracking |
...
| v4.1.2 | **最新版**，177 releases、57.7k ⭐、219 貢獻者；Discipline Agents 模型更新（Sisyphus: claude-opus-4-7/kimi-k2.6/glm-5.1、Hephaestus: gpt-5.5、Prometheus: claude-opus-4-7/kimi-k2.6/glm-5.1）、configurable agent ordering、CDP browser tools、safe deps refresh、ralph-loop compaction continuation ownership guard、.opencode 目錄遷移至 .agents |
...
graph TB
    subgraph "介面層（Clients）"
        TUI["TUI（終端機介面）"]
        Desktop["Desktop App"]
        IDE["IDE Extension<br/>（VS Code SDK v1.14.50）"]
        Web["Web 介面"]
        Mobile["遠端操控<br/>（行動裝置）"]
    ...
    subgraph "OpenCode Server 層"
        ...
        Tools["內建工具 15+<br/>bash/edit/write/read/grep/multiedit..."]
...
OpenCode 內建 15+ 工具，讓 AI Agent 能直接操作開發環境：
...
| `multiedit` | 多檔案編輯 | edit | 單次呼叫即可對多個檔案執行編輯（受 `edit` 權限控制） |

> **注意**：`write`、`patch`、`multiedit` 工具均受 `edit` 權限控制，該權限涵蓋所有檔案修改操作（`edit`、`write`、`patch`、`multiedit`）。`websearch` 無需 API 金鑰，可直接連接 Exa AI 託管 MCP 服務。底層搜尋工具（`grep`、`glob`、`list`）使用 [ripgrep](https://github.com/BurntSushi/ripgrep)，預設遵循 `.gitignore` 規則。
...
OpenCode 提供 **VS Code 擴充（SDK v1.14.50）**：
...
#### Scout SubAgent（v1.14.50+）

Scout SubAgent 是 v1.14.50 新增的 **只讀外部研究** 子代理，專門用於查詢外部文件與相依套件：
...
> **品牌說明**：Oh My OpenAgent 已將 npm 套件名正式更名為 `oh-my-openagent`（舊名 `oh-my-opencode` 過渡期仍可安裝）。GitHub repo 與官方品牌統一使用 `oh-my-openagent`。CLI 入口為 `ultrawork` 或 `ulw`。
...
graph TB
    subgraph "Discipline Agents 編排架構"
        ...
        subgraph "專業代理"
            Hephaestus["🔨 Hephaestus<br/>深度工匠<br/>gpt-5.4"]
...
| **Hephaestus** 🔨 | 深度工匠 | `gpt-5.4` | 複雜程式碼實作、長時間深度工作（The Legitimate Craftsman） |
| **Prometheus** 🔥 | 策略規劃 | `claude-opus-4-6` / `kimi-k2.5` / `glm-5` | 多方案分析、風險評估、長期規劃 |
...
| `deep` | 自主研究 + 端到端執行 | `gpt-5.4` |
| `quick` | 單檔案修改、修正錯字 | 快速模型 |
| `ultrabrain` | 高難度邏輯、架構決策 | `gpt-5.4 xhigh` |
...
| 規劃諮詢 | ❌ | ✅ Metis（v3.12+） |
| ultrabrain 模式 | ❌ | ✅ GPT-5.4 xhigh（v3.13+） |
...
| **ultrabrain** | Sisyphus + GPT-5.4 xhigh | 超複雜推理、跨領域分析 |
...
| 模型偏好 | Thinking Model（高推理） | claude-opus-4-6 |
...
### 10.25 GPT-5.4 xhigh 路由與 ultrabrain 模式（v3.13+ 新增）
...
graph TB
    subgraph "Agent Orchestration 四大類別"
        ...
        Deep["🔨 deep<br/>Hephaestus<br/>gpt-5.4"]
        Quick["⚡ quick<br/>Librarian<br/>claude-haiku-4-5"]
        Ultra["🧠 ultrabrain<br/>Sisyphus<br/>GPT-5.4 xhigh"]
    end
...
| 特點 | 說明 |
|------|------|
| 模型 | GPT-5.4 xhigh（最高推理能力） |
| 上下文 | 最大可用上下文視窗 |
| Token 消耗 | 極高（建議僅用於關鍵任務） |
| 適用場景 | 跨領域架構設計、安全審計、遺留系統分析 |
| 自動降級 | 若 xhigh 不可用，自動降級為 opus-4-6 |
...
### 10.30 OmO v4.0-v4.1.2 更新摘要（Team Mode 重大版本）

OmO 在 v4.0 實現了重大版本跳躍，引入 **Team Mode** 多代理並行系統，以下為完整更新：
...
| 版本發佈 | **1,202+** |
| 授權 | **MIT** |
| 技術堆疊 | TypeScript 86.4% / JavaScript 9.4% / CSS 2.6% / Shell 0.9% / MDX 0.7% |
| OpenCode 版本 PIN | 透過 `constants.json` 固定 |
...
> **遷移說明**：v0.13.0 之前使用 Tauri（Rust 後端 + WebView），遷移至 Electron 後改用 Node.js 主程序。Rust 程式碼比例從 4.9% 降至 3.6%，JavaScript 比例從 7.1% 升至 8.3%。
...
### 11.18 OpenWork v0.11.192-v0.13.0 更新摘要
...
| v0.13.8 | **最新版**（1,202 releases、15.2k ⭐、61 貢獻者）；Electron browser automation in dev 完整啟用、CDP browser tools（直接 OpenCode 工具，無需 MCP，多目標）、AGENTS.md 更新、安全性改進 |
...
> **文件結束**  
> 本文件涵蓋 OpenCode v1.14.50、Oh My OpenAgent v4.1.2、OpenWork v0.13.8 的完整生態系教學。  
