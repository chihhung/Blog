...
> **版本**: 10.0  
> **最後更新**: 2026年5月28日  
> **適用於**: Spec-Kit v0.8.16+ / Spec Kit Templates - 0.8.16  
...
- [2.15 Self Management 指令（v0.7.5 新增）](#215-self-management-指令v075-新增)
...
Spec-Kit 支援多種主流 AI 編碼助手（截至 v0.8.16，30+ 個代理）：
...
- 🔧 **自訂代理** → Generic 模式（搭配 `--integration generic --ai-commands-dir <path>`）

> 💡 **v0.7.1 起，`--ai` 旗標已棄用**，請統一使用 `--integration`。舊寫法仍可用但會顯示棄用警告。
...
| `--integration <tool>` | 指定 AI 整合（v0.7.1+ 建議，取代 `--ai`） | `copilot`、`claude`、`cursor-agent`、`gemini`、`qwen`、`opencode`、`codex`、`windsurf`、`junie`、`kilocode`、`auggie`、`roo`、`codebuddy`、`amp`、`shai`、`kiro-cli`（別名：`kiro`）、`bob`、`qodercli`、`agy`、`tabnine`、`vibe`、`kimi`、`iflow`、`pi`、`trae`、`forge`、`goose`、`devin`、`generic` |
...
```bash
specify init my-project --integration iflow
```
...
截至 v0.8.16，社群目錄中已有豐富的擴充可供使用（約 100+ 個）：
...
### 2.15 Self Management 指令（v0.7.5 新增）
...
# 升級 CLI 至最新版（stub，未來將完整實作）
specify self upgrade
```

> 💡 目前 `self check` 與 `self upgrade` 為初步實作（stub），未來版本將提供完整的自動升級功能。現階段仍建議使用 `uv tool install --force` 或 `pipx install --force` 手動升級。
...
v0.8.2 已將所有官方文件中的 `--ai` 旗標替換為 `--integration`。雖然 `--ai` 仍可使用（向下相容），但強烈建議遷移至 `--integration`：
...
此功能對**企業環境**特別重要——團隊可以統一設定認證方式，避免每個開發者各自管理 Token。
...
v0.8.16（2026-05-27）是截至本文撰寫時的**最新版本**，引入工作流預設系統與規格品質閉環驗證。
...
> **升級提示**：從任何版本升級至 v0.8.16，請使用以下指令。詳見 [升級指南](https://github.com/github/spec-kit/blob/main/docs/upgrade.md)：
>
> ```bash
> # 檢查是否有新版本
> specify self check
>
> # 升級 CLI（uv）
> uv tool install specify-cli --force --from git+https://github.com/github/spec-kit.git@v0.8.16
>
> # 或使用 pipx
> pipx install --force git+https://github.com/github/spec-kit.git@v0.8.16
>
...
*文件版本: v10.0 | 適用 Spec-Kit v0.8.16+*  
*最後更新: 2026-05-28*  
*適用於: Spec-Kit v0.8.16+ / Spec Kit Templates - 0.8.16*
